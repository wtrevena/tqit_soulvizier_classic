...
* `WIN_BACKUP_ROOT=...` REQUIRED by `deploy_to_custommaps.ps1`: the network-drive root for deploy + character snapshots, e.g. `Z:\Computer Backup\tqit_soulvizier_classic`. The deploy refuses to run if the key is missing, the path is not fully qualified (a drive path `Z:\...` or a UNC path `\\server\share\...`; drive-relative `C:foo` and root-relative `\foo` are rejected because they resolve against the current directory), the path is inside the repo, the root or any folder above it is a junction / symlink / mount point, the drive is unreachable, or the root is not writable. No local fallback.
...
* Before copying, back up any existing deployed folder:
  ...
  * Copy it to `<WIN_BACKUP_ROOT>\deployed\SoulvizierClassic\<timestamp>\` on the network drive (R-259) after refusing a target that is or contains a junction, verify the copy by file count AND total bytes, then keep only the newest `BACKUP_KEEP` (default 5) snapshots. Character saves go to `<WIN_BACKUP_ROOT>\characters\<timestamp>\` (newest 10 kept).
