r"""uber_hoard_generosity - R-250 (Will 2026-08-14): the uber/boss chests pay like ubers.

WILL, VERBATIM (2026-08-14), five reports in one play session - the full quotes and the
whole measurement live in `tools/svc_uber_hoards.py` and docs/WILL_RULINGS.md R-250:
...
_r250_exempt` takes the family out of R-240's trim so it keeps the volume the monolith
...
trim UNTRIMMED, which is the live proof that the R-250 carve-out actually fired - and it
...
MODULE_NAME = ("uber/boss chest generosity - every hoard chest opens its own table at "
               "uber volume, and the Secret Present box pays 3x (R-250)")
...
def apply(db, tags):
    ...
    # ── PROOF THAT THE R-250 CARVE-OUT FIRED ─────────────────────────────────
    ...
    if trimmed:
        ...
        raise SystemExit(
            ...
            "`_r250_exempt` stopped covering the family or a new writer appeared. R-240's "
    ...
    print("    uber/boss hoard family: %d table(s) came through R-240 untrimmed at %s "
          "(the R-250 carve-out fired)." % (len(SUH.tables(db, lk)), SUH.HOARD_MIN_EQ))
