r"""svc_uber_hoards.py - THE UBER/BOSS CHEST GENEROSITY CONTRACT (Will 2026-08-14, R-250).
...
WHAT R-250 DOES, AND THE LINE IT DOES NOT CROSS
------------------------------------------------
...
2. VOLUME (the second nerf, undone by scope). The family LEAVES R-240's trim scope
   (`svc_loot_volume.R250_UBER_HOARD_EXEMPT`) exactly the way R-247.7(a) took the
...
NOT TOUCHED, deliberately, and each for a stated reason:
  ...
  * COMPOSITION anywhere - members, weights, group chances and the guaranteed row belong
    to R-180/R-181/R-220. R-250 moves a `tables` pointer and a `numSpawn` multiplier.
...
# with these exact two equations. R-240 trimmed them; R-250 takes the family out of that
...
def is_hoard_table(path):
    """Is this record one of the uber/boss hoard loot tables R-250 governs?

    Name-shape based and shared with `svc_loot_volume.R250_UBER_HOARD_EXEMPT`, so the
...
def problems(db, lk=None, report=None):
    ...
    # shipped, and the one that reds on every pre-R-250 artifact by design.
    ...
    for key in sorted(table_map):
        ...
        if str(mn) != HOARD_MIN_EQ or str(mx) != HOARD_MAX_EQ:
            out.append("H3 %s carries numSpawn %r/%r; R-250 pins the family at %r/%r "
                       ...
                       "or the R-250 carve-out stopped covering this record."
...
def calibrate(db, lk=None):
    ...
    print("\n=== R-250 uber/boss hoard family: %d chest(s), %d table(s) ==="
...
def pass_line(report):
    return ("R-250 PASS: %d uber/boss hoard chest(s) each name their OWN loot table, "
