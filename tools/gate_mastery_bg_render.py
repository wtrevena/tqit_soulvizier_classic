r"""MASTERY-UI RENDER GATE (b60 wave, ships build42, fail-loud): every UI
background / icon bitmap referenced by a LIVE mastery skill-pane record must
RESOLVE in the shipped arc set (mod Resources UNION base-game Resources,
engine-faithful archive rule), AND every one of the 9 live pane-background
records must be the vanilla `BitmapUIAware` widget class with a non-empty
`bitmapNames` array.

Born from the b60 black-background RCA (docs/reports/b60_mastery_bg_render.md):
...
def validate(arz_path, mod_resources, game_dir):
    ...
    print("=" * 72)
    print("MASTERY-UI RENDER GATE (b60 wave): resolution + pane structure")
    print("  arz :", arz_path)
