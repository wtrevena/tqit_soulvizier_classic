...
# ── R-250 SCOPE CARVE-OUT (Will 2026-08-14) ─────────────────────────────────────────
...
# were orphans nobody could open (docs/WILL_RULINGS.md R-250; tools/svc_uber_hoards.py
# has the full measurement). R-250 re-wires them, which makes them LIVE - and a live
...
# carve-out and R-250's own contract are provably the same set - a second typed list is
...
# volume - stays valid across R-250.
try:
    from svc_uber_hoards import is_hoard_table as _r250_exempt
except ImportError:                                              # pragma: no cover
    def _r250_exempt(path):                                      # pragma: no cover
        raise SystemExit(
            "[svc_loot_volume] tools/svc_uber_hoards.py is MISSING, so R-250's uber/boss "
...
def _exempt(path):
    """Out of R-240's trim scope: the R-247.7a stash, or the R-250 uber/boss hoards."""
    return _r247_exempt(path) or _r250_exempt(path)
    ...
    for _label, tables, _w, tier in SAB.all_surfaces(db, lk):
        for t in tables:
            ...
            if _exempt(real):
                ...
                # R-250:    so did every uber/boss `svc_*hoard_loot_0N` table.
                continue
...
def problems(db, lk=None, report=None):
    ...
    for label, tables, weights, tier in SAB.all_surfaces(db, lk):
        ...
        if all(_exempt(t) for t in tables):
            ...
            # R-250:    the uber/boss hoards are ruled uber-rich; gate_uber_hoard_
