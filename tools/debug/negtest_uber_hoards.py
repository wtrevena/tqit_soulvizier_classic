r"""negtest_uber_hoards.py - the NEGATIVE battery for R-251 (Will 2026-08-14).

A gate nobody has watched FAIL is a gate nobody should trust, and R-251 exists because
...
def _healthy():
    """A post-R-251 database in miniature: 3 families, wired, at authored volume."""
    db = _Stub()
...
def _p_h8_relic_tier(db):
    ...
    on the shipped b888f022, and invisible to the wire-walking relic gate until R-251
...
def _p_h8_gift_tier(db):
    """The Secret Present box is per-difficulty too, and R-251 is the wave that changes
...
PLANTS = [
    ...
    ('N9 H8 a Normal-tier relic on the Epic hoard table (the R-251 round-2 defect)',
     _p_h8_relic_tier, 'H8'),
...
def main(argv):
    print("\n=== R-251 negative battery (uber/boss chest generosity) ===")
