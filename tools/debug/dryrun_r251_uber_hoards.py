r"""dryrun_r251_uber_hoards.py - apply R-251 IN MEMORY to a built arz, then gate it.

WHY THIS EXISTS. R-251 lands in three places a static reader cannot join up on its own:
...
WHAT IT REPRODUCES, and where the real thing lives:
  0. `_create_obsidian_roulette` (content pass)    - the guaranteed relic row, per tier.
     ...
     the Epic/Legendary hoards guaranteed an Essence relic. R-251's own wiring is what
  ...
  1. `_create_propontis_superboss` (content pass)  - author svc_dorushoard_loot_0N and
     ...
     R-251 separated them. Cloning the raw donor here instead reds `gate_loot_distribution`
     D3/D5/D8 on exactly the three new records and nothing else, which is a measurement
     of the missing widening, not of R-251. The authored tables are also registered with
  ...
  2. `svc_loot_volume._r251_exempt` (the trim's scope) - restore every uber/boss hoard
...
Usage:
  py tools/debug/dryrun_r251_uber_hoards.py <arz>            # apply + gate + report
  py tools/debug/dryrun_r251_uber_hoards.py <arz> --before   # gate the arz as-is (control)
"""
import sys
...
def simulate(db, lk, raw_donor=False):
    ...
    # relic gate walks the WIRE and those chests named `boss_default_*` - so R-251's own
    ...
    for t in SUH.TIERS:
        ...
        if not lk.real(table):
            ...
            OWN.note_write(table, 'dryrun_r251 (dorus hoard)')
...
def _battery(db, lk):
    """Every coexisting contract R-251 could plausibly disturb, in one pass."""
    results = []
    r251 = {}
    results.append(('R-251 uber hoard generosity (H1-H8)', SUH.problems(db, lk, r251)))
    ...
    # R-251 is the wave that makes 30 previously-orphaned branches REACHABLE, and this
    # gate only ever sees a branch something points at - so R-251 is precisely the change
    ...
    r251['relic_branches'] = grt_branches
...
def main(argv):
    if len(argv) < 2:
        print("usage: py tools/debug/dryrun_r251_uber_hoards.py <arz> [--before]")
        return 2
    ...
    if not before:
        print("\n=== R-251 dry run: applying the wave in memory%s ==="
        ...
        print("\n=== R-251 dry run: CONTROL, arz measured as built ===")

    results, r251 = _battery(db, lk)
    print("\n=== gate battery %s R-251 ===" % ('BEFORE' if before else 'AFTER'))
    bad = 0
