...
# Drive the injected portal entities (real, DB-backed, clickable NPCs).
# portal_bloodcave_entrance.dbr is created in build_svc_database.py
# (create_blood_cave_portal) and injected into HiddenValley01 by
# build_section_surgery.py INJECT_SPECS at local (16,18,26). The old target
# silkroad_villager1.dbr does NOT exist in the DB or in HiddenValley01, so the
# dialog never fired. Verified present in the built .arz and in HiddenValley01's
# 0x05 entities (string idx 81, world pos (-118,-102,2200)).
...
# Portal 1: HiddenValley01 NPC -> Blood Cave entrance
...
def main():
    ...
    # This is the quest slot proven to load in Custom Quest mode; it drives ENTRY.
