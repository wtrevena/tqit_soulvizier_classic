r"""gate_uber_hoard_generosity.py - AUDIT: does an uber/boss chest pay like one? (R-250)
...
⚠ THIS GATE REDS ON EVERY PRE-R-250 ARTIFACT, AND THAT IS THE GATE WORKING. On the
shipped build92 arz it reports 21 H1 findings and 21 H2 orphans - that IS the bug Will
filed five times. Do not "fix" a red by relaxing H1; measure a POST-R-250 arz.
...
def main(argv):
    ...
    if '--calibrate' in argv[2:]:
        SUH.calibrate(db, lk)
        return 0
    print("\n=== uber/boss chest generosity audit (R-250) ===")
    report = {}
    ...
    if problems:
        ...
        for p in problems:
            print("  FAIL  %s" % p)
        print("\nGATE FAIL: an uber/boss chest does not pay like one. On a PRE-R-250 "
