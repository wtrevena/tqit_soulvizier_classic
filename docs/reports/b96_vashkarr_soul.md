# b96 - Vashkarr, Eldest of the Ancients: spear-and-shield soul retune (R-70)
...
Appended to `docs/WILL_RULINGS.md` as **R-70**, status IMPLEMENTED.
...
```python
RUNSPEED_DOWNSIDE = ('gorrahk',)   # R-70: NOT vashkarr (see the amendment above)
...
...
R-70 documents the supersession by function name rather than inventing a number; the gap is
...
|---|---|---|
| 1 | the shipped R-70 shape, untouched | contract **silent** |
| **2** | **`characterRunSpeedModifier = -8.0` on all 3 tiers - i.e. EXACTLY today's shipped pre-R-70 state** | contract **FIRES** |
...
pre-R-70 arz reports rather than crashes.
...
=== contracts_souls NEGATIVE TESTS ===
  ...
  [PASS] SOUL-IDENTITY-SHAPE silent on the shipped R-70 shape
  [PASS] SOUL-IDENTITY-SHAPE fires on the pre-R-70 -8% speed PENALTY
...
physical-immune); -8% run speed now GORRAHK-ONLY (R-70: Vashkarr carries a run-speed BONUS + a
...
4. **BL-b96-DEBT-4 (P2, ledger gap found in passing):** `_apply_b7_eldest_soul_rebalance` implements
   an **unledgered** Will decision (quote lives only in its docstring, no R-number), so R-70 had to
...
- After the merged rebuild, the lane is re-verifiable in one command: contracts
  ...
  now the permanent guard that no later pass clobbers R-70 back out.
