...
particle for `343_dark_smoke` (`SVEffects/ambient/dark_smoke.pfx`) - the WILL_DECISIONS-sanctioned
"proven dark smoke" already worn by the confirmed-dark Enslaver ENCOUNTER rig
(`svc_enslaver_darksmoke_charfxpak`).
...
both hands (the same dark-smoke FX the Will-confirmed-black Enslaver encounter uses) as the "black
...
`bloodtoxeus_envenomweapon` (crimson) had exactly **5** references, all Devourer-family:

| record                                   | field(s)                    | before                      | after            |
|------------------------------------------|-----------------------------|-----------------------------|------------------|
| um_bloodtoxeus_99 (fought Devourer)      | initialSkillName, skillName3| bloodtoxeus_envenomweapon   | **svc_black_poison** |
| bloodtoxeus_1 / _2 / _3 (soul pets)      | skillName3                  | bloodtoxeus_envenomweapon   | **svc_black_poison** |
| toxeus_eoat_1 / _2 / _3 (EoAT pets)      | buffSelfSkillName           | bloodtoxeus_envenomweapon (crimson, round-1 flag) | **svc_black_poison** |
...
`0X_actY_arcaneformulae.dbr` at **weight 2** (i.e. supra formulas are the ~2% rare tail of the
formula drop) - the rarest formula tier:

| parent LootMasterTable (Epic `02_`, Legendary `03_`, all acts) | -> child (weight) |
|----------------------------------------------------------------|-------------------|
| `0X_actY_arcaneformulae.dbr` (8 tables)                        | `..._table` (98) + **`supra.dbr` (2)** |
| `03_act4_arcaneformulae_sp.dbr`                                | **`supra_special.dbr` (2)** |
...
supra weapon formula drops**, at that same rarest (~2%) formula tier - and inside the pool it is one
of 25 equal-weight entries. Runs after `uber_orphan_weapons` so the two modules' appends do not
collide.
...
`supra_special.dbr` formula pools at the rarest ~2% tier) AND is a **guaranteed 100% on-kill drop
...
- **Full scratch build on the merged branch** (first build of the merged champions+EoAT state):
  `local/bp_changed.arz` md5 **`32e0f2f709de1ba6954c4a6362ecbf0c`**. All in-build invariants + the
  29-module registry verifies GREEN, incl. `black_poison.verify OK` and the extended
  `toxeus_endofallthings.verify OK`; Occult/Hunting golden freeze guard `RESULT: PASS` (84 waived,
  0 other).
- **Clean pre-change merged-state build** (content OFF, ridx infra fix ON): `local/bp_clean.arz` md5
  **`532003ecc70dfbcc3dadf9796cba1d81`**, golden guard PASS.
- **Record-diff** (`local/diff_bp.py` clean -> changed): **exactly 17 delta records, zero collateral**
  = 4 ADDED (svc_black_poison, svc_black_poison_charfxpak, svc_rite_guaranteed,
  svc_devourer_misc4_master) + 13 CHANGED (um_bloodtoxeus_99 initialSkillName/skillName3/Misc4;
  bloodtoxeus_1/2/3 skillName3; toxeus_eoat_1/2/3 + eoat_disciple_1/2/3 buffSelfSkillName crimson->
  svc_black_poison; um_toxeus_enslaver_99 Misc4; supra.dbr + supra_special.dbr Rite append). Nothing
  else. (The 6 EoAT-family pet buffSelfSkillName flips follow from the module's `_BLACK_POISON` const
  now naming svc_black_poison - intended and coherent.)
- **Contracts** (`tools/contracts/run_contracts.py --only souls,summons,resources`): run on BOTH
  clean and changed arz - **identical totals** (19196 violations, 100 P0 / 1001 P1, all pre-existing
  ported-monster mesh/skill false positives from not passing `--resource-arc-dir`). **ZERO new
  violations from this change**; none of the 4 new records is flagged. Map/Quests/Levels are
  **byte-untouched** (this change is DB-only - no map-tooling edit), so the map contract is
  unaffected.
- **Negative test** (`local/negtest_bp.py`): `black_poison.verify` PASSES on the built arz, and
  planting a green tint (skillWeaponTintGreen=1.0) on svc_black_poison makes it correctly FAIL
  ("GREEN tint survived - the exact bug"). NEGTEST PASS.
- **Idempotent / deterministic**: a second full build of the changed state reproduces
  `local/bp_changed.arz` md5 byte-for-byte (`32e0f2f7...`); the modules use collision guards +
  an idempotent pool append (skip-if-listed) and no randomness.
