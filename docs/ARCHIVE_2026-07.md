# ARCHIVE - 2026-07 (historical; DO NOT trust for current state)

> **Trust level: ARCHIVE.** This file preserves point-in-time history that was moved out of the
> live docs during the 2026-07-08 documentation consolidation. Nothing here is guaranteed to
> reflect the current deploy state. For that, read `docs/HANDOFF_LIVE_STATE.md` and `docs/BACKLOG.md`.
>
> What is preserved below, in full, for zero information loss (em dashes were normalized to hyphens
> to comply with the standing no-em-dash rule; the text is otherwise unchanged):
> - The prior `HANDOFF_LIVE_STATE.md` (written 2026-07-07 night, last committed 2026-07-08) - the
>   running fix-workflow IDs + transcript paths, the full task queue (#28/#30/#31/#32/#35/#36/#37),
>   the entity-contract-suite spec (§4b), the build/deploy cheat sheet, Will's character/save facts,
>   and the TESTHUB revert recipe. The current items live on in `BACKLOG.md`; the workflow IDs and
>   the detailed contract-suite spec live only here.
>
> Older narrative history (the 17-attempt wall campaign, the navmesh RE, the souls/boss waves) lives
> in the per-topic RCA docs under `docs/` and in the user's memory board
> (`tq-soulvizier-2026-07-resume.md`). See `docs/README.md` for the doc map + trust levels.

---

## PRIOR docs/HANDOFF_LIVE_STATE.md, preserved in full (2026-07-07 night, committed 2026-07-08; em dashes normalized to hyphens per the no-em-dash rule, otherwise unchanged)
