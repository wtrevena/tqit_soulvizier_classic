...
> - **DEPLOY SIDE - still yours, and this is the half that can still bite.** The DEV deploy is a TARGETED copy. **`Resources\Creatures.arc` must be ON that file list beside the arz, and hash-verified.** Deploying the arz without it leaves six records pointing at an asset that resolves nowhere - an **INVISIBLE boss**, strictly worse than the missing smoke. `Creatures.arc` has been md5-`8c0d8d53` and "byte-unchanged" in every gate record since build93; **that line stops being true with this lane** (staged size 42,617,179 -> 42,764,976 B).
>
