...
>   `Finger2` soul and the orb both did. **R-13's rant scroll and R-92's EoAT formula are probably dead on all
>   three champions** - `BL-R258-DEBT-2`, owed before the next EoAT claim.
...
> - 🛑 **THE COLD BUILD COULD NOT BE RUN, AND THAT IS THIS LANE'S OPEN P0.** `upstream/`, `reference_mods/` and
>   `third_party/` are **GONE from this checkout** (emptied/removed 2026-09-09 ~13:18, not by this lane), and
>   `work\SoulvizierClassic\{Resources,Maps}` were stripped at the same time.
>   `py tools/check_build_inputs.py --all --verify-hashes` FATALs on **every** SV input, so
