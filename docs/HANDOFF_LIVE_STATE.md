...
> - ⚠️ **NOT PROVEN IN-GAME (`BL-R258-DEBT-1`, P1, WILL):** kill the Devourer on DEV or Steam (full quit TQ + restart Steam first): ONE soul = the `Finger2` channel was dead and the fix carried it; TWO souls = it never was; either way the next lane tightens it. Also open: `BL-R258-DEBT-2` (the `Misc4` audit, all 32 carriers, the next P1 lane), `BL-R259-DEBT-1` (the legacy ~300 GB `backups\` tree, Will's call; the background copy to `Z:\Computer Backup\tqit_soulvizier_classic\archive_pre_R259\` was still running at ship time, neither tree touched), `BL-b98-DEBT-5` (no preview image, warned again), `BL-R250-DEBT-1` (the shroud render, sixth carry).
>
...
> touch `backups\` or that folder until it completes (`BL-R259-DEBT-1` stays Will's call).
