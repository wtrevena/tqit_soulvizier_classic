...
> **AND IT IS NOW FORCED: `scripts/package_workshop.ps1` REFUSES to stage a byte unless step 0 ran green for this ship** (`BL-b98-DEBT-3`, closed 2026-08-15 after the **FOURTH** consecutive stale dispatch, `toxeus-boss-equipment-and-soul`, already live as build96). A PASS writes `local/step0_receipt.json` (gitignored, unforgeable through a commit); packaging runs `--verify-receipt` first and checks **R1** verdict PASS, **R2** an intended tag was named, **R3** the receipt's HEAD is an **ancestor** of the current HEAD (forward-only: step 0 runs before the merge, so HEAD may advance but never diverge), **R4** the tag is **still** free re-measured now, **R5** clean tree + `main` not behind origin, **R6** any lane it cleared is **contained in HEAD now** (the inverse of S1, so one lane's receipt cannot authorise another's package). A failing step 0 deletes the stale receipt. Note the fourth dispatch's shape: **its tag was FREE, so S2 was green and only S1 caught it** - do not assume a free build number means a fresh ship. Escape for a note-only re-upload: `--repackage "<reason>"` (waives R4 only, demands a reason, prints it loudly). **R7** it has not already been SPENT: a green `--verify-receipt` **CONSUMES** the receipt, so one step 0 authorises exactly **ONE** package (`BL-b98-DEBT-4`, **CLOSED 2026-08-15**, commit `e4a5ea7`, after the **FIFTH** consecutive stale dispatch `boss-arena-spawn-and-polish`, already live as build97). That was the last hole: a receipt naming **no lane** used to be invalidated only by its tag being taken, so between a legitimate package and the tagging step it could authorise a second package of different bytes inside one build-number window. Closing it at the tagging step was the original proposal and was rejected on measurement - there is **no tagging script** to hang it on, so it would have been another habit. An honest re-package is not blocked; it re-runs step 0, which costs seconds. `--negtest` **24/24 + 3 live replays + the P0 clean-tree control**; the packaging refusal is proven live twice (`PKG_EXIT=1` both times, with the `dist/` tree unchanged across the refusal - **56** files before and after when the R-252 refusal lane proved the R1 wiring, **112** before and after when this lane proved the new R7 arm on a receipt that had just been legitimately spent).
> **FIVE consecutive ship dispatches (2026-08-15) were spent on work that was already live** (`enslaver-shroud-ship-r250`, `dev-parity-r249-testhub-quests`, `chest-generosity-shared-cause`, `toxeus-boss-equipment-and-soul`, `boss-arena-spawn-and-polish`), and one of them additionally proposed a deploy that would have **REVERTED Will's play surface off build97**. The last two shared a shape worth knowing: **the tag they asked for was FREE, so only S1 lane containment caught them** - a free build number is not evidence of a fresh ship. The fifth went further and quoted, as its own "impl" sha, a commit that lives only on `main`: the docs commit written by the fourth refusal. A stale orchestrator board is the NORMAL case for this repo, not the exception. The gate answers it mechanically in seconds: **S1** the lane tip is not already an ancestor of `main` (it names the merge commit and the tag that shipped it), **S2** the intended tag is free locally and on the remote plus the next free build number, **S3** every artifact hash the dispatch QUOTES still matches the live bytes, **S4** clean tree and `main` not behind origin. `--negtest` 10/10 + 3 controls, including replays of the two real stale dispatches. A PASS only says the dispatch is not already done; it is not permission to ship. Residual: **none** - `BL-b98-DEBT-1/2/3/4` are all closed, the check is mechanical, forced at packaging, and single-use.
> ## ✅ INTEGRATED 2026-09-11 (was IN-FLIGHT): R-261 `fix/chest-generosity` (from `24d3383`/build103-ship) - THE CHEST-GENEROSITY SV-PARITY GATE. ZERO record writes. **INDEPENDENT VET GO -> MERGED to `main` as `7801ded` (`--no-ff`, branch tip `e82b872`) + an integration docs commit. NO build104, NO DEV deploy, NO packaging, NO Steam push, NO tag** - the vet's WORK-LAYOUT cold build under `SVC_REQUIRE_GATES=1` (A9 PASS, F2 0 P0 / 0 P1 / 111 P2) wrote an arz **BYTE-IDENTICAL to build103 `eca50cdf`, record-diff 0/0/0**, and a byte-identical arz is not re-shipped. Next free build number stays **104, UNCONSUMED**; DEV and Steam stay at build103, which already holds every byte R-261 asserts.
> **THE STATE THIS LEAVES FOR WILL AND FOR THE NEXT LANE:** the five chest reports (`BL-W0814-2/-5/-7/-11/-13`) close on Will's in-game **COUNT** (`BL-R261-DEBT-2`, carries `BL-R251-DEBT-3`), taken on **build103 itself** - the Propontis chest, the Tantalus hoard, Ephialtes's Dread-Hoard, an Obsidian Hoard, the Devourer's stash and one Secret Present, after a full quit of TQ + a Steam restart. 🔴 **If he still sees ~2 items, the next lane is an EMPIRICAL ENGINE-EVALUATION PROBE, not another table edit**: the engine's evaluation of `numSpawn*Equation` is UNPROVEN (`BL-R240-DEBT-5`) and every R-261 number is a table-derived expectation, so raising multipliers again would be guesswork against an unmeasured evaluator. **Leinth's chest** (`bosschest_leinth_01/02/03`) sits at B6`*0.2283/*0.2609` (S 1.125) vs SV's B6`*2.2/*2.4` (S 10.58) = **1/9.4 of its SV volume**, SURFACED by the gate on every build; restoration is **Will's call** (`BL-R261-DEBT-7`, carries `BL-R261-DEBT-1`; his 08-12 R-242 freeze and his 08-13 "all the chests" point opposite ways for that one record). The **Propontis chest is still labelled "Obsidian Hoard"** = `BL-R251-DEBT-2` / `BL-R261-DEBT-4`, Will's naming call and an arz+Text coupled ship. Debts moved at integration: `BL-R261-DEBT-3` **DISCHARGED** (the work-layout build above), `BL-R261-DEBT-5` **DISCHARGED** (two real pre-fix artifacts on disk RED under `--control`: `leinth_vet.arz` `0d861748` -> 27 problems, P1 x15 b42 repoint + P2 x12 gift; `SoulvizierClassic_b37fg.arz` `4f69e9ab` -> 15, P1 x3 Dorus->Obsidian share + P2 x12), `BL-R261-DEBT-6` **REGISTERED** (P3 tooling: `load_sv()` trusts an explicitly supplied upstream path unhashed - `resolve()` hashes the fallback ladder only).
> Re-dispatch of Will's five chest reports (`BL-W0814-2/-5/-7/-11/-13`). Measured by field-level diff of build103 `eca50cdf` against the upstream SV 0.98i arz: **all six surfaces are already at or above SV** (the stash `*3.8/*4.1` string-identical + superset composition per R-247.7a; the 36-chest hoard family on its own tables at `*2.4/*2.8` with the guaranteed row per R-251; the Secret Present box exactly 3.0x with composition byte-equal to SV). The shared cause (the b42 `_svc_standardize_boss_chests` repoint `db2d409` + the R-240 sweep) was fixed at the source by R-247.7a + R-251 four weeks ago. What the lane adds: `tools/gate_chest_generosity_sv_parity.py` (P1 wire / P2 count / P3 chances / P4 unique share + member superset / P5 form / P6 upstream pin; floor READ from the sv098i arz on every build), wired into `build_svc_database.py` after the R-260 gates; build103 PASS, `--negtest` 12/12 on the real arz. Surfaced for Will, not acted on: LEINTH'S CHEST (`BL-R261-DEBT-1`, chest under Will 08-13 vs orb under R-242 08-12). Closing proof = Will's in-game COUNT (`BL-R261-DEBT-2`). **Scratch cold build with the gate wired: exit 0, arz `eca50cdf` / 55,631,702 B / 51,355 records = byte-identical to build103, the gate PASS in the post-write battery** (A9 / F2 did not run in THAT build - scratch layout). **The artifact of record is the vet's WORK-LAYOUT cold build under `SVC_REQUIRE_GATES=1`: A9 PASS, F2 0 P0 / 0 P1 / 111 P2, same arz `eca50cdf`, record-diff 0/0/0** - `BL-R261-DEBT-3` DISCHARGED. Ledger R-261; BACKLOG block at the tail (with the integration decision); MISTAKES 2026-09-11.
>
> **Current head as of 2026-09-11 (post-R-261 integration): `main` = `7801ded` (R-261 `fix/chest-generosity` @ `e82b872` merged `--no-ff`) -> the R-261 integration docs commit. There is NO build104 and the deployed artifacts did not move: `build103-ship` is still the tip tag, next free build number = 104, UNCONSUMED, because the vet's work-layout cold build of the merged tree wrote an arz byte-identical to build103 `eca50cdf` (record-diff 0/0/0). R-261 wrote ZERO records - it adds a build gate. Nothing is staged, nothing is packaged, no step-0 receipt is live. Prior line, kept for history: `main` = build103-ship; next free build number = 104, UNCONSUMED. DEV and Steam both at build103 (arz `eca50cdf`, 55,631,702 B, 51,355 records; Text `1be898a0`, Creatures `d65d92a3`, canonical map `08f3639e`, Quests `6271ceb2` unchanged; DEV carries TESTHUB map `821ceaa9` + TESTHUB Quests `d9f8c316` unchanged). Steam item 3759792705 ManifestID `6416846304768403107`, VDF `"visibility" "0"`. No lane is mid-flight, no artifact is half-deployed, and no step-0 receipt is live (build103's was CONSUMED by its own packaging at 08:08:15Z). Step 0 ran BRANCHLESS (`--tag build103-ship` + six `--claim`s) because R-260 was already merged on `main` (`b2294ac`) and `--lane` would trip S1 by design; S2 / S3 / S4 green. DEV rollback = `local\DEV_arz_deployed_prev.arz` (build102 `7dad9a8a`). OPEN: `BL-R260-DEBT-1` (the Devourer's rant+Rite relocation, Will's call), `BL-R260-DEBT-2` (the displaced potions), `BL-R260-DEBT-3` + `BL-R258-DEBT-1` (Will's kills), `BL-R259-DEBT-2` (still no canonical CustomMaps target here), `BL-b98-DEBT-5` (no preview image), and from R-261: `BL-R261-DEBT-2` (Will's chest COUNT on build103 - the closing proof for the five chest reports), `BL-R261-DEBT-7` + `BL-R261-DEBT-1` (Leinth's chest at 1/9.4 of SV, Will's ruling), `BL-R261-DEBT-4` (the Propontis chest's name, Will's call), `BL-R261-DEBT-6` (P3 tooling: hash an explicitly supplied upstream arz).** Prior line, kept for history:
> **Current head as of 2026-09-10 (post-ship): `main` = build102-ship; next free build number = 103, UNCONSUMED. DEV and Steam both at build102 (arz `7dad9a8a`, 55,631,843 B, 51,355 records; Text `1be898a0`, Creatures `d65d92a3`, canonical map `08f3639e`, Quests `6271ceb2` unchanged; DEV carries TESTHUB map `821ceaa9` + TESTHUB Quests `d9f8c316` unchanged). Steam item 3759792705 ManifestID `7343053903808963545`, VDF `"visibility" "0"`. No lane is mid-flight, no artifact is half-deployed, and no step-0 receipt is live (build102's was CONSUMED by its own packaging at 03:36:03Z). Step 0 ran BRANCHLESS (`--tag build102-ship` + six `--claim`s) because both lanes were already merged on `main` (`f8a48f2`, `0f6d450`) and `--lane` would trip S1 by design; S2 / S3 / S4 green. OPEN: `BL-R259-DEBT-2` (the first NAS backup run did NOT happen on this ship, see the build102 block) and `BL-R258-DEBT-1` (Will's kill).** Prior line, kept for history:
> **Current head as of 2026-09-11: `main` = `b2294ac` (R-260 `fix/misc4-audit` @ `efa975b` merged `--no-ff`) -> the R-260 integration docs commit; DEV and Steam both at build102 (arz `7dad9a8a`, tag `build102-ship`); next free build number = 103, UNCONSUMED; the build103 candidate is the R-260 arz `eca50cdf` (55,631,702 B, 51,355 records; the vet's det-2x work-layout cold build, reproducible by the command in `docs/WILL_RULINGS.md` R-260 section 8) - NOT staged at `work\SoulvizierClassic\Database\` (the vet restored build102 `7dad9a8a` there, md5-verified at integration), not deployed, not packaged, no step-0 receipt live; `docs/WORKSHOP_CHANGENOTE.bbcode` rewritten for build103 and committed BEFORE packaging (`BL-b97-DEBT-1`, VDF-safe: 0 double quotes, 0 backslashes). `backups\` now holds only `game_dll` (the MOVE OUTCOME in the build102 block).** Prior line, kept for history: *Current head as of 2026-09-10: `main` = the BUILD102 INTEGRATION - `f8a48f2` (R-259 `fix/backups-to-nas` merged `--no-ff`) -> `0f6d450` (R-258 `fix/devourer-soul-drop` merged `--no-ff`) -> the integration docs commit; next free build number = 102, UNCONSUMED; DEV and Steam still at build101 (`9712f58f`); the build102 candidate is the COLD arz `7dad9a8a` (55,631,843 B, 51,355 records) staged at `work\SoulvizierClassic\Database\` - not deployed, not packaged, no step-0 receipt live; `docs/WORKSHOP_CHANGENOTE.bbcode` already rewritten for build102 and committed BEFORE packaging (`BL-b97-DEBT-1`, VDF-safe: 0 double quotes, 0 backslashes).* Prior line, kept for history: *Current head as of 2026-08-16 17:50: `main` = build101-ship; next free build number = 102, UNCONSUMED.* DEV and Steam both at **build101** (arz `9712f58f`, **`Creatures.arc d65d92a3`**, Text `1be898a0`, canonical map `08f3639e`, Quests `6271ceb2` unchanged; DEV carries TESTHUB map `821ceaa9` + TESTHUB Quests `d9f8c316` unchanged). No lane is mid-flight, no artifact is half-deployed, and **no step-0 receipt is live** (build101's was CONSUMED by its own packaging at 00:35:27Z). build101 is the **FIRST ART-ASSET SHIP** this repo has made: the coupling list is now Levels+Quests, arz+Text, **and arz+`Creatures.arc`**, and `Creatures.arc` has stopped being the "byte-unchanged `8c0d8d53`" line it had been in every gate record since build93. The stale-dispatch streak stays broken: `fix/enslaver-shroud-render` was a genuinely fresh lane, step 0 passed S1/S2 first try, and the ship ran end to end. Prior line, kept for history: *main = build100-ship; next free build number = 101, UNCONSUMED. DEV and Steam both at build100 (arz `6b89bb5d`, Text `1be898a0`, canonical map `08f3639e`, Quests `6271ceb2` unchanged; DEV carries TESTHUB map `821ceaa9` + TESTHUB Quests `d9f8c316` unchanged); no step-0 receipt live (CONSUMED at 09:54:41Z).* build100 was the first **MAP-REBUILDING** ship since build97: exactly ONE level blob changed in both map variants (`Rhakotis05`, +1,038 B), every navmesh byte-identical, the QUESTS section byte-identical, and both Quests variants rebuilt and proven byte-identical to build99 to honour the Levels+Quests coupling. `BL-R256-DEBT-5` ("the module has never run inside a real cold build") is **CLOSED** - the cold build exited 0 and the lane's clone-shape arm fired inside it. Prior line, kept for history: *main = build99-ship; next free build number = 100, UNCONSUMED. DEV and Steam both at build99 (arz `1113f2c6`);* no lane is mid-flight, no artifact is half-deployed, and **no step-0 receipt is live** (build99's was CONSUMED by its own packaging, the R7 arm's second real ship). The stale-dispatch streak stays broken: `fix/enslaver-pet-shroud` was a genuinely fresh lane, step 0 passed S1/S2/S4 first try, and the ship ran end to end. Prior line, kept for history: *main = build98-ship; next free build number = 99, UNCONSUMED. DEV and Steam both at build98;* no lane is mid-flight, no artifact is half-deployed, and **no step-0 receipt is live** (build98's was CONSUMED by its own packaging, which is the R7 arm's first real ship). **The six-dispatch stale streak is BROKEN**: `fix/death-penalty-halve-again` was a genuinely fresh lane, step 0 passed S1/S2/S4 on the first try, and the ship ran end to end. Prior line, kept for history: *main = build97-ship (`318cc2f`) + the step-0 gate, its packaging precondition and its single-use receipt; next free build number = 98, still UNCONSUMED. DEV and Steam both at build97.* **R-253 (build97) re-verified on the LIVE build97 DEV bytes 2026-08-15 04:15**: `gate_arena_spawn_guarantee` run with `--arz`/`--quests`/`--map` pointed at the DEV files exits **0** ("every destination encounter spawns unconditionally, from a static placement, with no one-shot quest source"), its `--negtest` **16/16 + 5 controls**, DEV byte-exact build97 on all four artifacts and the packaged Steam payload byte-exact to the canonical set with the VDF still PUBLIC. No build, deploy, Steam push or tag was run by that lane either. **R-252 (build96) re-verified on the LIVE build97 DEV bytes 2026-08-15 03:55**: `gate_toxeus_boss_equipment` exit 0 (4 pinned `Finger2` carriers all deliver a ring, no champion can hold a bow or staff on any difficulty) and `verify_soul_drop_rates --gate` PASS, with the anti-inert control exiting 1 with 48 problems on the pre-fix `b888f022`. No build, deploy, Steam push or tag was run by that lane.

> ## SHIPPED AS BUILD103 (2026-09-11; was IN-FLIGHT 2026-09-10): R-260 THE `Misc4` AUDIT on `fix/misc4-audit` (`BL-R258-DEBT-2`) - BUILT TO A SCRATCH LAYOUT + GATED; [INTEGRATED 2026-09-11: INDEPENDENT VET PASSED, its det-2x work-layout cold build = the artifact of record (arz `eca50cdf`, 55,631,702 B, A9 / M2 / F2 PASS, contracts 0 P0 / 0 P1, record-diff vs build102 ADDED 0 / REMOVED 0 / CHANGED 35), MERGED TO MAIN `b2294ac` `--no-ff`]; DEPLOYED TO DEV + STEAM 2026-09-11 as build103, tag `build103-ship` (the BUILD103 block below)
> The 32 build102 `Misc4` carriers (7 "guaranteed" @100 + 25 animal-relic sources at 7-10%) rode a slot `characterloot.tpl` does not declare and never paid. 31 are re-homed onto declared Misc slots at a MEASURED rate (`chance x weight / sum(weights)`, re-derived from the bytes by two gates and an independent script); the Devourer's rant+rite channel is WITHHELD for Will (`BL-R260-DEBT-1`: no empty slot, `Misc1` + `Finger2` frozen under `BL-R258-DEBT-1`, the master is a pick-one 50/50 - `BL-R258-DEBT-5` settled). Five bosses lose one potion per kill to the takeovers (`BL-R260-DEBT-2`). Helper = `tools/svc_loot_slots.py` (one implementation; tiers EMPTY > DORMANT > pinned 100%-takeover > opt-in rate-preserving SHARE > FAIL LOUD), registry module `phantom_loot_slots` (mod-wide sweep + verify), artifact gates `gate_no_phantom_loot_slots` (RED on build102: 35 carriers) + `gate_guaranteed_drops_measured` (RED on build102: 31/31 at 0.0000%). Scratch build exit 0: arz `eca50cdf`, 55,631,702 B, 51,355 records; record-diff vs build102 ADDED 0 / REMOVED 0 / CHANGED 35, zero unattributed; both R-260 gates PASS on it and RED on build102; the Devourer's `Finger2` + `Misc1` byte-identical to build102 (the `BL-R258-DEBT-1` freeze held). The ship lane owes the work-layout cold build with the A9/F2 asset gates (`BL-R260-DEBT-5`), det-2x, the record-diff on it, the vet, the promote. [2026-09-11: the cold build (`BL-R260-DEBT-5` DISCHARGED), det-2x, the record-diff and the vet are DONE - only the promote remains; `work\SoulvizierClassic\Database\SoulvizierClassic.arz` is still build102 `7dad9a8a`, the vet restored it after measuring.] [SHIPPED 2026-09-11 as build103: `eca50cdf` restaged at `work\SoulvizierClassic\Database\`, DEV targeted copy 1 of 62 + Steam ManifestID `6416846304768403107`, tag `build103-ship`.] Ruling: `docs/WILL_RULINGS.md` R-260; lane block: `docs/BACKLOG.md` tail; errors: `docs/MISTAKES.md` 2026-09-10 (two entries).

> ## BUILD103 SHIPPED TO DEV **AND** STEAM (2026-09-11) - R-260: THE `Misc4` AUDIT - 31 OF 32 PHANTOM-SLOT DROPS RE-HOMED ONTO DECLARED SLOTS AT MEASURED RATES (the Devourer's rant+Rite channel withheld, `BL-R260-DEBT-1`). arz-ONLY. **STEAM = DEV = main** (modulo the two TESTHUB variants DEV always carries).
> **Steam/canonical/DEV arz `eca50cdf9d2afbf4c1c08c66a24386ba`** (55,631,702 B, **51,355 records**; **-141 B / +0 records** over build102 `7dad9a8a`; record-diff ADDED 0 / REMOVED 0 / CHANGED 35, zero unattributed). `Text.arc 1be898a0` / `Creatures.arc d65d92a3` / canonical `Levels.arc 08f3639e` / `Quests.arc 6271ceb2` md5-proven BYTE-UNCHANGED (no partner file moved; the arz+Text and arz+Creatures couplings hold by identity). Steam item 3759792705: push-gates PASS (dist == work on all 5, TESTHUB guard OK, contracts on the DIST payload 0 P0 / 0 P1 / 4516 P2 GATE PASS), `Committing update...Success.`, workshop log `Uploaded new content ( ManifestID 6416846304768403107 )` + `Upload finished for workshop item 3759792705 : OK`, `-Update -Visibility 0` read back `"visibility" "0"`. Tag `build103-ship`. Full gate record: `docs/BACKLOG.md` BUILD103 GATE RECORD.
> - **DEV METHOD (the runbook in the build101 block below and the build85 block, unchanged):** targeted copy of the ONE changed artifact, `work\SoulvizierClassic\Database\SoulvizierClassic.arz` -> `CustomMaps\SoulvizierClassicDEV\Database\SoulvizierClassicDEV.arz` (the engine-facing name), md5 source == dest `eca50cdf`; NOT `scripts/deploy_to_custommaps.ps1`. DEV md5-inventoried before and after: **1 of 62 files changed, 0 added / 0 removed, 61 byte-identical**; DEV `Levels.arc` stays TESTHUB `821ceaa9`, `Quests.arc` stays TESTHUB `d9f8c316`, `Text.arc` `1be898a0` and `Creatures.arc` `d65d92a3` were already equal to work. Reparse-point scan 0 before and after; TQ.exe / Steam not running before, between or after (nothing killed). **Rollback (one step, arz-only):** `local\DEV_arz_deployed_prev.arz` = build102 `7dad9a8a` (55,631,843 B) -> copy back over the DEV arz.
> - **Player-facing (the change note):** the Toxeus Rite formula @100 from the Enslaver + both Hunts, the Golden Bough @100 from Charon form 2, Lethe's Draught @100 from the Mnemophage, the Mask of Dread @100 from Ephialtes; 25 animal-relic sources really roll 7-10%; each takeover boss gives up one potion (`BL-R260-DEBT-2`). The Devourer's rant scroll + his own Rite stay parked (`BL-R260-DEBT-1`, Will's call); his soul channels are byte-identical to build102 (the `BL-R258-DEBT-1` freeze held). New build gates `gate_no_phantom_loot_slots` + `gate_guaranteed_drops_measured` refuse the class from shipping again. Nobody has seen one fall yet: `BL-R260-DEBT-3` (kill the Enslaver, then Ephialtes; restart Steam first).
> - `BL-R259-DEBT-2` STAYS OPEN, unchanged from build102: no canonical `CustomMaps\SoulvizierClassic` target on this machine, DEV is the targeted copy by the runbook, the R-259 snapshot + keep-5 path was not exercised.

> ## BUILD102 SHIPPED TO DEV **AND** STEAM (2026-09-10) - R-258: THE DEVOURER'S SOUL RIDES A SECOND GUARANTEED CHANNEL (`Misc1` row 3) + R-259: DEPLOY BACKUPS MOVE TO THE NAS (code only on this ship, see the open debt). arz-ONLY. **STEAM = DEV = main** (modulo the two TESTHUB variants DEV always carries).
> **Steam/canonical/DEV arz `7dad9a8ad377ff9a65d17c144a57dde3`** (55,631,843 B, **51,355 records**; **-21 B / +3 records** over build101 `9712f58f`). `Text.arc 1be898a0` / `Creatures.arc d65d92a3` / canonical `Levels.arc 08f3639e` / `Quests.arc 6271ceb2` md5-proven BYTE-UNCHANGED (no partner file moved; the arz+Text and arz+Creatures couplings hold by identity). Steam item 3759792705: push-gates PASS (dist == work on all 5, TESTHUB guard OK, contracts on the DIST payload 0 P0 / 0 P1 / 4516 P2 GATE PASS), `Committing update...Success.`, workshop log `Uploaded new content ( ManifestID 7343053903808963545 )` + `Upload finished for workshop item 3759792705 : OK`, `-Update -Visibility 0` read back `"visibility" "0"`. Tag `build102-ship`. Full gate record: `docs/BACKLOG.md` BUILD102 GATE RECORD.
> - **DEV METHOD (the runbook in the build101 block below and the build85 block, unchanged):** targeted copy of the ONE changed artifact, `work\SoulvizierClassic\Database\SoulvizierClassic.arz` -> `CustomMaps\SoulvizierClassicDEV\Database\SoulvizierClassicDEV.arz` (the engine-facing name), md5 source == dest `7dad9a8a`; NOT `scripts/deploy_to_custommaps.ps1`. DEV md5-inventoried before and after: **1 of 62 files changed, 0 added / 0 removed, 61 byte-identical**; DEV `Levels.arc` stays TESTHUB `821ceaa9`, `Quests.arc` stays TESTHUB `d9f8c316`, `Text.arc` `1be898a0` and `Creatures.arc` `d65d92a3` were already equal to work. Reparse-point scan 0 before and after; TQ.exe / Steam not running before, between or after (nothing killed). **Rollback (one step, arz-only):** `local\DEV_arz_deployed_prev.arz` = build101 `9712f58f` (55,631,864 B) -> copy back over the DEV arz.
> - 🔴 **`BL-R259-DEBT-2` STAYS OPEN - the first real NAS backup run did NOT happen on this ship, and the integration docs' claim that this deploy would discharge it was wrong for this machine** (`docs/MISTAKES.md` 2026-09-10). `scripts/deploy_to_custommaps.ps1` deploys to the hardcoded `CustomMaps\SoulvizierClassic`, which does not exist here (only `SoulvizierClassicDEV` does), and the DEV runbook forbids running it (a second mod, or the TESTHUB pair clobbered). So the R-259 deployed-snapshot + keep-5 rotation path is not exercisable on this machine as the script stands; the standing evidence remains the sandbox harness (69/69). Discharge needs either a real `CustomMaps\SoulvizierClassic` target or a lane that teaches the script the DEV target with the TESTHUB coupling respected; that is a lane of its own, not a ship operator's call.
> - ⚠️ **NOT PROVEN IN-GAME (`BL-R258-DEBT-1`, P1, WILL):** kill the Devourer on DEV or Steam (full quit TQ + restart Steam first): ONE soul = the `Finger2` channel was dead and the fix carried it; TWO souls = it never was; either way the next lane tightens it. Also open: `BL-R258-DEBT-2` (the `Misc4` audit - FIXED by R-260, vet PASSED, merged to main `b2294ac` 2026-09-11; awaiting the build103 promote), `BL-R259-DEBT-1` (the legacy ~300 GB `backups\` tree, Will's call; the background copy to `Z:\Computer Backup\tqit_soulvizier_classic\archive_pre_R259\` was still running at ship time, neither tree touched) [DISCHARGED - MOVE OUTCOME under Will's 2026-09-10 rule, recorded at the R-260 integration 2026-09-11: `backups\{deployed,dev_deploy,characters}` = 92,504 files / 303,439,495,345 B moved to `Z:\Computer Backup\tqit_soulvizier_classic\archive_pre_R259\` (verified before the local delete: list-only diff 0, file count + total bytes equal on both sides, 15 spot md5 equal), the local copies deleted only after those gates passed; C: +282.85 GiB; `backups\` now holds only `game_dll`], `BL-b98-DEBT-5` (no preview image, warned again), `BL-R250-DEBT-1` (the shroud render, sixth carry).
>
> ## 🟡 PRIOR IN-FLIGHT ENTRY, kept for history (SHIPPED AS build102 2026-09-10, see the block above): R-258 `fix/devourer-soul-drop` + R-259 `fix/backups-to-nas`, both INDEPENDENT-VET GO, integrated as the build102 candidate
> **`BL-W0814-10` (the Devourer's soul) has a fix on `main`.** Branch tip `b1ab068` (vet GO), merged `--no-ff` as
> `0f6d450`. Beside it, R-259 (`a31d8d6`, vet GO + F1-F5 hardening, sandbox harness 69/69) merged as `f8a48f2`;
> its `WIN_BACKUP_ROOT` + `BACKUP_KEEP` are already in the LIVE `local\config.env`, so **this build's DEV
> deploy is the first real NAS backup run - `BL-R259-DEBT-2` is discharged by it and the ship commit records [SUPERSEDED at ship time 2026-09-10: it was NOT - the DEV deploy is the targeted coupled copy and `deploy_to_custommaps.ps1` has no target on this machine; the debt STAYS OPEN, see the build102 block above]
> the result** (root created under `Z:\Computer Backup\tqit_soulvizier_classic\`, snapshot count + bytes
> verified, SMB copy time, keep-5 rotation outcome). A background copy of the legacy `backups\` tree to
> `Z:\Computer Backup\tqit_soulvizier_classic\archive_pre_R259\` was in progress at integration time: do not
> touch `backups\` or that folder until it completes (`BL-R259-DEBT-1` stays Will's call). [Completed and verified: the MOVE OUTCOME in the build102 block above; `BL-R259-DEBT-1` DISCHARGED.]
> Ruling: `docs/WILL_RULINGS.md` -> **R-258** (and **R-259**). Module:
> `tools/patches/devourer_soul_delivery.py` (registry entry after `toxeus_boss_equipment`); standalone gate
> `tools/gate_devourer_soul_delivery.py`.
> - **ROOT CAUSE (shipped `9712f58f`, byte-decoded):** the soul's ONLY path is the `Finger2` EQUIPMENT slot
>   (chance 100 / row weight 100 / `lootFinger2Item1` = `blood_toxeus_soul_{n,e,l}`, no difficulty, champion or
>   rank gate, all three resolving as `Jewelry_Ring`). Byte-correct, and a SINGLE POINT OF FAILURE - which is
>   why three audits cleared it while the boss still paid nothing.
> - 🔴 **THE PRE-DESIGNED `Misc4` ESCALATION IS REFUTED, and this changes three other rulings.** `Misc4` is not a
>   variable of `Templates\TemplateBase\CharacterLoot.tpl` (which declares exactly ELEVEN slots and no
>   `Misc4`/`Neck`); `chanceToEquipMisc4` occurs in **0** of the 74,013 base-AE records, **0** of SV 0.98i and
>   **0** of SV 0.9, only on the 32 records this mod wrote; and R-247.6a's "Will's kill DID drop it" is an
>   inference that contradicts Will's own report that the Misc4 formula did NOT drop on the kill where the
>   `Finger2` soul and the orb both did. **It is wider than three champions: all 32 `Misc4` carriers are dead -
>   7 guaranteed @100 (the Devourer's `svc_devourer_misc4_master`, the EoAT rite `svc_rite_guaranteed` on the
>   Enslaver + both Hunts, the Ferryman's `svc_goldenbough_{n,e,l}` (R-231), Mnemophage's `lethesdraught`
>   amulet, Ephialtes' `svc_maskofdread_{n,e,l}`) + 25 animal-relic sources at 7-10% (`svc_sepulchralscale` on
>   8 wyrms, `svc_erebanheartstone` on `em_brute_43/45`, `svc_revelersruse` on `ar_archer_01..06`,
>   `svc_sanguinetithe` on the 9 Sileni); root for the guaranteed seven = `apply_svc_patches._svc_guarantee_unique`'s
>   slot order `for n in (4, 5, 6, 3)`, which writes the non-existent Misc4/5/6 before the real Misc3** -
>   `BL-R258-DEBT-2`, WIDENED at integration and REGISTERED AS THE NEXT P1 LANE (the `Misc4` audit, all 32).
> - **THE FIX:** three new one-row `LootItemTable_FixedWeight` tables `svc_devourersoul_guaranteed_{n,e,l}` on
>   the Devourer's FREE `Misc1` row 3 @ weight 100, potion rows MUTED to 0 (values kept). Measured share
>   **100.0000%** on all three difficulties. `Finger2` asserted, never written.
> - **GATES:** `--selftest` PASS, `--negtest` **27/27**, standalone gate **RED on the shipped `9712f58f`** (4
>   problems = Will's report as an artifact fact), `--dryrun` **RED -> GREEN in memory on those same bytes**.
>   Record-diff vs `9712f58f`: **ADDED 3 / REMOVED 0 / CHANGED 1**, zero unattributed. **No new Text tags -> the
>   arz+Text coupling is satisfied trivially, Text stays `1be898a0`.**
> - ✅ **THE COLD BUILD RAN (2026-09-10) - `BL-R258-DEBT-6` and `BL-R257-DEBT-3` DISCHARGED.** On 2026-09-09 the
>   lane could not run it: `upstream/`, `reference_mods/` and `third_party/` had been pruned that morning (not by
>   this lane) and a filename-only search found no archive on any drive. That negative was wrong
>   (`docs/MISTAKES.md` 2026-09-10, the orchestrator's own): five of the six inputs were inside the NAS's
>   `TQ.7z` + `Games.7z`, SV 0.4.1 was re-downloaded, and all four archives are now copied off-repo to
>   `Z:\Computer Backup\tqit_soulvizier_classic\third_party_archives\` (README there). The REAL entrypoint then
>   ran cold TWICE by the builder (`SVC_NO_CACHE=1 PYTHONHASHSEED=0 SVC_RELEASE_DROPS=1 SVC_REQUIRE_GATES=1`)
>   plus a THIRD byte-identical run by the vet: **arz `7dad9a8ad377ff9a65d17c144a57dde3`, 55,631,843 B, 51,355
>   records** - THE ARTIFACT OF RECORD, staged at `work\SoulvizierClassic\Database\` as the build102 candidate;
>   `Text.arc` **`1be898a0`** and `Creatures.arc` **`d65d92a3`** unchanged; record-diff vs `9712f58f` **ADDED 3 /
>   CHANGED 1 (4 fields) / REMOVED 0, ZERO unattributed**. The lane's apply-over-shipped artifact
>   (`tools/debug/r258_apply_over_shipped.py`: arz `e819a9a3`, 55,632,541 B, 51,355 records, det-2x md5-equal
>   AND `cmp` byte-identical, a third re-apply **ADDED 0 / REMOVED 0 / CHANGED 0**, the same record-diff, 15
>   gate rows exit 0 on it incl. `verify_soul_drop_rates --gate`, `gate_toxeus_boss_equipment`,
>   `validate_soul_augments`, `validate_tags` and the nine loot gates) stays in the record as **EVIDENCE
>   ONLY** - it is not the build and does not ship. **SHIP LANE: deploy `7dad9a8a` (arz only - Text and
>   Creatures are unchanged), md5 source==dest, record-diff it against the DEV bytes, and record the first NAS
>   backup run (`BL-R259-DEBT-2`).**
...
> **Steam/canonical/DEV arz `9712f58fcc1a73ec1fba2d5a9e811cbc`** (55,631,864 B, **51,352 records**; **-2,873 B / -5 records** over build100 `6b89bb5d` - the b99 belief chain retired) **+ `Resources\Creatures.arc d65d92a36adcde0cef6e1bb58afebd28`** (42,764,976 B, **+147,797 B** over `8c0d8d53`, carrying the one net-new 348,913-byte rig). **`Text.arc 1be898a0` / canonical `Levels.arc 08f3639e` / `Quests.arc 6271ceb2` md5-proven BYTE-UNCHANGED, Text re-BUILT and byte-compared after the fact; 0 new tags (463/463 authoritative, unchanged).** DEV runs that pair with the **TESTHUB** twins `Levels.arc 821ceaa9` + `Quests.arc d9f8c316`, both **UNCHANGED**. Workshop item 3759792705, `Committing update...Success.`, workshop log `Upload finished for workshop item 3759792705 : OK`, **ManifestID `9021554728139346124`**, `-Update -Visibility 0` with the VDF read back `"visibility" "0"` (stays PUBLIC). `main` `87c95b2` (build100-ship) -> `25bb2ad` (`git merge --no-ff fix/enslaver-shroud-render`) -> `4f9f7b4` (change note, committed BEFORE packaging) -> `6e09ddc` (two more operator MISTAKES) -> this gate record. Tag **`build101-ship`**.
> - **What it is:** `BL-W0816-1`, Will verbatim, the **FIFTH** filing, this time with a screenshot (the Enslaver summoned at Hathor Basin on DEV, post-b99/b100, standing out of combat, ZERO smoke): *"toxeus the murderer enslaver of souls summoned pets (when i summon them from their souls) still do not have the black smoke around them"*. Design: `docs/WILL_RULINGS.md` -> **R-257**.
> - **THE RULING IS A STANDARD, NOT A PATCH - THE EXEMPLAR STANDARD.** A channel being DECLARED in the template does not mean the pet controller EXECUTES it or that a CharFxPak ATTACHES on this rig. **A record-level proof is not a rendering proof**, and three rounds of record-level verification failed this one feature. A fix for a rendering defect must now use a mechanism with a **SHIPPING RENDERING EXEMPLAR** and replicate it byte for byte; where none exists, say so and take the one that does. The census (566 templates / 74,013 records / 341 Pets / 591 meshes) found exactly ONE: **mesh-compiled FX**, worn by the marauders **Will confirmed by eye** with `initialSkillName = buffSelfSkillName = None`. The b99 shape occurs **ZERO times in shipping data**; `charFxPakRunningNames` is in zero templates and zero base records; `Pet.tpl`'s 1,923-field closure has **no ambient effect field at all**, which is why the base game compiles auras into meshes.
> - **RECORD-DIFF vs the shipped build100 `6b89bb5d`: ADDED 0 / REMOVED 5 / MODIFIED 6, ZERO unexplained - and it matched the shape this handoff PREDICTED IN WRITING BEFORE THE BUILD RAN, item for item.** REMOVED = `svc_enslaver_shroud` + `_charfxpak` + `_fx` + the two `svc_alwayson_*` controller clones. MODIFIED = the wild boss and the 3 soul-summon pet tiers (each: `initialSkillName` -> None, `buffSelfSkillName` -> None, `controller` back to the SHARED original, `mesh` -> the new rig) plus the 2 preview proxies (`mesh` only). Nothing else in 51,352 records moved.
> - **det-2x:** `9712f58f` across two independent COLD builds (`SVC_NO_CACHE=1`, `PYTHONHASHSEED=0`, `SVC_RELEASE_DROPS=1`, `SVC_REQUIRE_GATES=1`), **exit 0 both**, MD5-equal AND `fc /b` *"no differences encountered"* over all 55,631,864 B. `STRICT failures : 0` in both; zero `Traceback` in either log. run2 into `local/b101_run2/` with a junction to work's `Resources/`.
> - 🟢 **`BL-R257-DEBT-3` DISCHARGED ON THE FIRST TRY, AND IT IS THE HEADLINE OF THIS SHIP.** The debt said a lane that changes what a build REQUIRES must run the build's own entrypoint. **The ship lane's path (cold-order control run D) fired exactly as specified inside the real build:** `build_svc_database._preflight_mod_creatures_arc()` found the archive lacking the rig, printed *"the Enslaver family would resolve NO MESH"*, **RESTAGED it 42,617,179 -> 42,764,976 B seconds into the build**, and re-verified it byte-identical before a single record was touched. The vet's dead cold build did not recur.
> - **THE THREE IN-BUILD ASSET GATES ALL FIRED GREEN ON THE SHIPPED BYTES:** `enslaver_shroud` **M1+M2 [ANCHORED]** (rig derived from the two BASE binaries and byte-identical in the shipping archive, 348,913 B, A1..A8); **A9 render resolution** (`ShadowStalker_Smoke01.pfx` -> the base `Effects.arc`); and the household walk - **all 8 members smoke by the ONE proven mechanism, 4 on `ShadowStalker.msh` + 4 on `svc_enslaver_shroudrig01.msh`, with NO field channel surviving anywhere** and zero live references to the retired R-250 chain.
> - **ANTI-INERT, RUN BOTH DIRECTIONS ON ARTIFACTS THAT ACTUALLY SHIPPED** (`tools/debug/b101_anti_inert.py`, written this ship because the battery's first attempt used a flag the module does not accept and exited 0 having done nothing - logged in MISTAKES): **RED with 17 problems on the shipped `6b89bb5d`**, including *"FAMILY FX PARITY: ... does NOT smoke. `SkeletonGrayBlack01New.msh` embeds 'NOTHING'"* - Will's screenshot as an artifact fact - and **GREEN on `9712f58f`**, and **GREEN again on the LIVE DEV bytes**.
> - **Gates:** `build_shroud_rig --selftest` PASS / `--negtest` **10/10** / `--check-arc` **exit 0 on the staged archive AND exit 1 on the base game's own** (both directions run); `enslaver_shroud --selftest` PASS / **`--negtest` 37/37** (31 stubbed + **6 UNSTUBBED arms against real archives on disk**); `champion_mesh --negtest` **15/15** incl. the R-257 regression plant; `gate_dye_skins` PASS; `_check_registry` **69 modules**, order `d83e9744` unchanged; `validate_tags` PASS **463/463, 0 new**; `gate_ruling_ids --branches` PASS (**53 rulings, highest R-257**, 167 branches); plus the whole coexisting battery green - `verify_soul_drop_rates --gate`, `gate_toxeus_boss_equipment`, `gate_uber_hoard_generosity`, `gate_loot_volume`, `gate_loot_distribution`, `gate_chest_loot_breadth`, `gate_orb_loot_breadth`, `gate_relic_difficulty_tiers`, `gate_chest_artifacts`, `gate_orb_legendary`, `gate_supra_recipe_laws`, `gate_arena_spawn_guarantee`, `gate_atlantis_voyage_cap`, `gate_dlc_act_ui_cap`, `validate_soul_augments`. Every row was run with its FULL argv printed beside its exit code.
> - ⚠️ **THE TWO NON-ZERO EXITS, BOTH MY OWN ARGV, BOTH STATED RATHER THAN BURIED (and both in MISTAKES):** (1) `validate_render_chain <arz>` exits **2** without its `<resources_dir> <game_dir>` anchors, by its own `B-GATE-HARDEN-1` design - re-run correctly it is **exit 0, RESULT PASS, 269 pets / 3,089 art refs**. (2) `validate_summon_pets <arz>` exits **1** without base+upstream, promoting 134 known-upstream warnings to STRICT; **the build's own run is `STRICT failures : 0` over 293 chains / 268 pets.** The second mattered because R-257 narrowed `B-SUMMON-1`'s witness set for the pet tiers to one record, so it is the shape a real regression would take - **control: the identical wrong invocation on the shipped build100 arz returns the SAME 72 not-proven / 210 BROKEN with a ZERO set-diff both ways, and no enslaver row under either invocation.** The "RIG PAIRING UNVOUCHED" arm did not bite.
> - **Contracts SET-DIFFED vs the build100 set on `(contract, severity, subject)`: 4,516 keys both sides, `0` only-in-new, `0` only-in-base**, `0 P0 / 0 P1 / 4516 P2` GATE PASS. Re-run end to end on the **DIST payload**: identical.
> - **DEV (the `BL-R257-DEBT-1` deploy half, discharged):** targeted **COUPLED** copy of **arz + `Resources\Creatures.arc`**, md5 source==dest on both, **TQ.exe NOT running before, during or after** (nothing killed, Steam not restarted). DEV md5-inventoried before and after: **exactly 2 of 62 files changed**, 0 added / 0 removed, **60 byte-identical**; DEV `Levels.arc` stays TESTHUB `821ceaa9`, DEV `Quests.arc` stays `d9f8c316`, DEV `Text.arc` stays `1be898a0`. **Post-deploy proof on the DEPLOYED bytes, all three green:** `build_shroud_rig --check-arc "<DEV>\Resources\Creatures.arc"` **exit 0** ("byte-identical to the derived rig, 348913 bytes"); the R-257 gate GREEN on the live DEV arz; and `validate_render_chain` **PASS against the DEV Resources dir**, which is the direct answer to the invisible-boss risk.
> - **PUSH-GATE PASS:** `dist == work` on **all 5** artifacts (arz `9712f58f`, Text `1be898a0`, Levels `08f3639e`, Quests `6271ceb2`, **Creatures `d65d92a3`**); **TESTHUB guard PASS** (*"packaged MD5 08F3639E... differs from TESTHUB MD5 821CEAA9..."*, and that TESTHUB hash is byte-identical to what is live on DEV); contracts on the DIST payload 0 P0 / 0 P1 / 4516 P2; single `SoulvizierClassic` wrapper, **56 files / 1188.6 MB**. Step-0 receipt verified then **CONSUMED at 00:35:27Z**.
> - ✅ **`BL-b97-DEBT-1` honoured (operator half):** `docs/WORKSHOP_CHANGENOTE.bbcode` rewritten for THIS build and **committed BEFORE `package_workshop.ps1` ran** (`4f9f7b4`), machine-checked VDF-safe (0 double quotes, 0 backslashes) after build100's refusal, pure ASCII, no BOM. Tooling half still open; `BL-b98-DEBT-5` (no preview image) unchanged and warned again on this upload.
> - **Rollback (COUPLED - both or neither):** `local/DEV_arz_deployed_prev.arz` `6b89bb5d` + `local/DEV_creatures_deployed_prev.arc` `8c0d8d53` -> copy both back. Canonical baselines at `local/build100_shipped_6b89bb5d.arz` + `local/build100_shipped_creatures_8c0d8d53.arc`; this build's twin at `local/b101_run2/`; logs under `local/b101_logs/`.
> - ⚠️ **NOT PROVEN IN-GAME (`BL-R250-DEBT-1`, P2, CARRIED FORWARD FOR THE FIFTH TIME) - and this ship deliberately does NOT claim otherwise.** The verify banner is split in two on purpose: **WHAT IS CLAIMED** = these records reach the screen by the SAME mechanism, through the SAME attach point, with a BYTE-IDENTICAL block, as the marauders Will confirmed by eye; **WHAT IS NOT CLAIMED** = that anyone has SEEN this build render. Will's check, full quit TQ + restart Steam first: **summon the Enslaver from ANY tier of his soul and just stand there, out of combat -> PASS = black smoke from the moment he appears, on him and on the marauders he raises, and it looks like theirs.** FAIL 1 = still clean (then the fallback is `ShadowStalker.msh` outright, which costs the skeleton silhouette - **Will's call**, `BL-R257-DEBT-2`). FAIL 2 = INVISIBLE (the asset half; checked on the deployed bytes, should be unreachable). FAIL 3 = smoke too THICK (something emits twice). **`BL-W0816-1` stays OPEN until Will says it smokes** - the b99 lesson about a CLOSED backlog item over an unrendered feature is not repeated here. Test note: `docs/WILL_TEST_GUIDE.md` top entry.
> - 🔁 **Carried forward:** `BL-R257-DEBT-2` (P2, a mod-shipped copy of a base rig has never been loaded on THIS creature - class proven by 1,105 mod-served meshes, bytes proven by A1..A9, this exact file unproven); `BL-R255-DEBT-1` (P1, six records still park skills above the `skillName17` ceiling, `um_toxeus_enslaver_99`'s `unholy_rally` among them, printed by this build again - needs Will's call because his 1..17 are full); `BL-R255-DEBT-2`, `BL-R130-DEBT-3`, `BL-b98-DEBT-5`.
>
> ## 🟡 PRIOR IN-FLIGHT ENTRY, kept for history: `fix/enslaver-shroud-render` (R-257) before it shipped
> Implemented, gated STATICALLY, **not built, not deployed, not tagged**. **VET ROUND 2 (2026-08-16) FOUND THE COLD BUILD DEAD AND IT IS NOW FIXED.** Branch tip carries `tools/build_shroud_rig.py` (new), `tools/debug/r257_cold_order_control.py` (new), `tools/patches/enslaver_shroud.py` (rewritten), `tools/patches/champion_mesh.py`, `tools/mesh_assets.py`, `tools/build_svc_database.py`, `tools/build_creatures_dye_skins_arc.py`, `scripts/bootstrap_working_mod.ps1` and the R-257 doc set.
>
> 🚨 **READ THIS BEFORE SHIPPING IT: THE `Creatures.arc` COUPLING IS A BUILD-TIME PRECONDITION FIRST, A DEPLOY COUPLING SECOND.** R-257 gives `um_toxeus_enslaver_99`, the three `toxeus_enslaver_*` pet tiers and his two preview proxies a `mesh` that ships ONLY in the mod's own `work\<mod>\Resources\Creatures.arc` (`monster/skeleton/svc_enslaver_shroudrig01.msh`, authored by `build_shroud_rig` from the base rig + the 115-byte `CreateEntity` block copied verbatim off `ShadowStalker.msh`).
> - **BUILD SIDE - now automatic, and it has to be, because THE SHIP LANE DRIVES `build_svc_database.py` DIRECTLY AND HAS NEVER RUN THE BOOTSTRAP.** Round 1 staged the archive after the build; three fail-loud gates inside the build read it (`enslaver_shroud` M2, `validate_render_chain` A9, `champion_mesh.verify`), so the cold build ABORTED - and bootstrap's `$LASTEXITCODE` fallback then silently copied the RAW upstream SV 0.98i database over the mod database. Now `build_svc_database._preflight_mod_creatures_arc()` checks the archive seconds in and RESTAGES it through `build_creatures_dye_skins_arc` (still the single writer), or ABORTS naming the fix. Opt out with `SVC_AUTOSTAGE_CREATURES=0`; the gates then red exactly as before. Bootstrap Step 0e does the same before Step 1, and a fired gate now STOPS the bootstrap (`SVC_ALLOW_UPSTREAM_FALLBACK=1` to override).
> - **DEPLOY SIDE - still yours, and this is the half that can still bite.** MEASURED, both paths: `scripts/deploy_to_custommaps.ps1` copies the WHOLE `work\<mod>` tree recursively (line 138), so `Resources\Creatures.arc` rides along for free - **but the last five ships did NOT use it.** They used a TARGETED coupled copy (arz + Text + TESTHUB Levels + TESTHUB Quests), because DEV runs the TESTHUB map/quest twins that a full-tree copy would clobber. **So `Resources\Creatures.arc` must be ADDED to that targeted file list, beside the arz, and hash-verified.** Deploying the arz without it leaves six records pointing at an asset that resolves nowhere - an **INVISIBLE boss**, strictly worse than the missing smoke. One-line post-deploy proof on the DEPLOYED bytes: `py tools/build_shroud_rig.py --check-arc "<DEV>\Resources\Creatures.arc"` (exit 0 = the rig is there and byte-identical). `Creatures.arc` has been md5-`8c0d8d53` and "byte-unchanged" in every gate record since build93; **that line stops being true with this lane** (staged size 42,617,179 -> 42,764,976 B, so the DEV md5 inventory will show it CHANGED and that is expected).
>
> **Expected record-diff shape vs build100 `6b89bb5d`** (so the ship lane can hold it to zero unexplained): **MODIFIED 6** - the wild boss + 3 pet tiers lose `initialSkillName`/`buffSelfSkillName` and revert `controller` to the shared original, and those 4 plus the 2 preview proxies change `mesh`; **REMOVED 5** - `svc_enslaver_shroud`, `svc_enslaver_shroud_charfxpak`, `svc_enslaver_shroud_fx` and the two `svc_alwayson_*` controller clones are no longer authored at all. That REMOVED count is deliberate and is R-257's retirement, not a regression: those records are the b99 belief channel, and on a member that already smokes from its rig they would DOUBLE the shroud.
>
> **Static gates green on the branch (post vet round 2):** `build_shroud_rig --selftest` PASS / `--negtest` 10/10 / `--check-arc` PASS-and-exit-1 both directions; `enslaver_shroud --selftest` PASS / **`--negtest` 37/37** (31 stubbed + **6 UNSTUBBED arms running the real `rig_asset_state()` against real archives**); `champion_mesh --negtest` 15/15; `gate_dye_skins` PASS; `gate_ruling_ids --branches` PASS (53 rulings, highest **R-257**); patches-registry `selfcheck` OK (69 modules, order `d83e9744` unchanged); bootstrap parses clean; encoding sweep over all 14 touched files CLEAN; STATIC dry-run of both modules in registry order over the shipped build100 arz **GREEN** (8/8 household members MESH, 6 records touched).
>
> **COLD-ORDER CONTROL, five runs against the real build100 checkout** - `py tools/debug/r257_cold_order_control.py --root <checkout>`: **A** shipped state -> `apply()` **ABORT** (the vet's repro; the gate still bites); **B** staged-first -> PASS; **C** good anchor + stale archives on the glob path -> PASS (round 1 red-locked here); **D** the ship lane's own path, autostage default -> restaged 42,617,179 -> 42,764,976 B then PASS; **E** `SVC_AUTOSTAGE_CREATURES=0` -> LEFT ALONE, still FAIL. **ALL FIVE BEHAVED AS SPECIFIED.**
>
> **No claim is made that it renders** - that is `BL-R250-DEBT-1`, open for the fifth time, and only Will's eye closes it.

> ## BUILD99 SHIPPED TO DEV **AND** STEAM (2026-08-15) - R-255: the ENSLAVER'S **SUMMONED PETS** FINALLY CARRY THE BLACK SHROUD. b93 wrote it into skill slots **the engine does not read**. **arz-ONLY.** **STEAM = DEV = main** (modulo the two TESTHUB variants DEV always carries).
> **Steam/canonical/DEV arz `1113f2c69fc3f188b0b5bece340614f2`** (55,608,016 B, 51,331 records; **-20 B / +0 records** over build98 `15dacc68` - four dead `skillLevel` ints retired and two live string fields added, on four records). **arz-ONLY: `Text.arc 82d5b810` / canonical `Levels.arc 1bf86461` / `Quests.arc 6271ceb2` / `Creatures.arc 8c0d8d53` md5-proven BYTE-UNCHANGED, re-hashed after the build; 0 new tags.** DEV runs that arz with the **TESTHUB** twins `Quests.arc d9f8c316` + `Levels.arc 666789ab`. Workshop item 3759792705, `Committing update...Success.`, workshop log `Upload finished for workshop item 3759792705 : OK`, **ManifestID `7650143609962912100`**, `-Update -Visibility 0` with the VDF read back `"visibility" "0"` (stays PUBLIC). `main` `9e512f0` -> `64a1b3e` (`git merge --no-ff fix/enslaver-pet-shroud`) -> `f7904c1` (change note, committed BEFORE packaging) -> this gate record. Tag **`build99-ship`**.
> - **What it is:** `BL-W0815-1`, Will verbatim, the **FOURTH** filing: *"toxeus the murderer enslaver of souls summoned pets (when i summon them from their souls) still do not have the black smoke around them"*. Design: `docs/WILL_RULINGS.md` -> **R-255**.
> - **ROOT CAUSE, ours:** `MonsterSkillManager.tpl` (included by `Monster.tpl`, inherited by `Pet.tpl`) declares **`skillName1..17` and nothing above it**, and across **74,013** base-game records no Monster or Pet uses a higher slot. R-250/b93 wired the shroud to `skillName19` (boss) and `skillName18` (all 3 pet tiers) on a written-down belief that *"the template reaches 23"* - 23 being only the highest slot THIS MOD had already overflowed into. b93 shipped four fields nobody reads. Now on `initialSkillName` (cast at spawn, ON from frame one) + `buffSelfSkillName` (re-applied by the `svc_alwayson_` clones at `WheneverPossible`); neither takes a level field. Both defects logged in `docs/MISTAKES.md` in the same commit as the fix (R-254).
> - **RECORD-DIFF vs the shipped build98 `15dacc68`: ADDED 0 / REMOVED 0 / MODIFIED 4, ZERO unexplained** - the wild boss + the 3 soul-summon pet tiers, four fields each (dead `skillName/skillLevel` slot out, the two always-on channels in). The shroud skill, its pak, the EffectEntity mirror and both controller clones come out byte-identical; the 4 MESH-route marauders are untouched.
> - **THE PARITY GATE IS NOW THE WHOLE HOUSEHOLD** (the second defect): R-250 never looked at the marauders (wild escorts or pet-of-pet), which were covered only by luck because `ShadowStalker.msh` carries the FX compiled in. The roster is a bounded WALK (boss -> escorts -> pet tiers -> pet-of-pet + family name-tag clones) and each member must satisfy **MESH** (rig embeds `ShadowStalker_Smoke.dbr`, read from the `.msh` each build) or **FIELD** (shroud in both channels). Shipped bytes: **8 members, 4 MESH / 4 FIELD**.
> - **det-2x:** `1113f2c6` across two independent COLD builds (`SVC_NO_CACHE=1`, `PYTHONHASHSEED=0`, `SVC_RELEASE_DROPS=1`, `SVC_REQUIRE_GATES=1`), exit 0 both, MD5-equal AND `fc /b` *"no differences encountered"* over all 55,608,016 B.
> - **Gates:** in-build `enslaver_shroud.verify()` PASS (8-member walk, running FX untouched, no `charFxPak` on any SpawnPet skill); `--negtest` **40/40** (incl. the "strip one pet" plant and the R-255 defect replayed); `--selftest` PASS, re-deriving the ceiling from `Templates.arc` every run; `gate_ruling_ids` PASS (51 rulings, highest **R-255**, 165 branches); `validate_tags` PASS (394 mod tags, 455/455 authoritative, **0 new**); `contracts_balance` 0 violations + `tests_balance_negative` PASS; and the coexisting battery all PASS on the post-wave artifact - `gate_arena_spawn_guarantee`, `gate_toxeus_boss_equipment`, `verify_soul_drop_rates --gate` (20/10/0/**100**), `gate_uber_hoard_generosity` (**33** families), `gate_loot_volume` (39 surfaces), `gate_loot_distribution`, `gate_chest_loot_breadth`, `gate_orb_loot_breadth`, `gate_relic_difficulty_tiers`, `gate_chest_artifacts`, `gate_orb_legendary`, `gate_supra_recipe_laws`. Registry **68 modules**, order `e90bcca8601c` (unchanged). In-build `STRICT failures : 0`.
> - **Contracts SET-DIFFED vs build98:** new **0 P0 / 0 P1 / 4513 P2**, baseline identical, **0 only-in-new / 0 only-in-base**. No contract in the suite covers the always-on skill channels, which is exactly why the lane's own gate had to exist.
> - **ANTI-INERT on the artifact that actually shipped:** `enslaver_shroud.verify()` against the **shipped build98 arz** EXITS 1 with **8 problems** - 4x *"FAMILY FX PARITY: ... wears NEITHER route"* and 4x *"DEAD SLOT: ... carries the shroud at skillName19/18 ... read by nobody. This is exactly what b93 shipped"*. PASSES on `1113f2c6` and again on the LIVE DEV bytes.
> - **DEV:** targeted **arz-only** copy, md5 source==dest, **TQ.exe NOT running** (nothing killed, Steam not restarted). DEV inventoried before and after: **1 of 62 files changed**, 0 added / 0 removed, 61 byte-identical; DEV `Levels.arc` stays TESTHUB `666789ab`, DEV `Quests.arc` stays `d9f8c316`. Push-gate PASS (dist==work all 5; TESTHUB guard PASS - packaged canonical `1bf86461` vs live DEV TESTHUB `666789ab`; contracts on the DIST payload 0 P0 / 0 P1 / 4513 P2; single wrapper, 56 files / 1188.4 MB).
> - ✅ **`BL-b97-DEBT-1` honoured on the operator side:** the note was rewritten for THIS build and committed before packaging (`f7904c1`), and it discloses the previous update's false claim instead of hiding it. Tooling half still open; `BL-b98-DEBT-5` (no preview image) unchanged.
> - **Rollback (one step, uncoupled):** `local/DEV_arz_deployed_prev.arz` `15dacc68` -> copy back. This build's arz at `local/build99_run1.arz`; baseline verbatim at `local/build98_shipped_15dacc68.arz`; determinism twin at `local/b99_run2/`; logs under `local/b99_logs/`.
> - ⚠️ **NOT PROVEN IN-GAME (`BL-R250-DEBT-1`, carried forward and now the whole point).** Three prior rounds were record-level claims that were wrong in game. Will's check, full quit TQ + restart Steam first: **summon the Enslaver from ANY tier of his soul and just stand there, out of combat -> PASS = black smoke on him from the moment he appears, and on the marauders he raises.** FAIL = clean, or smoking only while running. **The souls already in his stash reach it** (the FX rides the pet RECORDS, resolved live from the database at summon time; it is not baked into the soul item at pickup). Detail: `docs/BACKLOG.md` -> BUILD99 GATE RECORD; test note: `docs/WILL_TEST_GUIDE.md` top entry.
> - 🆕 **`BL-R255-DEBT-1` (P1, other lanes):** six records park REAL combat skills above the `skillName17` ceiling where nobody reads them - `um_charonform2_ferryman_99` (18-21), `um_mnemophage_99` (18-23), `um_ephialtes_99` (18,19), `um_bloodtoxeus_99` (18), `um_mnemophage_core_99` (18), `um_toxeus_enslaver_99` (18 = `unholy_rally`, printed by the build on every run). The Enslaver's slots 1..17 are FULL, so his cannot simply move down: something has to give and that is Will's call. 🆕 **`BL-R255-DEBT-2` (P2):** two `## R-254` headings live in `docs/WILL_RULINGS.md`; `gate_ruling_ids`'s A1 arm cannot see the second.
>
> ## BUILD98 SHIPPED TO DEV **AND** STEAM (2026-08-15) - R-254: DYING COSTS **EXACTLY HALF** WHAT IT DID, and the TOMBSTONE STILL RETURNS **ALL** OF IT. **arz-ONLY.** **STEAM = DEV = main** (modulo the two TESTHUB variants DEV always carries).
> **Steam/canonical/DEV arz `15dacc68c118f50900a5a7100225e2a8`** (55,608,036 B, 51,331 records; **+1 byte / +0 records** over build97 `98741a4e` - the `/ 90)` -> `/ 180)` character, which is the whole ship). **arz-ONLY: `Text.arc 82d5b810` / canonical `Levels.arc 1bf86461` / `Quests.arc 6271ceb2` / `Creatures.arc 8c0d8d53` md5-proven BYTE-UNCHANGED, re-hashed after the build; 0 new tags.** DEV runs that arz with the **TESTHUB** twins `Quests.arc d9f8c316` + `Levels.arc 666789ab`. Workshop item 3759792705, `Committing update...Success.`, workshop log `Upload finished for workshop item 3759792705 : OK`, **ManifestID `7915955164139761983`**, `-Update -Visibility 0` with the VDF read back `"visibility" "0"` (stays PUBLIC). `main` `318cc2f` (build97-ship) -> `d3f7780` -> `bbd73bf` (`git merge --no-ff fix/death-penalty-halve-again`) -> `13bb427` (built-artifact table + change note) -> this gate record. Tag **`build98-ship`**. Next sequential ship after build97 = **BUILD98**.
> - **What it is:** `BL-W0814-4`, Will verbatim: *"lets reduce the penalty for dying by another 50% from what it currently is at."* Measured against what he plays, not against vanilla: the shipped state was R-80's pair (divisor **90**, cap **50,000**), so the scalar lands on THAT - **divisor 90 -> 180, cap 50,000 -> 25,000**, `deathPenaltyMin` untouched at 0. Exactly half the build97 loss at every level on every difficulty; a uniform x0.05 of vanilla. Design: `docs/WILL_RULINGS.md` -> **R-254**.
> - **THE CAP HAD TO MOVE WITH THE EQUATION.** The penalty is cubic and the mod ships `maxPlayerLevel = 1000`, so above ~L87 Legendary the old cap, not the formula, set the number. Halving the equation alone leaves L100 Legendary clamped at 50,000 - **no change at all** in exactly the high-level regime the report was about. `BAL-DEATHXP-2` reds that shape and its evidence must name a level >= 80.
> - **RECORD-DIFF vs the shipped build97 `98741a4e`: ADDED 0 / REMOVED 0 / MODIFIED 1, ZERO unexplained** - `records\xpack\game\gameengine.dbr`, exactly two fields (`deathPenaltyEquation`, `deathPenaltyMax`). Nothing else in 51,331 records moved. The narrowest record-diff any build in this repo has shipped.
> - **THE R-109 COUPLING HELD WITH ZERO EDITS ON THE RECOVERY SIDE - the first live exercise of the property R-109 was ruled for.** `tombstone_xp_recovery.py` was not touched. `Game.dll` hands the marker the **realised** loss (`RegisterExperienceLoss` VA `0x10194540` -> `GraveInfo+0x0C`, read back by `GetPlayerExperienceRedemptionAmount` VA `0x10194f60`) and never reads `deathPenalty*`, so `RedemptionMultiplier = 1.0` follows any retune by construction. Build verify on the shipped bytes: *recovered == lost EXACTLY on **3,012** checked points* (L1-1000 x N/E/L against the LIVE knobs plus the realised-loss domain), cap **671x** inside the float32 exact-integer bound.
> - **REPORTED BOTH WAYS (R-109's reporting clause), re-measured on the BUILT artifact AND again on the deployed DEV bytes** (`docs/reports/r254_logs/r254_before_after_table_BUILT_b98.txt`): L55 Legendary 12,940 -> **6,470**; L85 Normal 6,824 -> **3,412**; L85 Legendary 47,765 -> **23,883**; L100 Legendary 50,000 -> **25,000**. All 21 rows: `LOST now` exactly half of `LOST build97`, `back/lost` exactly **1.0000** on all three difficulties.
> - **det-2x:** `15dacc68` across two independent COLD builds (`SVC_NO_CACHE=1`, `PYTHONHASHSEED=0`, `SVC_RELEASE_DROPS=1`, `SVC_REQUIRE_GATES=1`), exit 0 both, MD5-equal AND `fc /b` *"no differences encountered"* over all 55,608,036 B.
> - **Gates:** `contracts_balance` **0 violations**; `tests_balance_negative` **55/55**; `death_xp_penalty --negtest` **8/8**; `tombstone_xp_recovery --negtest` **8/8** (plants 7+8 retune then halve the penalty and require the equality to still ACCEPT untouched); plus the coexisting battery all PASS on the post-wave artifact - `gate_arena_spawn_guarantee` (R-253), `gate_toxeus_boss_equipment` (R-252), `verify_soul_drop_rates --gate`, and the **nine** loot gates with `gate_uber_hoard_generosity` still measuring **33** hoard families; `gate_ruling_ids` PASS (50 rulings, highest R-254, no collision - `BL-R251-DEBT-6` did NOT bite); `validate_tags` PASS with **0 new tags**; in-build `STRICT failures : 0`.
> - **`BL-R254-DEBT-1` DISCHARGED:** registry re-derived on the merged tree = **68 modules**, order `e90bcca8601c` (unchanged), `damage_display` 14 -> `death_xp_penalty` 15 -> `tombstone_xp_recovery` 16, adjacent as required; the collision gate names `records\xpack\game\gameengine.dbr <- damage_display, death_xp_penalty, tombstone_xp_recovery` - **exactly the predicted TRIO, no fourth module** (which the debt says would have been a real finding).
> - **Contracts SET-DIFFED vs build97:** new **0 P0 / 0 P1 / 4513 P2**, baseline **3 P0 / 0 P1 / 4513 P2**; **0 only-in-new**, and only-in-base = exactly the two keys this ship closes (`BAL-DEATHXP-1/2`). The P2 debt floor is untouched and the only contract movement in the suite is the ruling landing.
> - **ANTI-INERT on the artifact that actually shipped:** `contracts_balance` against the shipped build97 arz **EXITS 1 with 3 P0s** - *"want '... / 180)', got '... / 90)'"*, *"want 25000, got 50000"*, and *"largest XP error at level 87 difficulty DV=2: vanilla 500000 -> shipped 50000 (90.0% cut, want 95.0%)"* - Will's report as an artifact fact, at exactly the capped level a half-fix would have missed. PASSES on `15dacc68` and again on the LIVE DEV bytes.
> - **DEV:** targeted **arz-only** copy, md5 source==dest, **TQ.exe NOT running** (nothing killed, Steam not restarted). DEV md5-inventoried before and after: **1 of 62 files changed**, 0 added / 0 removed, 61 byte-identical; DEV `Levels.arc` stays TESTHUB `666789ab` and DEV `Quests.arc` stays `d9f8c316`. Push-gate PASS (dist==work all 5; TESTHUB guard PASS - packaged canonical `1bf86461` vs live DEV TESTHUB `666789ab`; contracts on the DIST payload 0 P0 / 0 P1 / 4513 P2; single wrapper, 56 files / 1188.4 MB).
> - ✅ **`BL-b97-DEBT-1` honoured on the operator side:** the change note was rewritten for THIS build and **committed before packaging** (`13bb427`), so no stale note rode along. The tooling half (make `upload_workshop.ps1` refuse a note identical to the last one) is still open. ⚠️ New `BL-b98-DEBT-5` (P2): the upload warned *"no preview image - the Workshop item will show a BLANK tile"* - a standing public-facing defect, unchanged by this ship.
> - **Rollback (one step, uncoupled):** `local/DEV_arz_deployed_prev.arz` `98741a4e` -> copy back over `SoulvizierClassicDEV\Database\SoulvizierClassicDEV.arz`. Nothing else moves. This build's arz at `local/build98_run1.arz`; baseline verbatim at `local/build97_shipped_98741a4e.arz`; determinism twin at `local/b98_run2/`; logs under `local/b98_logs/`.
> - ⚠️ **NOT PROVEN IN-GAME (`BL-R254-DEBT-2`, P2).** Nobody has died on this build. Will's check, full quit TQ + restart Steam first: **die on purpose at a level you know and note the XP lost -> PASS = half what you are used to** (do it at a HIGH level too, that is where the cap fix shows); **then walk back to the marker -> PASS = you get back exactly the number you just lost, not half of it.** FAIL = the loss is unchanged (especially at high level), or the marker returns less than it took. Detail: `docs/BACKLOG.md` -> BUILD98 GATE RECORD; test note: `docs/WILL_TEST_GUIDE.md` top entry.
>
