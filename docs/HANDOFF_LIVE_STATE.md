...
> Implemented, gated STATICALLY, **not built, not deployed, not tagged**. Branch tip carries `tools/build_shroud_rig.py` (new), `tools/patches/enslaver_shroud.py` (rewritten), `tools/patches/champion_mesh.py`, `tools/build_creatures_dye_skins_arc.py`, `scripts/bootstrap_working_mod.ps1` and the R-257 doc set.
>
> 🚨 **READ THIS BEFORE SHIPPING IT: A NEW DEPLOY COUPLING JOINS THE OTHER TWO.** Levels+Quests, arz+Text, **and now arz+`Creatures.arc`**. R-257 gives `um_toxeus_enslaver_99`, the three `toxeus_enslaver_*` pet tiers and his two preview proxies a `mesh` that ships ONLY in the mod's own `work\<mod>\Resources\Creatures.arc` (`monster/skeleton/svc_enslaver_shroudrig01.msh`, authored by `build_shroud_rig` from the base rig + the 115-byte `CreateEntity` block copied verbatim off `ShadowStalker.msh`). **A ship that rebuilds the arz WITHOUT restaging and deploying that archive leaves six records pointing at an asset that resolves nowhere - an INVISIBLE boss, strictly worse than the missing smoke.** Restage is cheap and standalone: `py tools/build_creatures_dye_skins_arc.py --out work\SoulvizierClassic\Resources\Creatures.arc`. It is GATED, not merely documented: `enslaver_shroud.verify()` arm **M2** fails the build loud whenever a reachable mod `Creatures.arc` does not carry the rig byte-identical to the derived bytes. `Creatures.arc` has been md5-`8c0d8d53` and "byte-unchanged" in every gate record since build93; **that line stops being true with this lane.**
>
...
>
> **Static gates green on the branch:** `build_shroud_rig --selftest` PASS / `--negtest` 10/10; `enslaver_shroud --selftest` PASS / `--negtest` 29/29; `champion_mesh --negtest` 15/15; `gate_dye_skins` PASS; `gate_ruling_ids --branches` PASS (53 rulings, highest **R-257**); patches-registry `selfcheck` OK (69 modules); STATIC dry-run of both modules in registry order over the shipped build100 arz **GREEN** (8/8 household members MESH, 6 records touched). **No claim is made that it renders** - that is `BL-R250-DEBT-1`, open for the fifth time, and only Will's eye closes it.
