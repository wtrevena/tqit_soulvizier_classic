...
- **2026-09-11 | b42 round-2 agent lane (`db2d409`, 2026-07-13 20:54, author wtrevena = an agent on Will's account), reported by Will in play 2026-08-13/14 (SIX times: the stash, the Obsidian Hoards, "aphoryteus dread hoard", Propontis, Tantalus, plus the gift box ask), diagnosed and fixed at the source by R-247.7a + R-251 (2026-08-13/14), blamed and floor-gated by R-261:** `apply_svc_patches._svc_standardize_boss_chests` ("b42 CHEST: reduce + region-tune every placed-uber bespoke hoard to Cyclops-grade base loot") rewrote EVERY placed-uber hoard chest's `tables` to a base-game `boss_default_<lo>-<hi>` at finalization - an implementer's scope decision (`docs/reports/b42_fixedboss_dedup.md` S3.3), never a Will ruling (Will's b42 words were about the chest COUNT), and its own report said the consequence out loud: *"The old bespoke guaranteed-unique loot tables are left unreferenced."* On the shipped build92 arz `b888f022`: 21 of 30 uber chests on a base-game table (S 7.36, `loot3Chance` 10, NO guaranteed row), 18 of 27 bespoke hoard tables with ZERO references, the Propontis chest with no table of its own at all. R-180 / R-181 / R-220 then tuned the dead records for three waves, and R-240's volume trim (`53ae6d5`, 2026-08-11, Will-ratified for the CAGE) swept the hoard family and the Devourer's stash (`*3.8/*4.1` -> `*0.323/*0.3485`, an ~11x cut) a second time. Cost: **every placed-uber chest in the mod paid Cyclops-grade loot from build42 (2026-07-13) to build94, the stash paid ~1.6 iterations from build84 to build94, through FOUR green chest gates; six angry reports from Will in one session; three tuning waves spent on records nobody could open; two fix lanes (R-247.7a, R-251) and this forensic re-dispatch.** Root cause: a finalization pass that re-pointed a WIRE, and four gates that every one audited the TABLE against a MOD-AUTHORED band the nerf itself could re-author - none compared the chest to Will's yardstick (what the original Soulvizier paid) and none audited which table the player actually opens. Guard: R-251's H1 (the wire) + H2 (no orphans) since 08-14; and now R-261's `tools/gate_chest_generosity_sv_parity.py`, whose floor is READ from the upstream sv098i arz on every build (never authored here) and which reds on every re-nerf this repo has actually shipped (12/12 planted on the real build103 arz), wired into the post-write battery.

- **2026-09-11 | orchestrator brief for the `fix/chest-generosity` re-dispatch (minor, no damage):** the brief prescribed a RESTORE of five chests + a gift box that the BACKLOG lines for `BL-W0814-2/-5/-7/-11/-13` already marked "ADDRESSED BY R-251" (merged 08-14, shipped build95 08-15, re-verified live on build97), reading the queue's "all still open" (= awaiting Will's in-game count) as "unfixed". The hypothesis (one shared cause) was right; the fix status was four weeks stale. Cost: none realised - the lane measured before writing, found ZERO bytes to change, and spent the dispatch on the one thing the repo lacked (a floor gate keyed to SV, not to the mod). Root cause: a brief that carried the item's ASK without its STATUS line. Guard: a chest/loot brief quotes the BACKLOG status of every BL id it re-dispatches, and a restore is prescribed only after a bytes-vs-reference measurement says the surface is actually under its reference (the same STALENESS clause as the 2026-08-10 volume-trim entry, now applied to fix status as well as repo facts).

## 2026-09-10

- **2026-09-10 | build36 content-wave agent lane (`79de1d7`, 2026-07-11 15:46, author wtrevena = an agent on Will's account), found by the R-258 vet and repaired by R-260:** `apply_svc_patches._svc_guarantee_unique` chose a monster's "free" loot slot with `for n in (4, 5, 6, 3)`, i.e. it looked for an unused `lootMisc4Item1` FIRST - and `Misc4` is unused on every record in existence because `characterloot.tpl` declares exactly Misc1..Misc3. Its own comment called this "the exact proven `_create_soul` mechanism" (which is `Finger2`, a declared slot). Four later writers copied the idiom (`toxeus_suite` b37, `turtleshell_relics` 07-15 - whose docstring said "`lootMisc4` is verified genuinely free on every body in the family", `toxeus_endofallthings` b83 - "Misc4 is used by 29 monsters", all 29 our own writes, `toxeus_hunt_encounter` b98), and every verify() asserted the FIELD (`chanceToEquipMisc4 == 100`) instead of the DELIVERY. Cost: **7 "guaranteed" drops dead since they shipped - the Golden Bough, Lethe's Draught and the Mask of the Waking Dread since build36 (2026-07-11), the rant scroll since build37, the Rite formula off the Enslaver and the Devourer since b83, off both Hunts since b98 - plus 25 animal-relic sources at 7-10% that never paid (the Reveler's Ruse, the Sepulchral Scale, the Sanguine Tithe, the Ereban Heartstone), through eleven builds of green gates, up to and including build102; and Will's Legendary Endless-Hunt kill that did not drop the formula (R-247) was explained away as a visibility defect instead of read as the measurement it was.** Root cause: a slot chosen by a name that was never checked against the template, and gates that read the record's own field back to itself. Guard: R-260 - `tools/svc_loot_slots.py` is the one slot model (the eleven declared groups, hard-coded), `gate_no_phantom_loot_slots` holds every arz at ZERO undeclared slot variables (RED on build102, 35 carriers), `gate_guaranteed_drops_measured` re-derives every roster drop's rate from `chance x weight / sum(weights)` on a declared slot (RED on build102, 31 of 31 at 0.0000%), and the standing law from R-258 section 4 now has teeth mod-wide: a drop this mod calls guaranteed is MEASURED, never declared.

- **2026-09-10 | the first R-260 implementer (a Fable subagent on `fix/misc4-audit`), killed by a session limit mid-work with ~16 files UNCOMMITTED** - `tools/apply_svc_patches.py`, `arz_patcher.py`, `build_svc_database.py`, `patches/__init__.py`, eight patch modules, four brand-new files (the helper, the registry module, both gates) and no commit, no push, no REPORT.md, so its reasoning was LOST and every hunk had to be re-derived and re-vetted by the resuming implementer. Cost: the recovered tree carried two build-breakers the author had not yet run a build to see (`charon_rework` muted only Misc1/2 so the re-cloned terminal's Misc3 stayed LIVE at the donor's 75 and the pinned takeover would have refused it; the roster's four frost-wyrm clones and the endless-Hunt clone inherit their rows by `clone_record` and never passed through the helper, so the module's own ledger cross-check would have redded the build), plus a review day. Nothing was lost only because the successor's brief ordered `git add -A tools/ && git commit && git push` BEFORE reading a line (checkpoint `1ffd0ec`). Root cause: sequencing by "what is finished" instead of by "what survives a kill" (the 2026-09-09 R-258 entry's sibling), in a session with no commit-early rhythm. Guard: **commit + push after EVERY meaningful step of a lane** (a new file, a helper rewrite, a caller re-wired, a gate green) - a WIP commit on the lane branch is free and a lost session is not; a lane's first commit lands within minutes of its first edit; and `docs/MISTAKES.md`'s own protocol applies to the orchestrator that dispatches a long lane without a checkpoint cadence in the brief.

- **2026-09-10 | orchestrator, my own:** I reported a 'confirmed negative' (no copy of the Soulvizier archives on any mounted filesystem) to Will TWICE, built on filename-only searches that cannot see inside .7z/.rar containers; the content-based hunt then found 5 of 6 inputs on the NAS inside `Z:\Computer Backup\Files\static\Games\TQ.7z` + `Games.7z` (Will's 2020 laptop backup). Cost: I told Will he was wrong when he was right; the re-download was strictly needed only for SV 0.4.1. Root cause: reporting a negative as confirmed when the method could not observe the space. Guard: a negative on 'does X exist on disk' is only confirmed after container archives are LISTED (`7z l`) and content-hashed; otherwise say 'not found by <method>'. Follow-through: all four archives are now copied off-repo to `Z:\Computer Backup\tqit_soulvizier_classic\third_party_archives\` (README there), and `BL-b102-DEBT-1` asks `check_build_inputs --extract` to learn `.7z`/`.rar` so a fresh machine never depends on a human search again.

- **2026-09-10 | R-258 implementer's docs, found at the build102 integration (minor, batched):** `BL-R258-DEBT-6` had been appended to `docs/BACKLOG.md` BELOW the unrelated `BL-W0814-11` paragraph instead of inside the R-258 DEBTS list it belongs to, so the P0 ship blocker sat under a chest item. Cost: none realised (the integrator found it while rewriting the debt as discharged). Root cause: an append anchored on "the next blank line" instead of on the list. Repair: re-seated in the integration docs commit. Guard: a debt line goes under the `DEBTS` heading of its own ruling block, and the vet of any docs commit reads the debt list top to bottom.

- **2026-09-10 | deploy tooling, inherited by every agent that ran a deploy since 2026-02: `scripts/deploy_to_custommaps.ps1` snapshotted the ENTIRE deployed mod (~1.3 GB) into the repo's `backups\deployed\` on every deploy with NO rotation, and nobody noticed for seven months** - the character snapshots beside it had a keep-10 from day one, the deploy snapshots never got one, and ~250 of them (2026-02-21 onward) added up to the ~300 GB Will found when C: ran out of space (Will, verbatim: "why do we have a 300gb backups tree? no wonder i have no disk space"). Cost: ~300 GB of Will's system drive, and his time. Root cause: an unbounded per-run copy of a gigabyte-scale artifact onto the system drive, with no retention policy and no size signal in the deploy output. Guard: R-259 (branch `fix/backups-to-nas`) - snapshots go to the network drive under the REQUIRED `WIN_BACKUP_ROOT`, only the newest `BACKUP_KEEP` (default 5) per mod are kept, the deploy prints the root and the keep count every run, and there is no local fallback; the rotation delete itself is guarded per the 2026-09-09 entry below. The legacy tree is `BL-R259-DEBT-1` (Will's call; not deleted by the lane).

- **2026-09-10 | build102 integration docs (the orchestrator's), found by the build102 ship operator:** the integration commit `6fac909` wrote in three places (`docs/BACKLOG.md` R-259 DEBTS + the integration block, `docs/HANDOFF_LIVE_STATE.md` in-flight block, `docs/WILL_RULINGS.md` R-259 DEBT-2) that `BL-R259-DEBT-2` "is discharged by the build102 DEV deploy and the ship commit records the result", without checking the DEV runbook that sits beside those words: every DEV deploy since build85 is a TARGETED coupled copy into `CustomMaps\SoulvizierClassicDEV` (HANDOFF build101 + build85 blocks), never `scripts/deploy_to_custommaps.ps1`, whose hardcoded target `CustomMaps\SoulvizierClassic` does not exist on this machine and would create a second mod or clobber the TESTHUB pair. The R-259 snapshot + rotation code therefore had no path to run on this ship, the ship brief itself said so, and the debt stays OPEN with the sandbox harness (69/69) as its only evidence; all three claims were annotated rather than deleted. Guard: a debt whose discharge is "the next deploy" must name the script AND the target that deploy actually uses, and a ship brief that inherits such a debt must say which path it will take before the ship starts.

## 2026-09-09

- **2026-09-09 | repo housekeeping operator, my own: a three-pass worktree prune DESTROYED the gitignored build inputs (`upstream\` = SV 0.98i/0.9/0.4.1 databases + SV 0.98i Text_EN.arc/Levels.arc/Creatures.arc; `reference_mods\SVAERA_customquest`; `work\SoulvizierClassic\Resources` = the 55 shipped arcs) at 13:18:48-51** - the zero-data-loss gates I wrote covered git-TRACKED content (is-ancestor-of-main proofs; backup tags peel-verified on origin) and worktree cleanliness (`status --porcelain` empty), and every one of them held: no commit was lost and 18 `backup/prune-2026-09-09/*` tags preserve every pruned branch. What they never checked was REPARSE POINTS. A worktree root carried Windows junctions into the main checkout's shared caches - the exact shape docs/BACKLOG.md:6633 prescribes for worktree builds, beside its own :6637 "HAZARD, because a junction writes through" warning - and the recursive delete under `git worktree remove` descended them and emptied their TARGETS, leaving the target directories as shells. The signature is unambiguous: `Database` (never junctioned) intact beside an emptied `Resources`; six surviving `local\b96-b101_run2\...\Resources` junctions enumerating 0 entries; the three victims stamped in alphabetical walk order three minutes after the prune tag. Cost: the DB + Text builds cannot run until the three Soulvizier upstream archives are re-downloaded - no copy exists on any local drive, and `third_party\` had already been emptied on 2026-08-12, which removed the offline fallback. `work\...\Resources` and `reference_mods` were restored the same day from dist / Workshop 3759792705 and Workshop 2076433374. DEV play surface, dist, backups, local, Steam: intact, hash-verified. Root cause: I gated deletion on git state and never enumerated `ReparsePoint` attributes before a recursive removal on Windows. Guards: (1) before ANY recursive delete or `git worktree remove` on Windows, run `Get-ChildItem -Recurse -Force -Attributes ReparsePoint` over the tree and REFUSE if any link targets outside it; (2) never create junctions inside worktrees - the build-input resolver already falls back to the main checkout's cache (CLAUDE.md "Build inputs"), so the BACKLOG:6633 recipe is RETIRED; (3) keep an off-repo copy of the three upstream archives, because the loss was unrecoverable ONLY because `third_party\` was already gone. Addendum, same day: my restore brief prescribed `Copy-Item -LiteralPath <src>\* ...`, which is a silent NO-OP (`-LiteralPath` never expands wildcards) that reports success while copying nothing; the recovery agent caught it only because it re-counted the destination after the copy. Guard (4): every restore/copy step verifies the destination by count + hash, never by the command's exit status; and never pair `-LiteralPath` with a wildcard. Correction, 2026-09-10: the sentence "no copy exists on any local drive" above was the filename-only negative retracted in the 2026-09-10 orchestrator entry; five of the six inputs were inside the NAS's `TQ.7z` + `Games.7z`, only SV 0.4.1 needed a re-download, and all four archives are now copied to `Z:\Computer Backup\tqit_soulvizier_classic\third_party_archives\` (guard 3 satisfied). The cold build has since run on the restored inputs (arz `7dad9a8a`, 2026-09-10).
