...
- **`chanceToRun` is THE roulette dial, and it is native to this exact region:** the halls' own trash proxies ship at `chanceToRun` 5 / 15 / 25 (`ug_undead_liche_01n/02n/03n`, `area007 - tomb`). Our mod already ships a working sub-100% (`q_bloodtoxeus_ambush`, `chanceToRun=33`, the Toxeus entrance ambush). It re-rolls each session, so the roulette re-shuffles every run. *(The old `q_bloodtoxeus_lone_50` @50 that this line used to cite was RETIRED with the parchment feature on 2026-07-14; the ambush proxy is the live sub-100% precedent.)*
