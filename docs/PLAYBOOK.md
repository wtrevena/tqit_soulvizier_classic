...
- **Four DB build invariants + map gates are fail-loud.** A build that trips one does NOT write/ship.
...
- `-Visibility 0` public / 1 friends-only / 2 hidden / 3 unlisted.
- **NEVER package/upload a TESTHUB build.** package_workshop.ps1 packages `work/` — ensure `work/`
  holds the CANONICAL map, not the hub, before packaging.
