...
- **PR-5 (TRIAGED 2026-08-06 -> NEEDS-WILL; classification (a) present+reachable-but-undiscoverable):** cannot find the secret portal to the "Depths of the Spartan Crypt" in the Athens catacombs.
  **GROUND TRUTH from the SHIPPED bytes (canonical map I built this session md5 `f30303ed48c8861ad9b38a94eff8393d`, 688,690,310 B, `py tools/svaera_plus_portals.py`; shipped arz `adc7ee4afbae54dfd883a3c52ddbcf51`; shipped Quests.arc `bd0fb5f99d88fab74b81f27b7cb952b2`):**
  - **The crypt IS present and IS REACHABLE on ship** - NOT "unreachable by design". The prior RCA (`9dc5b09`) that concluded "Crypt L2 unreachable by design" was run against the DEV/**TESTHUB** deploy where Almyros (`portal_master_helos`) is de-duped/UNPLACED. On the **canonical/Steam** map Almyros IS placed and the route works. VERIFY-THE-BYTES caught the stale doc.
  - Proof chain (all from the built canonical map + shipped arz/Quests): (1) `portal_master_helos.dbr` ("**Almyros the Wayfarer**", Class=Npc, msgDialogTag `tagSVCHelosPortalChat`) placed **x1** in the **Helos starting plaza** (startingfarmland06d, world (-5972,2,918)); (2) his `sv_commonmechanics.qst` boat-dialog carries **4** destinations incl. `tagSVCHelosToSparta`="The Sparta Crypt" (`portal_master_helos` x4 + `tagSVCHelosToSparta` x2 in the shipped quest bytes); (3) that route lands at world **(-5602,-2,-1409)** which is **ON-MESH inside spartacryptlevel2** (nearest walkable cell 0.14u in all 3 tilesets, main component comp#1/75244); (4) spartacryptlevel2 has a **valid navmesh** (v0x0e, 0x0b present + no 0x0a, `parse_rec02` OK, center (-5600,12,-1376), own GUID `797c7859...` resolves, 225,744 walkable cells) - b89-class concern CLEAR; (5) `svc_testhub_return_sparta.dbr` placed **x1 inside** the crypt (world (-5599,-2,-1409), on-mesh) so the player is not stranded.
  - **Why Flozer44 (and Will on 2026-07-14) can't find it:** there is **NO portal/door in the Athens catacombs**. CataCube02_FloorLast's 0x14 (portal/level-change) section is **EMPTY**; **map-wide there is exactly ONE** occurrence of the crypt GUID - the crypt's own navmesh - i.e. **zero inbound map-portal to the crypt anywhere**. Entry is by NPC boat-dialog only, from an NPC in the **start town**, whose name/menu give no hint it is the way into the Sparta Crypt. Real, severe **discoverability** defect (not a data bug).
  - **Minor debt (not the report, not a blocker):** the Almyros Sparta landing (-5602,-2,-1409) sits **2.38u** from a `greece_sarcophagia02_02` container (gate_landing_clearance FAIL vs the 4.0u container class-min; NOT deadly - PIN is 1.5u, footing 60%). Cramped but walkable. Nudge-fixable if the entry is reworked.
  - **NEEDS-WILL design pick (the real fix; do NOT blind-edit the map):** see ATHENS-CRYPT-L2-REACH entry below (corrected + turnkey spec). Immediate safe stopgap shipped this session = the WILL_TEST_GUIDE canonical-access note + the player answer.

> 🏛️ **ATHENS-CRYPT-L2-REACH (PR-5, canonical-scoped RE-TRIAGE 2026-08-06 - awaiting Will's design pick; supersedes the TESTHUB-scoped 9dc5b09 entry).** The Sparta Crypt is reachable on ship ONLY via **Almyros the Wayfarer in the Helos start plaza** (proven above). Nobody expects that - the player hunts the Athens catacombs; even Will (2026-07-14) "thought the path was in the ancient tomb." Two ways to resolve, pick one:
> - **Opt-1 (doc/hint only, ZERO map risk, shippable now):** keep the Helos-Almyros hub; tell players where the entrance is (Workshop description line + in-game already names it "The Sparta Crypt" in Almyros's menu). The WILL_TEST_GUIDE canonical note is in this session's commit. Cheapest; leaves the mental-model mismatch.
> - **Opt-2 (RECOMMENDED - add a discoverable catacomb entrance, matches player+author mental model; a real map+quest lane needing in-game QA):** place a gatekeeper NPC in the DEEPEST Athens catacomb, right by the stairs-down, that offers "Descend into the Sparta Crypt". **Turnkey spec (all coords proven this session):**
>   - MAP (`build_section_surgery.py`, BASE `INJECT_SPECS` = canonical, NOT the testhub merge): place record `records\quests\svc_area_return_sparta.dbr` in `levels/world/greece/athens/underground/catacube02_floorlast.lvl` at **level-local (25, 1, 38)** [= world (-6587,1,-3180); catacomb corner (-6612,0,-3218); surveyed on-mesh comp#1, nearest hard collider 3.69u > mins, next to `stairsdown01.dbr` @6.07u, amid `AG_Beastmen_Gorgon` catacomb mobs]. This is a **0x05 instance add into a v0x0f base blob - the 0x0b navmesh is UNTOUCHED (byte-identical)**; prove with the standard dry-run-into-copy + blob-diff (navmesh identity) before shipping.
>   - QUEST (`build_quest_files.py`, canonical boat-dialog - mirror `_add_helos_portal_travel`): ONE `Action_BoatDialog` on `svc_area_return_sparta` -> dest **(-5602,-2,-1409)** with a new menu tag `tagSVCEnterSpartaCrypt`="Descend into the Sparta Crypt", appended to the always-loaded `sv_commonmechanics` refire step (registry law - no new QUESTS registration). Keyed to the DISTINCT record = warden-mute-safe (it is that NPC's sole route on canonical).
>   - TEXT (`apply_svc_patches.py`): `tagSVCEnterSpartaCrypt` (+ optionally rename `svc_area_return_sparta` to a catacomb-appropriate "Keeper of the Spartan Crypt" via its existing `description` tag). validate_tags gates it.
>   - RECORDS ALREADY SHIP: `svc_helos_trav_sparta.dbr` AND `svc_area_return_sparta.dbr` are BOTH already in the shipped arz (inert on canonical because unplaced) - **NO new arz record needed**, only a placement + a route + one text tag.
>   - RETURN: `svc_testhub_return_sparta` already stands inside the crypt (returns to Helos on canonical; the "back to this catacomb door" primary is currently TESTHUB-gated - add it canonically in the same lane if desired). Fold the 2.38u sarcophagus landing nudge in while here.
>   - GATES: `gate_landing_clearance` already covers (-5602,-2,-1409); add the catacomb NPC spot (25,1,38) to the placement check; **in-game QA on a fresh Custom Quest char required** (walk to the catacomb, talk, descend, return). **Opt-C from 9dc5b09 (a walk-in GridEntrance door) stays BANNED** per the 2026-07-12 no-walk-through-teleport directive.
> - **Opt-3 (one-line minimal, quests-only):** repoint Almyros's `tagSVCHelosToSparta` menu to also stand a copy at the catacomb - NOT recommended (still leaves the primary NPC in Helos). Opt-2 is the clean version of this.
