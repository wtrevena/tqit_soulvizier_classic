...
- **PR-4 (DIAGNOSED 2026-08-06, NEEDS-WILL, P3, text/data-only):** Gorgon names SWAPPED - "Impious" and "Geomancy Adept" on the wrong monsters.
  - **Ground truth (proven from shipped arz `adc7ee4afbae54dfd883a3c52ddbcf51` + base TQAE Text_EN):** the player's "Impious" = **"Gorgon ~ Profaner"** (`tagMonsterName1256`); "Geomancy Adept" = **"Gorgon ~ Geomancer"** (`tagMonsterName1263`). Those exact literals ("Impious"/"Geomancy Adept") appear in NO text source (base, SV098i, or our modstrings) - the player paraphrased. The two carriers are the gorgon casters:
    - `records\creature\monster\gorgon\ar_pyromancer_13.dbr` + `_16` (kit = BlazingWeapons + PillarofFlame = **FIRE**) -> our build shows **"Gorgon ~ Profaner"** (1256).
    - `records\creature\monster\gorgon\ar_venomancer_13.dbr` + `_16` (kit = Arachnos_VenomBolt + Arachne poison cloud = **POISON**) -> our build shows **"Gorgon ~ Geomancer"** (1263).
  - **Origin:** our arz is **byte-identical to SV098i** on these records (SV098->ours field-diff = empty). SV098i deliberately rebalanced these records (life/loot/skill levels) AND flipped the description tag vs **base TQAE**, which assigns the OPPOSITE: `ar_pyromancer`->1263 (Geomancer), `ar_venomancer`->1256 (Profaner). Both text files agree on the strings, so this is purely a record->tag pointer difference. Relative to the base game the reporter knows, the two names ARE swapped.
  - **Why NEEDS-WILL not auto-fixed:** the only two gorgon-caster names that exist are "Profaner" (neutral) and "Geomancer" (=earth); **neither matches fire or poison**, so no element-correct answer is derivable from the kit, and the two authorities (base TQAE vs amgoz1's SV098i) disagree. "Restore base" just moves the earth-name misnomer from the poison caster onto the fire caster. Cosmetic display-name taste call on amgoz1/SV-lineage content -> Will's call.
  - **Scope (proven):** EXACTLY 4 records reference 1256/1263 (pyro_13/16, veno_13/16); no elite/champion pyro/veno variants; zero other carriers. No tag STRING changes (tags 1256/1263 already resolve correctly), so no new text surface for Option A.
  - **Option A (minimal, restore base TQAE, RECOMMENDED as safe default):** repoint `ar_pyromancer_13/16` description -> `tagMonsterName1263`, `ar_venomancer_13/16` -> `tagMonsterName1256`. **Dry-run proven** on a copy of the shipped arz: exactly 4 records change, only `description`, 0 added/0 removed; read-back = pyro shows "Gorgon ~ Geomancer", veno shows "Gorgon ~ Profaner" (matches vanilla). Fixed-copy md5 `19bbc6b6c5f6e8af04ed485d4f751577`. Zero new text; arz-only (rebuild Text.arc alongside per the coupling law, but Text content is unchanged).
  - **Option B (element-matched, amgoz1-worthy, needs 2 new tags + a resolves-gate + Will naming approval):** follow SV's OWN precedent (SV already added `tagNewMonster60 = "Gorgon ~ Cryomancer"` for the ice gorgon) and name each caster to its element: pyro -> "Gorgon ~ Pyromancer", veno -> "Gorgon ~ Venomancer" (exact wording amgoz1/Will's taste). Eliminates the earth-name misnomer entirely; unambiguously correct, but a NEW player-visible surface.
  - **Recommendation:** if Will wants the cheap un-swap, take Option A (proven, reversible, zero new surface). If he wants it actually right, Option B. Awaiting his pick; no ruling appended and default build UNCHANGED until he decides.
