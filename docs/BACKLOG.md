...
- **PR-2 (DIAGNOSED 2026-08-06, NEEDS-WILL - `fix/pr2-dyes-grey`):** special DYES from the Garden of Merchants grey the PC. ROOT CAUSE (byte-proven vs the shipped arz md5 adc7ee4a + shipped Levels.arc + base Creatures.arc + SVAERA upstream arcs): the GoM hosts SIX SVAERA "IN PROGRESS New Dyes Merchant" AllSkins vendors - `dudechest`/`skirtchest`/`slutchest01` (containers) + `ven_dudes`/`ven_skirt_adult`/`ven_skirt_gen` (merchants), all under `records\drxmap\zgardenofmerchants\`. Each sells via `marketFileName -> records\allskins\asvendors\marketcfg\0{1,2,3}_jack_sparrows_wares.dbr` -> ~245 `OneShot_Dye` items whose `maleTexture`/`femaleTexture` reskin the PC to custom `Creatures\PC\{Male,Female}\{male,female}pc01_<Author>_<Style>.tex` (adodds/aishtaras/amorbiis/aooalejooos/mauls/... community AllSkins pack). **244 distinct reskin textures resolve in NEITHER our mod arcs NOR base Creatures.arc NOR SVAERA's own SV_NewSkins.arc / N66_Mods.arc** (resolved-only-in-upstream = 0). TQ resolves a `Creatures\...tex` from a `Creatures.arc` we do not ship; the custom skin is absent everywhere -> engine renders the PC with an unresolved (flat grey) material = the report. This is unfinished SVAERA content built on the `OneShot_Dye` allskins system already on our DELIBERATELY-SKIP list (SVAERA-graft-scope decision). NOT a map/navmesh issue; the 6 vendors live ONLY in the GoM hub (`world01.map`, GoM-specific, no shared-record carrier). CONTRAST - the WORKING dyes (`dye01_white`..`dye15_purple`, base-game recolors) are stocked by GoM GENERAL merchant records (`merchant_startingtown_general`, `0{1,2,3}_market_startingtown_general`, `xtra_merchant_*`) via `marketBraceletTable -> dye_{01-07,11-65}all.dbr` and reskin to base `MalePC01_<color>.tex` textures that DO resolve (verified 60/60); those work fine. **RECOMMEND (drop):** neutralize the 6 AllSkins vendors so they stop selling grey dyes. Cleanest DB-only edit = blank the `marketFileName` (3-entry string array) on all 6 GoM-specific NpcMerchant records (no shared-record edit, no .lvl/navmesh touch, reversible); alternatively de-place the 6 NPC instances from `gardenofmerchants.lvl` (proven MAP-REF-1 `remove_0x05_instances_by_dbr` pattern). **ALT (source art):** obtain the original third-party multi-author AllSkins skin pack (244+ `Creatures\PC\...tex`), pack + ship a Creatures.arc, clear multi-author permissions + payload - NOT present in SVAERA's own arcs, so a real sourcing/permissions/size effort, not an arc un-gut. Awaiting Will's drop-vs-source call before any arz change. (Repro scripts in session scratchpad: probe_gom_full.py / probe_upstream_skins.py / probe_resolve2.py.)
