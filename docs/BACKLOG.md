...
- **PR-5 (IMPLEMENTED 2026-08-06, branch `fix/pr5-catacomb-traveler`; IN-GAME WALK IS THE REMAINING GATE):** cannot find the entrance to the "Depths of the Spartan Crypt" in the Athens catacombs. RESOLVED per Will's decision (WILL_RULINGS **R-160**): "the Sparta Crypt should be entered from the Athens CATACOMBS, not from Helos." Coupled MAP + QUESTS change; **arz + Text UNTOUCHED** (the enter-offer menu tags already ship, and neither `build_svc_database` nor `apply_svc_patches` imports the two edited modules - proven).
