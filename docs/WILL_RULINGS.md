...
- R-10 [2026-07-16] IMPLEMENTED fix/runtime-green (b75) + PENDING follow-up: Enslaver summon = BLACK (steal the marauders' shadowcloak smoke). Diadochi generals + Helepolis share the green-rendering 343_dark_smoke - swap after Will confirms the Enslaver reads black. ⚠️ b92 UPDATE: b75's shroud swap was a REAL fix to a REAL second surface but was NOT the green Will kept reporting - see R-50.
- R-50 [2026-07-27] IMPLEMENTED b92 (fix/green-diff, tag build53-dev) - THE GREEN GLOW, verbatim: "i am pretty sure it is a skill or ability or something that is causing the green glow. i think this was inherited from the Toxeus the Murderer uber boss base monster that we created these monsters off of. If you compare the visuals to the secret passage toxeus the murderer who doesnt have the green glow, you may be able to find the difference". Will's method was the fix: the secret-passage Toxeus `um_toxeus_99` is the CONTROL and the differential against it produced the answer immediately after four failed rounds. ROOT CAUSE, PROVEN FROM ASSET BYTES (not names): the `mesh` field. A TQ `.msh` can attach an entity to a bone, and `Creatures\Monster\Skeleton\RevenantPoison.msh` ends with `CreateEntity { attach = "Waist"; entity = "Records\Effects\MonsterFX\Buffs\RevenantPoison_FX.dbr" }` -> `Effects\MonsterFX\Buffs\RevenantPoison.pfx`, whose three colour keyframe tracks decode to R 0.534 / G 1.000 / B 0.591 = GREEN (the same decoder independently reproduces fire=orange, frost=ice-blue, storm=blue-violet, which is what validates the channel assignment). The Devourer + Enslaver INHERITED that mesh from the green-by-design Greece Toxeus `um_toxeus_21`; the control wears `RevenantStorm.msh` (green is its LOWEST channel). Roster corroboration: every OTHER RevenantPoison.msh wearer also wears `newskeleton_grean.tex` - vanilla pairs that mesh with the green skin on purpose, and our two bosses were the only wearers given a crimson/charcoal skin. ⚠️ THE LESSON, now a standing rule: **a `.msh` can carry its own FX attachment, so "the DB is clean" NEVER proves a visual is clean.** b55 (FX fields), b71 (soul->skill->icon->pet chain), b75 (shroud pak) and b81 (pet identity) were all DATABASE fixes and could not see it; b75 even opened RevenantPoison.msh, read its texture strings and cleared it as "innocent" without reading the trailing entity script. Decisive tell that was available the whole time: the Devourer carries NO `charFxPakRunningNames` at all, so no shroud theory could ever have explained his green. FIX: `mesh` -> `Skeleton01.msh` (same shader + same textures so the crimson/charcoal skin overrides still land; closest geometry; canonical for the `anm_skeleton01` table both bosses already use; NO CreateEntity; already worn by 721 shipped records) on exactly 12 records - the 2 fought bosses, their 6 soul pets, and their 4 spawn proxies. Owner: registry module `tools/patches/toxeus_mesh_aura.py`, whose `verify()` gates it in BOTH directions (targets clean AND `um_toxeus_21`'s intentional green + the `um_toxeus_99` control preserved); negative test 17/17 incl. the deployed arz `1c27d5fa` failing the gate. NOT TOUCHED, deliberately: the Greece green Toxeus (Will: intentional), the secret-passage control, and the EoAT pets (other lane - BL-b92-DEBT-1). docs/reports/b92_green_differential.md.
