...
- R-17 [pre-M4, STANDING] Duplicate rant-scroll drops on repeat Blood-Toxeus kills are ACCEPTED (Will)
  ...
  across kills. Source: docs/MULTIPLAYER_COMPAT.md M4.3. ⚠️ **R-260 (2026-09-10): the monster-side rant wire (`Misc4`) never paid; the per-player container fallback this ruling names is the lane's recommended re-homing, `BL-R260-DEBT-1` (PENDING Will).**
...
- R-92 [2026-07-28] IMPLEMENTED b98 (feat/endless-hunt) [paraphrased; the ask was to "align with
  ...
  rant scroll to co-schedule. [⚠️ R-260, 2026-09-10: the `Misc4` mirror above never paid - not an engine slot; the Rite now rides `Misc1` row 3 on both Hunt records at a MEASURED 100%, IMPLEMENTED pending vet.] VERIFIED ALREADY-CORRECT, no change needed: `svc_toxeus_eoat_formula`
...
## R-250 [2026-08-11] IMPLEMENTED (branch `fix/toxeus-shroud`, module `tools/patches/enslaver_shroud.py`) - the ENSLAVER OF SOULS gets the demons' black shadow shroud

**WILL, VERBATIM:**

> "toxeus the murderer, devourer of souls we need to add the black shadow shroud around him, the same
> one that his demon summon guys have"

### THE NAME IS FUSED. THE DEMON CLAUSE RESOLVES IT, AND IT RESOLVES TO THE ENSLAVER.

No variant is called "Devourer of Souls": the phrase fuses **"Toxeus the Murderer, DEVOURER OF BLOOD"**
and **"Toxeus the Murderer, ENSLAVER OF SOULS"**. The only discriminating words in the sentence are
*"the same one that his demon summon guys have"*, so the target is decided by measuring, for each
variant, whether ITS summons carry a black shadow shroud. Measured on the shipped `build83` arz
(`44499f56`, the hash `docs/HANDOFF_LIVE_STATE.md` records as LIVE) - **and measured at the MESH layer,
which is where that shroud actually lives**:

| variant | its "demon summon guys" | do the summons carry a black shadow shroud? |
|---|---|---|
| **Enslaver of Souls** (`um_toxeus_enslaver_99`) | Enslaved Shadow Marauders (`um_enslaver_marauder_99`, `pets\enslaver_marauder_1..3`) - **`ShadowStalker.msh`** | **YES.** `CreateEntity{attach="SpecialHit01"; entity="Records\Effects\MonsterFX\ShadowStalker_Smoke.dbr"}` compiled into the mesh -> `Effects\MonsterFX\ShadowStalker_Smoke01.pfx`. Always on. Will confirmed it by eye (R-102 fourth amendment). |
| Devourer of Blood (`um_bloodtoxeus_99`) | Blood Demons (`drxcreatures\blooddemon\um_devourer_bloodspawn_99`) - `DRX\meshes\blooddemon01.msh` | **NO.** Its embedded FX are `FX_blood_CHEST/HANDS/HEAD_fx` - blood effects. It carries no `charFxPakRunningNames` and no shadow FX of any kind. |
| The Endless Hunt (`um_toxeus_hunt_99`) | Coursers (bloodhounds, not demons) | n/a. He himself wears `ShadowStalker.msh`, so he already HAS the embedded smoke. |

**This is the same sentence Will has now filed three times** - R-95 (2026-07-28: *"the same black shroud
smoke his summoned demons have"*), R-102 second amendment (*"that is still not implemented"*), and this
one. A prior round of this lane resolved it to the Devourer on the reasoning that the Enslaver "already
has a shroud in data, so ADD cannot mean him". That reasoning was wrong twice over: the Devourer's demons
have no shroud to copy, and what the Enslaver "has in data" does not render (below).

### WHY TWO WAVES SAID IT WAS DONE AND WILL KEPT SEEING NOTHING

Every prior wave worked in the `.dbr` FIELD layer. **The demons' shroud was never in a field.** It is
compiled into `ShadowStalker.msh`, and a mesh renders every frame - which is exactly why the marauders
smoke while standing still and the Enslaver does not. What he actually had:

1. `charFxPakRunningNames` -> `drxshadowcloakrunning_fx_pak`: renders **only while RUNNING** (R-95's
   finding). He is a caster who stands and casts.
2. `svc_enslaver_shroud` (b98/b102), a `Skill_BuffSelfToggled`: both his controllers shipped
   `BuffSelfBehavior = WhenEnemyIsSeen`, so it is **OFF out of combat** - i.e. off in precisely the
   situation Will inspects the summoned pet in (the R-102 screenshot).
3. Both of those channels pointed at `drxshadowcloakrunning_fx`, whose own
   `boneList = Bone_R_Weapon; Bone_L_Weapon` **pins the smoke to his two weapon bones**. Even when it
   played it came off his fists. b98's round 4 fixed that at the PAK layer while the EffectEntity
   underneath kept doing it - which is why "shape fixed" was true and still looked wrong.
4. `champion_mesh` (b102, R-102) moved him off `RevenantPoison.msh` to kill the green, onto
   `SkeletonGrayBlack01New.msh`, **which carries no embedded FX at all**. That fix was correct and is
   not being undone - but it removed his last always-on emitter, which is why this request arrives now.

### WHAT SHIPPED (all of it on monster-record fields + skill records; CRASH LAW respected)
...
Editing either in place to fix one champion's FX is the `genericbossorb_04` mistake (R-103's required
check) with an AI field instead of a loot field. Measured blast radius of `apply()` on the live arz:
**9 records** - 3 new (the EffectEntity + two controller clones), the pak (2 fields), the skill
(FileDescription), and **one field (`controller`) on each of the 4 roster surfaces**. Both donor
controllers verified still `WhenEnemyIsSeen` and not in `_modified`.

**THE AI FLIP IS PROVABLY VISUAL-ONLY, and gated as such.** `BuffSelfBehavior` governs `Skill_BuffSelf*`
skills only (`BuffAllyBehavior` and `DebuffEnemyBehavior` are separate fields), and the shroud is the
**only** `Skill_BuffSelf*` in either kit (monster slot 19, pets slot 13; his aura `character_speedall`
...
- **ALWAYS-ON:** every roster controller is SVC-owned AND at `WheneverPossible`; **SHARED-RECORD LAW:**
  no record outside the roster may carry one of our clones.
- Plus the inherited legs: visual-only (no payload, no tint), roster two-witness agreement, running
  channel preserved, and **CRASH LAW** (no `charFxPak*` on any `Skill_SpawnPet*` in either kit - the
  build28 trap).

### WHAT THIS RULING DOES **NOT** DECIDE

1. **How it READS in game.** Unseen. The particle is the one Will has looked at and called *"the proper
   black shroud"*, on the marauders, and it provably ships - but nobody has seen the Enslaver wearing it
   at his waist. `BL-R250-DEBT-1`.
2. **The Devourer of Blood, who has no shroud either** - `GoldenSkeleton01.msh` is FX-free, he carries no
   `charFxPakRunningNames`, and his only FX is R-7's `svc_black_poison` (two HAND emitters of the
   not-colour-confirmed `343_dark_smoke`). That is a real gap, but Will did not ask for it, his demons
...
An earlier version of R-250 named the Devourer of Blood as the target and shipped
`tools/patches/devourer_shroud.py`. **That module is deleted and this ruling supersedes it.** Two things
it got wrong are recorded so the failure mode is not repeated: (a) its own measurement already showed the
...
## R-253 [2026-08-14] IMPLEMENTED (branch `fix/boss-arena-spawn-and-polish`) - THE BOSS ARENA BOSS MUST SPAWN ON EVERY VISIT (he was quest-one-shot + player-level-gated); THE ARENA GETS A REWARD AND A FIRE RING
...
> **R-252**, which no branch claimed at the time, and ships `tools/gate_ruling_ids.py` so the next
> collision is caught mechanically instead of at merge time. **Cross-reference: the UBER/BOSS
> CHEST GENEROSITY ruling** (`fix/chest-generosity-shared-cause`) is semantically coupled
...
> again, renumber this section and its `BL-R253-DEBT-*` ids together.
>
> **INTEGRATION ADDENDUM (build97 ship, 2026-08-15) - RENUMBERED AGAIN, `R-252 -> R-253`, the
> THIRD consecutive collision.** By the time this lane reached integration, main had shipped a
> DIFFERENT `R-252` as build96 (`fix/toxeus-boss-equipment-and-soul`, itself an `R-251 -> R-252`
> renumber) and a DIFFERENT `R-251` as build95 (`fix/chest-generosity-shared-cause`, an
> `R-250 -> R-251` renumber). The shipped number always wins, so this ruling and its debts are
> **`R-253` / `BL-R253-DEBT-1..8`** everywhere. **No content changed** - the renumber is textual
> only. Two consequences for the prose below, both corrected in place:
> **(1)** every forward-looking reference to "R-250" as the chest-generosity ruling now reads
> **R-251**, and it **HAS LANDED** (build95) - `_svc_standardize_boss_chests` was REPLACED, not
> converted, so gate check C8 retires on the no-pass-at-all branch and the arena chest keeps its
> own table on today's main with no roster row needed. The roster row was deliberately NOT added:
> the end state C8/C5 assert is already true without it, and this ship changes nothing it does not
> have to. **(2)** R-251 also restored the 27 `svc_*hoard_loot_0N` volumes to the authored
> `(3+(1.8*numberOfPlayers))*2.4 / *2.8`, and the arena family is inside that pass's name-shape
> scope, so the trimmed `*0.2188/*0.25` figures quoted in the REWARD paragraph and in
> `BL-R253-DEBT-2` are the build92-era measurement, not this build's. **Check C9 VOLUME TRUTH is
> the authority** - it asserts the arena hoard ships at whatever its named peer family
> (`tantalus`) ships at, tier for tier, and it is run against the BUILT arz of this deploy.
> `BL-R253-DEBT-7` (should the arena hoard be exempted from R-240's trim) is therefore
> **narrowed, not closed**: R-251 already moved the whole family up together.
...
- **"Needs more work" part (b), scoped:**
  - **REWARD.** The arena guarded NOTHING (b43 RCA sec 6 item 5: zero chest/loot strings in the
    ...
    **THE NUMBERS, RE-MEASURED (vet round 2 - the round-1 figures in this paragraph were
    ...
    chest", "nerfed too much") and is the subject of **R-251**
    (`fix/chest-generosity-shared-cause`). A chest created THIS wave does not ship into a defect
    reported five times the same day. **R-251 LANDED as build95** and that pass stopped repointing
    (it became a WIRING pass, `tools/svc_uber_hoards.py`: each chest -> its own
    `svc_<family>hoard_loot_<tier>`, scope derived from the NAME SHAPE so a future family is covered
    the moment it is named). Because it REPLACED `_svc_standardize_boss_chests` outright rather than
    converting it, check C8 retires on its no-pass-at-all branch and the family-roster row
    (`'svc_aithonhoard': ('55-57','63-65','63-65')`, from his charLevel `[55,69,75]`) is **not
    needed and deliberately not added** - the arena chest already opens its own table, which is the
    only end state C5/C8 assert. **Round-3 correction:** C8 first
    retired on the function NAME being absent, which covers a delete or a rename but NOT an in-place
    conversion; had R-251 converted the pass in place and the integrator followed the INTEGRATION
...
- `tools/debug/gate_uber_placement.py` learns the arena (BOSS_MARKER for the
  ...
  **Vet round 2 addendum - the fix also HID something, so the gate now says it.** Choosing the
  ...
  portal this oracle does not model, and the finding belongs to the Aniketos lane (`BL-R253-DEBT-8`).
...
- `tools/debug/gate_landing_clearance.py` learns the arena entities (`--placements r253`,
  ...
  **The error was self-revealing and the gate caught it:** with the round-2 value the row measured
  ...
  output (the marker the R-253 proxy is placed at). `G-NPC-LANDING-SEP` now **PASSES** at `+0.70u`.
  ...
  `BL-R253-DEBT-6` stays OPEN for the remaining rows.
...
**DEBT:**
- `BL-R253-DEBT-1` (WILL / in-game): confirm the boss is present on a SECOND and THIRD visit (the
  actual bug), and that only ONE Aithon spawns (the double-spawn a mis-coupled Levels/Quests deploy
  would produce).
- `BL-R253-DEBT-2` (WILL / in-game): confirm the Ember-Crowned Hoard chest appears and unlocks on
  ...
  **CORRECTED (vet round 2): this is NOT a look at an ungutted chest**, and round 1 said it was.
  ...
  right"; the ungutted question is R-251's. **R-251 landed as build95 and DID move the shared hoard
  volume constants** (the 27 `svc_*hoard_loot_0N` back to the authored `*2.4/*2.8`), and this chest
  moves with them by the same name-shape scope (same `_svc_build_dedicated_hoard` recipe) - gate
  check C9 reds if it ever stops moving with them, and is run against this deploy's BUILT arz.
- `BL-R253-DEBT-7` (WILL / DESIGN DECISION, opened by the vet round-2 correction): **should the
  ...
  fails loud on a half-landed one. **NARROWED by R-251 (build95)**, which restored the whole hoard
  family's authored volume - the question is now "is a sibling-volume hoard enough for the arena",
  not "is it gutted".
- `BL-R253-DEBT-3` (in-game, visual): confirm the six `pit_fx02` flames read as a fire ring and not
...
- `BL-R253-DEBT-4` (canonical/Steam reachability): the Boss Arena travel leg is TESTHUB-only
...
- `BL-R253-DEBT-6` (NOT this lane's, found by running its gates - travel lane): the landing gate's
...
- `BL-R253-DEBT-8` (NOT this lane's - Aniketos lane; surfaced by this lane's ISLAND advisory):
...
- `BL-R253-DEBT-5` (integration): the gate's `--arz` / `--quests` / `--map` halves have been proved
...
### DEBTS
- `BL-R259-DEBT-1` (**P1, WILL**): the existing ~300 GB at `<repo>\backups\deployed\` is NOT moved or deleted by this lane (other agents were reading `backups\` at the time, and a recursive delete on that tree is exactly the operation the 2026-09-09 entry restricts). The script no longer writes there. Will's call: delete it, or move the newest few snapshots to the NAS root first. Either way, enumerate reparse points over it before any recursive delete. [MOVE OUTCOME under Will's 2026-09-10 rule, recorded at the R-260 integration 2026-09-11 - DISCHARGED: `backups\{deployed,dev_deploy,characters}` = 92,504 files / 303,439,495,345 B moved to `Z:\Computer Backup\tqit_soulvizier_classic\archive_pre_R259\` (verified before the local delete: list-only diff 0, file count + total bytes equal on both sides, 15 spot md5 equal), the local copies deleted only after those gates passed; C: +282.85 GiB; `backups\` now holds only `game_dll`.]
...
- **build102 status (2026-09-10, ship operator):** `BL-R259-DEBT-2` STAYS OPEN. build102 went to DEV by the targeted coupled-copy runbook into `CustomMaps\SoulvizierClassicDEV`, not through `scripts/deploy_to_custommaps.ps1`, whose hardcoded target `CustomMaps\SoulvizierClassic` does not exist on this machine (and the DEV runbook forbids running it: a second mod, or the TESTHUB pair clobbered); the R-259 snapshot + keep-5 rotation path was therefore not exercised, and the standing evidence is still the sandbox harness (69/69). The integration note above claiming discharge by this deploy is superseded (`docs/MISTAKES.md` 2026-09-10).

---

## R-260 [2026-09-10] IMPLEMENTED (branch `fix/misc4-audit`, resumed in the main tree after the first implementer was killed by a session limit with ~16 files uncommitted - see `docs/MISTAKES.md` 2026-09-10; helper `tools/svc_loot_slots.py` + `apply_svc_patches._svc_guarantee_unique`, registry module `tools/patches/phantom_loot_slots.py`, gates `tools/gate_no_phantom_loot_slots.py` / `tools/gate_guaranteed_drops_measured.py`; [integration 2026-09-11: the independent vet PASSED, its det-2x work-layout cold build is THE ARTIFACT OF RECORD (section 8), MERGED to `main` as `b2294ac` (`--no-ff`); SHIPPED as build103 2026-09-11 to DEV + Steam, tag `build103-ship`] was: PENDING the independent vet and the work-layout cold build) - THE `Misc4` AUDIT: 32 DROPS RODE A SLOT THE ENGINE DOES NOT DECLARE; 31 ARE RE-HOMED ONTO DECLARED SLOTS AT MEASURED RATES, THE DEVOURER'S RANT+RITE CHANNEL IS WITHHELD FOR WILL'S CALL

**ORIGIN:** `BL-R258-DEBT-2` (widened at the build102 integration to all 32 carriers, registered as the next P1 lane). Standing autonomy; no new Will words. Will's words that GOVERN the content moved here, verbatim from their own rulings: R-13 *"also make it so if you kill either toxeus the devourer of blood of toxeus the enslaver of souls you also get the formula"*; R-15 *"you are good to ship the rant scroll"* (the Devourer's rant, wired by Will's own 2026-07-11 decision as *"guaranteed one-per-player Misc4 @100%, duplicates on repeat kills accepted"*, R-17); R-92 (the Rite off the Hunt); R-231 (the Golden Bough off the terminal); the build36 uber content wave (Lethe's Draught, the Mask of the Waking Dread) and the four animal-relic groups at their documented 7-10%. **None of that content has ever reached a player's hands from a kill**, and this ruling says why and what now.

### 1. THE DEFECT, FROM THE ENGINE BYTES (vet-proven on the build102 lane, re-measured here)

`<game>\Toolset\Templates.arc -> templates\templatebase\characterloot.tpl` (included by `monster.tpl`) declares exactly **ELEVEN** equip/loot slot groups: `Head Torso Forearm LowerBody LeftHand RightHand Finger1 Finger2 Misc1 Misc2 Misc3`, each with `chanceToEquip<S>` (real: the % the SLOT rolls at all), `chanceToEquip<S>Item1..6` (int: RELATIVE WEIGHTS among the slot's rows, one row picked per roll) and `loot<S>Item1..6` (file_dbr arrays, one entry per difficulty). `Misc4` / `Misc5` / `Misc6` / `Neck` occur in **0 of 566 templates and 0 of 74,013 base-AE records** (0 in SV 0.98i / 0.9 / 0.4.1 too). A field with one of those names is dead data the engine never reads. `dropItems` on `monster.tpl` drops whatever the generator equipped.

So the MEASURED chance that a monster drops a given table is `chanceToEquip<S> x weight(row) / sum(weights of the slot's rows)`, summed over every declared (slot, row) that names it - and for a table that sits only on `lootMisc4Item1` that number is **0.0000%**, whatever `chanceToEquipMisc4` says.

### 2. THE 32 (+3), READ OFF THE SHIPPED build102 ARZ `7dad9a8ad377ff9a65d17c144a57dde3`

The dump of all eleven slot groups of every carrier is in the lane's scratch record and was re-derived by the independent measurement below; the rows that matter:

| carrier(s) | item on `Misc4` | declared | build102 state of the Misc slots (chance: rows) |
|---|---|---|---|
| `um_bloodtoxeus_99` (the Devourer) | `svc_devourer_misc4_master` x3 @100 | rant + rite | Misc1 100: potions muted 0/0, R-258 soul chute row 3 @100 (FROZEN, `BL-R258-DEBT-1`); Misc2 18: relics 88 / formulae 12; Misc3 50: amulets 5000 / 13; Finger2 100 (FROZEN) |
| `um_toxeus_enslaver_99` | `svc_rite_guaranteed` x3 @100 | the EoAT Rite | Misc1 100: health 80 / energy 20; Misc2 18; Misc3 50 |
| `um_toxeus_hunt_99`, `um_toxeus_hunt_l_99` | `svc_rite_guaranteed` x3 @100 | the EoAT Rite | Misc1 100: health 80 / energy 20 (the donor ships it at 0; R-252 switches it to the family 100); Misc2 18; Misc3 50 |
| `um_charonform2_ferryman_99` (the terminal) | `svc_goldenbough_{n,e,l}` @100 | the Golden Bough | Misc1 0 / Misc2 0 / Misc3 0 (CORRECTION-20 mute over inherited act-4 rows: unique 100+0; potions 80/20/10+0; relics 25 / formulae 5 / misc 75) |
| `um_mnemophage_99` | `lethesdraught` x3 @100 | Lethe's Draught | Misc1 1.6: unique 100; Misc2 100: health 20 / mana 80 / act-4 misc 10; Misc3 75: relics 25 / formulae 5 / misc 75 |
| `um_ephialtes_99` | `svc_maskofdread_{n,e,l}` @100 | the Mask of the Waking Dread | Misc1 1.6; Misc2 100: health 100 / mana 20 / misc 10; Misc3 75 |
| `ar_archer_01..04` | `svc_revelersruse` 01/02/03 @7 | the Reveler's Ruse | Misc1 3.0 potions; Misc2 0.6 relics/formulae; **Misc3 0.0** over amulet rows 5000 / 4 (dormant) |
| `ar_archer_05..06` | same @7 | | Misc3 **0.5** over amulet rows 5000 / 4 (live) |
| `um_sepulchralwyrm_31/34/37/40` + the cloned `svc_frostwyrm_31/34/37/40` | `svc_sepulchralscale` 01/02/03 @7 | the Sepulchral Scale | Misc1 10 potions; Misc2 1.5 relics; **Misc3 0.0** over an obsidian row @100 (dormant) |
| the 9 Sileni (`01_bladedancer_35/36/37`, `02_spearrunner_37/38/39`, `03_ravager_38/39/40`) | `svc_sanguinetithe` 01/02/03 @7 | the Sanguine Tithe | Misc1 3-6 potions; Misc2 0.5-1.8 relics/formulae; Misc3 **0.5** (six) / **1.0** (ravagers) over amulet rows 5000 / 4 (live) |
| `em_brute_43`, `em_brute_45` | `svc_erebanheartstone` 01/02/03 @10 | the Ereban Heartstone | Misc1 8 potions; Misc2 2 relics/formulae; Misc3 **2.0** over amulet rows 5000 / 8 (live) |
| `eoat_disciple_1/2/3` (pets, `dropItems 0`) | `chanceToEquipNeck` = 0.0 | nothing | dead data inherited through the pet chain; no `lootNeck*` anywhere |

None of the 32 had an EMPTY Misc slot. Every one of the 25 relic sources and the Enslaver, both Hunts, the Mnemophage and Ephialtes carry live hand-designed rolls on all three Misc slots; the terminal, the wyrms and four archers carry a DORMANT Misc3 (chance 0 over rows that never rolled).

### 3. ROOT CAUSE, BY `git blame`

`tools/apply_svc_patches.py` `_svc_guarantee_unique`, commit **`79de1d7` (build36 CONTENT WAVE C1-C7, 2026-07-11 15:46, author wtrevena = the build36 agent lane on Will's account)**: `for n in (4, 5, 6, 3): if not db.get_field_value(monster, 'lootMisc%dItem1' % n): slot = n; break` - it looks for a FREE `lootMisc4Item1` first, and `Misc4` is free on every record in existence, so it never reached the real `Misc3`. Its comment called `lootMisc4` "the exact proven `_create_soul` mechanism" (which is `Finger2`, a declared slot). Four writers then copied the idiom directly: Group G's wyrm loop `(3, 4, 2, 1)` (which tested `lootMisc3Item1` for emptiness, found the dormant obsidian row, and fell through to `Misc4`), A3 Sanguine Tithe and C5 Ereban Heartstone (`dcb8a36` / `79de1d7`, 2026-07-11), `toxeus_suite` PART B (`cbe5c7c` build37, 2026-07-12), `turtleshell_relics` (`afd762b`, 2026-07-15, whose docstring said "`lootMisc4` is verified genuinely free on every body in the family"), `toxeus_endofallthings` (`32daf23` b83, 2026-07-16: "proven Misc-slot drop; Misc4 is used by 29 monsters" - the 29 were this mod's own writes) and `toxeus_hunt_encounter` (`7afaf3a` b98, 2026-07-28). Every verify() in those modules asserted the FIELD (`chanceToEquipMisc4 == 100`, `lootMisc4Item1 == the table`), never the DELIVERY, so eleven builds of green gates proved nothing. R-247.6a's "Will's kill DID drop it" was the inference R-258 retired.

### 4. THE HELPER IS FIXED, AND THERE IS ONE IMPLEMENTATION

`tools/svc_loot_slots.py` is the one model of the eleven slots, shared by `apply_svc_patches._svc_guarantee_unique` (now a thin wrapper that also switches `dropItems` on when it is off, dtype-free), the registry module, and both gates - so the number the build prints and the number a gate re-derives come from the same arithmetic. **Policy of `wire_misc_drop(db, rec, tables, pct, slot=None, why=None, share=False)`**, in order, every outcome printed as a disposition and appended to a per-process LEDGER:

| tier | condition | what is written | displaces |
|---|---|---|---|
| **A EMPTY** | a Misc slot with chance 0 and six empty rows, order Misc3 > Misc2 > Misc1 | chance = intended %, row 1 = the table @100 | nothing |
| **B DORMANT** | a Misc slot with chance 0 whose rows still carry tables (they never rolled) | chance = intended %, the table on the first free row @100, every inherited row MUTED BY WEIGHT (values kept - retirement protocol) | nothing observable: the slot paid 0 before |
| **C TAKEOVER** | intended 100 ONLY, on a LIVE slot already at chance 100 that the caller PINS with a written `why` (the R-258 pattern) | the table on the first free row @100, the live rows muted by weight; the slot chance is never moved | the slot's previous pick, documented per record below (`BL-R260-DEBT-2`) |
| **D SHARE** | intended < 100 ONLY, and only when the caller opts in (`share=True`), on a live slot where the compensating weight is an integer | chance raised by exactly the intended %; the new row gets weight `W x pct / chance` | NOTHING: with `c' = c + p` and `w_new = W p / c`, every existing row's absolute % is `c' w_i / (W + w_new) = c w_i / W`, byte-identical, and the new row measures exactly `p`. The gate proves both halves per record |
| **X FAIL** | anything else | `SystemExit` naming the record and printing its full slot map | - |

Idempotent (a table already riding a real Misc row at the intended measured % is 'kept'; any other pre-existing state is a fail-loud drift); strips every phantom `Misc4..9*` / `Neck*` field it meets; never passes a dtype to `set_field` (the old `dropItems, 1, INT` write flipped a BOOL field - `BL-R256-DEBT-7`). The lane brief's policy named tiers A, C and X only. **Tiers B and D are this lane's additions and they are load-bearing:** without B the terminal, the wyrms and four archers would FAIL LOUD on a slot that pays nothing; without D every one of the 25 relic sources would FAIL LOUD, because not one of them has an empty or dormant Misc slot, and "re-wire each relic at its documented 7-10%" (the brief's other requirement) would be unsatisfiable. D was chosen over "take a slot and skew its weights" precisely because it moves no existing row's absolute rate. Both are registered for Will's ratification as `BL-R260-DEBT-4`.

### 5. THE REWIRE, PER RECORD (intended vs MEASURED, the measurement re-derived from the final bytes by the gate AND by an independent script that shares no code with it)

| item | record(s) | old | new slot / row / tier | intended | measured | displaced |
|---|---|---|---|---|---|---|
| Rite of the Undivided (R-13) | `um_toxeus_enslaver_99` | Misc4 @100 | **Misc1 row 3, C** (pinned; `toxeus_endofallthings`) | 100 | 100.0000 | his one potion per kill (health 80 / energy 20 muted) |
| Rite of the Undivided (R-92) | `um_toxeus_hunt_99` | Misc4 @100 | **Misc1 row 3** (pinned; dormant at the encounter stage so tier B sets the 100 that R-252 later re-asserts; `toxeus_hunt_encounter`) | 100 | 100.0000 | the potion the family shape would have paid (health 80 / energy 20 muted) |
| Rite of the Undivided (R-92, endless) | `um_toxeus_hunt_l_99` | Misc4 @100 | **Misc1 row 3**, inherited by `toxeus_hunt_endless`'s one-field clone and re-measured through the helper ('kept') | 100 | 100.0000 | as the Hunt |
| the Golden Bough (R-231) | `um_charonform2_ferryman_99` | Misc4 @100 | **Misc3 row 4, B** (pinned; `charon_rework` mutes Misc1/2/3 to 0 first, so the slot is dormant; the inherited act-4 relic/formula/misc rows stay named at weight 0) | 100 | 100.0000 | nothing - those rows paid 0 under CORRECTION-20 and still do |
| Lethe's Draught | `um_mnemophage_99` | Misc4 @100 | **Misc2 row 4, C** (pinned) | 100 | 100.0000 | the shell's potion per kill (health 20 / mana 80 / act-4 misc 10 muted) |
| the Mask of the Waking Dread | `um_ephialtes_99` | Misc4 @100 | **Misc2 row 4, C** (pinned) | 100 | 100.0000 | his potion per kill (health 100 / mana 20 / act-4 misc 10 muted) |
| the Reveler's Ruse | `ar_archer_01..04` | Misc4 @7 | **Misc3 row 2, B** (amulet rows 5000 / 4 muted) | 7 | 7.0000 | nothing (chance was 0) |
| the Reveler's Ruse | `ar_archer_05..06` | Misc4 @7 | **Misc3 row 2, D**: chance 0.5 -> 7.5, charm weight 70056 beside amulets 5000 / 4 | 7 | 7.0000 | nothing: the amulet rows still pay 0.4996% / 0.0004% |
| the Sepulchral Scale | `um_sepulchralwyrm_31/34/37/40` + `svc_frostwyrm_31/34/37/40` (clones, 'kept') | Misc4 @7 | **Misc3 row 2, B** (obsidian row @100 muted) | 7 | 7.0000 | nothing (chance was 0) |
| the Sanguine Tithe | the 9 Sileni | Misc4 @7 | **Misc3 row 2, D**: chance 0.5 -> 7.5 (six) / 1.0 -> 8.0 (ravagers), charm weight 70056 / 35028 beside amulets 5000 / 4 | 7 | 7.0000 | nothing: amulet rows keep their exact 0.5% / 1.0% |
| the Ereban Heartstone | `em_brute_43`, `em_brute_45` | Misc4 @10 | **Misc3 row 2, D**: chance 2.0 -> 12.0, charm weight 25040 beside amulets 5000 / 8 | 10 | 10.0000 | nothing: amulet rows keep their exact 2.0% |
| (rant + rite master) | `um_bloodtoxeus_99` | Misc4 @100 | **WITHHELD** - section 6 | 100 | 0 (unchanged, now honest) | - |
| (nothing) | `eoat_disciple_1/2/3` | `chanceToEquipNeck` 0.0 | stripped by the registry sweep | - | - | - |

**THE SHARED-RECORD LAW, PER RELIC FAMILY (the vet's fix; the law says a shared record is changed for all, deliberately, and said so - this is the saying-so):** the Reveler's Ruse carriers `ar_archer_01..06` are referenced by the satyr pools `satyr_02_archer*` / `satyr_03_archer*` / `satyr_02_fodder*` / `satyr_03_fodder*` and by `centaur_02_satyrarcher01`, so every satyr archer those pools spawn now rolls the Ruse at a measured 7%, deliberately. The Sepulchral Scale carriers (`um_sepulchralwyrm_31/34/37/40` + the `svc_frostwyrm_31/34/37/40` clones) are referenced by `firesprite_0N_general06`, by the horde proxies `svc_wyrmhorde_01/02/03`, by `svc_broodnest_hatch`, by `svc_neferkha_court` and by `svc_broodmother_pool` / `q_yard_broodmother`, so every wyrm those pools, proxies and hatch paths spawn now rolls the Scale at a measured 7%, deliberately. The Sanguine Tithe carriers (the 9 Sileni) are referenced by `abom_dancer_champcnc` / `abom_dancer_spear_mix` / `abom_spear_champcnc` / `abom_ravager_lone` and by the `pandorasbox_summonskill` `spawnObjects`, so every Silenus those pools and that summon skill spawn now rolls the Tithe at a measured 7%, deliberately. The Ereban Heartstone carriers `em_brute_43` / `em_brute_45` are referenced by `brute_01`, so every brute that pool spawns now rolls the Heartstone at a measured 10%, deliberately. No referencing pool, proxy or summon skill was edited; the change is on the shared creature records and reaches every spawn of them.

`dropItems` is on for all 31 carriers (asserted by the gate). Every 'guaranteed' record has the item as the slot's ONLY live row (the gate reds on any competing weight); every shared slot pins its pre-existing rows' weights and absolute rates (the gate reds on skew).

### 6. THE DEVOURER: WHAT `svc_devourer_misc4_master` CARRIES, AND WHY IT IS WITHHELD

**What it carries (bytes):** `Class = LootMasterTable`, `lootName1 = records\item\loottables\svc\toxeus_rant_perplayer.dbr` @100 (the FixedItemLoot per-player table -> `toxeus_rant_item` -> `svc_toxeus_rant`, R-15's Murderer's Screed), `lootName2 = svc_rite_guaranteed` @100 (R-13's Rite formula). **It is NOT another route to the soul** - the soul rides `Finger2` (R-243) and the R-258 `Misc1` chute, nothing else names `blood_toxeus_soul_*` from this table - so the brief's "retire a third soul channel" branch does not apply; the content is the rant scroll and the formula, and the correct disposition is to RELOCATE it at its intended 100%.

**Why it cannot be relocated by this lane:** he has NO empty and NO dormant Misc slot. `Misc1` (potions muted, the R-258 soul chute on row 3) and `Finger2` are FROZEN byte-identical to build102 while Will counts souls (`BL-R258-DEBT-1`, the diagnostic in flight) - the independent measurement proves both slot groups unchanged, dtype and value. `Misc2` is his live 18% relic/formula roll and `Misc3` his live 50% amulet roll (R-252's family shape); the only way either could carry a 100% guarantee is a takeover that raises the slot to 100 and mutes the hand-designed rows - a balance change on R-252's record that no ruling authorises, and tier C refuses it by construction (it only takes over a slot ALREADY at 100). `Head` is his one chance-0 slot, but it is class-governed (R-252) and a formula there rests on the "a mis-class roll still drops" premise R-258 retired (`BL-R258-DEBT-4`). So the channel is **WITHHELD**: the dead `Misc4` fields are stripped (`toxeus_endofallthings`), the master, the rant tables, the item and its three tags stay authored and referenced by no slot (retirement protocol: records stay), and the gate holds three things - no phantom field on him, the master on no declared slot, the master not deleted.

**`BL-R258-DEBT-5` SETTLED FROM THE BYTES:** `toxeus_endofallthings` said a `LootMasterTable` "rolls each child independently at weight-as-percent, so both always drop"; R-252 said the master is "a 50/50". The build102 arz holds **1,970** `LootMasterTable` records whose `lootWeight*` sum to 200 (656 of them), 1004 (386), 300 (151), 400 (150), 100 (142), 140, 600, 150, 1000, 2000, ... - the base game's own `sword_n01`-style master tables are the classic common@1000 + unique@4 pick, and weights summing to 2000 cannot be percentages. **Weights are RELATIVE, one child per pick; the master is a 50/50 between the rant and the Rite.** So even on a declared slot R-13's "you also get the formula" would have needed TWO rows, not one master - which is the second reason the re-homing is Will's design call, not a mechanical move.

**OPTIONS FOR WILL (`BL-R260-DEBT-1`), costed:** (a) after `BL-R258-DEBT-1` collapses the soul to one channel, the freed `Misc1` carries the rant @100 (one slot picks ONE row per roll, so the Rite cannot ride beside it there; two guaranteed items need two slots or a container); (b) `Misc3` takeover (chance 50 -> 100, amulet rows muted): the rant guaranteed, the Rite still 100% off the Enslaver and both Hunts, the amulet roll lost; (c) `Misc2` takeover (18 -> 100, relic/formula rows muted): same shape, the relic roll lost; (d) the per-player CONTAINER fallback R-17 already names (a corpse/chest with `loottable = toxeus_rant_perplayer` spawned on death) - the only shape that honours "one-per-player" (the launch-gated numSpawn edge of `toxeus_suite` PART B was never proven on a MONSTER slot) and the only one that adds nothing to an equip slot, at the cost of one new record + one death skill; (e) drop the rant from the kill and keep the Rite on the Enslaver/Hunts only (R-13 says "either ... or", both sources coexist). The lane recommends (d) for the rant and nothing further for the Rite.

### 7. GATES + ANTI-INERT

- `tools/gate_no_phantom_loot_slots.py <arz>` (arm A): zero records carry any `chanceToEquipMisc4..9*` / `lootMisc4..9*` / `chanceToEquipNeck*` / `lootNeck*` field. **On build102 `7dad9a8a` it EXITS 1 naming 35 carriers** (`--expect-fail` is the anti-inert control); `--negtest` plants five phantom fields on a green arz and proves RED each time, restoration green.
- `tools/gate_guaranteed_drops_measured.py <arz>` (arm B + F): every roster record delivers its item at the intended MEASURED % on the pinned real slot/row, with the tier's side conditions (only live row for a guarantee; pinned base rows and un-skewed absolute rates for a share), `dropItems` on, no phantom field; the withheld Devourer channel stays withheld; his `dropItems`, `Finger2` pin and R-258 `Misc1` chute are exactly R-258's. **`--control <build102 arz>` demands all 31 measure 0.0000% there and the gate RED** (the anti-inert control); `--negtest` = 7 plants on the real arz + the module's 29-check stub battery (21 planted defects across arms A/B/F, 3 positive controls, 5 writer proofs incl. "refuses a 100% guarantee on all-live slots without a pin", "refuses to share without opt-in", "the share keeps the live row at exactly 5.0000%", idempotent second call, phantom stripped on re-apply).
- In-build: `phantom_loot_slots.apply()` = the mod-wide sweep (registered after `devourer_soul_delivery`, before the no-op `visuals`); `verify()` = arms A + B + F over the FINAL db plus the ledger cross-check that every roster record was wired through the helper THIS build (a caller that silently stops calling it, or a clone that stops inheriting, reds here). Both artifact gates run in `build_svc_database.py` after the voyage-cap gate, on the WRITTEN arz. The eight caller modules' own verify() hooks (`charon_rework`, `r247_boss_forms`, `toxeus_boss_equipment` E3, `toxeus_endofallthings`, `toxeus_hunt_encounter`, `turtleshell_relics`) now assert the MEASURED share, never a field name; their negtests carry the R-260 plants ("a potion row un-muted beside the rite", "a phantom Misc4 grows back", "the Bough diluted by an un-muted inherited row").
- `ArzDatabase.remove_field` (`tools/arz_patcher.py`) is new: the one sanctioned way to delete a field (matches the `###`-suffixed key form, marks the record modified, fires the mutation listeners). Deleting from the `get_fields()` dict by hand skips both.

### 8. BUILD EVIDENCE (this lane, 2026-09-10; a SCRATCH layout, NOT the artifact of record)

`py tools/build_svc_database.py <sv098i> <sv09> <sv041> <scratch>\r260_build\Database\SoulvizierClassic.arz <base-game arz>` under `SVC_NO_CACHE=1 PYTHONHASHSEED=0 SVC_RELEASE_DROPS=1 SVC_NO_STAGE_BASELINE=1` (deliberately NOT `SVC_REQUIRE_GATES=1`: the scratch layout has no `Resources` beside the output, so the A9 render-chain, the F2 summons contracts and `enslaver_shroud`'s M2 rig precondition report UNPROVEN and skip loudly - `BL-R260-DEBT-5`; `local\` is read-only for this lane, hence no stage baseline), tree = checkpoint `4f86dab` plus docstring / print-string edits that no arz byte depends on: **exit 0**, arz **`eca50cdf9d2afbf4c1c08c66a24386ba`, 55,631,702 B, 51,355 records** (-141 B / +0 records over build102 `7dad9a8a`); every other gate in the battery green; `patches.selfcheck()` OK - 71 modules, order `2996f8ff`. The FIRST build of the recovered tree (checkpoint `1ffd0ec` + the review fixes) REDDED at `charon_rework.verify` - *"THE GOLDEN BOUGH is NOT wired ... no lootMisc*Item1 carries the 3 amulet tiers"* - a check that walked `lootMisc3..6Item1` (row 1 only, three of the four slots nonexistent) and had passed eleven builds on a field the engine never reads; it now measures the Bough over every declared slot and row, and the rebuild is the green one above.

| gate | result |
|---|---|
| in-build `phantom_loot_slots.verify` | **OK**: 0 phantom carriers after the sweep (3 disciple `chanceToEquipNeck` stripped); 31 roster records at their MEASURED rate (13 dormant-slot, 5 live-100 takeover, 13 rate-preserving share); the Devourer withheld with no phantom field; ledger cross-check 31 of 31 wired through the helper this build (the 4 frost twins and the endless Hunt as 'kept' after their clones) |
| `gate_no_phantom_loot_slots` built arz / build102 `--expect-fail` / `--negtest` | **PASS** 0 of 51,355 / **ANTI-INERT CONTROL OK** (RED, 35 carriers) / **PASS** 5 of 5 plants caught, restoration green |
| `gate_guaranteed_drops_measured` built arz / `--control` build102 / `--negtest` | **PASS** 31 of 31 / **ANTI-INERT CONTROL OK** (31 of 31 measure 0.0000% and ride no declared slot on build102; the gate is RED there, 94 problems) / **PASS** 7 plants on the real arz + 29 of 29 stub checks |
| `phantom_loot_slots --selftest` / `--negtest` | **PASS** 14 checks / **29 of 29** |
| INDEPENDENT re-measurement (`measure_r260.py`: raw fields, `chance x weight / sum(weights)` over the eleven declared slots, no code shared with the helper or the gates) | **31 of 31 at the intended %** (every one of them 0.0000% on build102); the Devourer's `chanceToEquipFinger2*` (7 fields), `lootFinger2*` (1), `chanceToEquipMisc1*` (7), `lootMisc1*` (3) **BYTE-IDENTICAL to build102, dtype and values**; `dropItems` 1 -> 1; his phantom trio gone; the master on no declared slot; `Finger2` soul 100% and the R-258 `Misc1` chute 100%, unchanged |
| FULL RECORD DIFF, built arz vs build102 | **ADDED 0 / REMOVED 0 / CHANGED 35, ZERO unattributed**: the 32 carriers (each loses `chanceToEquipMisc4` + `chanceToEquipMisc4Item1` + `lootMisc4Item1`; the 31 re-homed each gain one `loot<S>Item<r>` array and its weight; the dormant / takeover ones mute two or three inherited weights; the shared ones raise one slot chance by exactly the intended %) + the 3 disciples (`chanceToEquipNeck` gone). The Devourer's diff is exactly his three dead fields. `dropItems` untouched on every record (the wrapper writes it only when off) |
| `gate_ruling_ids --vs main --branches` | **PASS** at the completing commit (R-260 defined once; no other branch claims it) |

**THE ARTIFACT OF RECORD (integration 2026-09-11; the independent vet's own det-2x WORK-LAYOUT cold builds; `BL-R260-DEBT-5` DISCHARGED):** `PYTHONIOENCODING=utf-8 PYTHONHASHSEED=0 SVC_NO_CACHE=1 SVC_RELEASE_DROPS=1 SVC_REQUIRE_GATES=1 SVC_NO_STAGE_BASELINE=1 py tools/build_svc_database.py upstream/soulvizier_098i/Database/database.arz upstream/soulvizier_0.9/Database/database.arz upstream/soulvizier_041/Database/database.arz work/SoulvizierClassic/Database/SoulvizierClassic.arz <base-game arz>`, run twice cold in the WORK layout (`Resources` beside the output): both runs produced arz **`eca50cdf9d2afbf4c1c08c66a24386ba`, 55,631,702 B, 51,355 records** - byte-identical to each other (det-2x) AND byte-identical to the implementer's scratch build above, so the scratch evidence transfers whole. The three gates the scratch layout could not prove - the A9 render-chain, `enslaver_shroud`'s M2 rig precondition and the F2 summons contracts - **PASS** under `SVC_REQUIRE_GATES=1`; the contract suite on it reports **0 P0 / 0 P1**; the record-diff against build102 `7dad9a8a` re-run on it = **ADDED 0 / REMOVED 0 / CHANGED 35** (the 32 carriers each lose the `Misc4` trio; the 3 disciples lose `chanceToEquipNeck`), **0 unattributed** - exactly the expectation DEBT-5 wrote down; `Text.arc 1be898a0` and `Creatures.arc d65d92a3` UNCHANGED (arz-only by construction); the Devourer's `um_bloodtoxeus_99` `Finger2*` and `Misc1*` fields byte-identical to build102 (the `BL-R258-DEBT-1` freeze held under the work layout too). After measuring, the vet restored `work\SoulvizierClassic\Database\SoulvizierClassic.arz` to the build102 arz `7dad9a8a` (55,631,843 B; md5 re-verified at integration), so the staged work tree is still build102 until the ship lane restages `eca50cdf`; the R-260 artifact of record is `eca50cdf`, reproducible by the command above. Merged to `main` as `b2294ac` (`--no-ff`); post-merge `patches.selfcheck()` OK - 71 modules, order `2996f8ff`; `gate_ruling_ids --vs origin/main` PASS (56 rulings, highest R-260).

### 9. WHAT THIS RULING DOES NOT TOUCH (asserted, never written)

`chanceToEquipFinger2` / `lootFinger2Item*` anywhere (R-243); the Devourer's `Misc1` (R-258, frozen under `BL-R258-DEBT-1`); any slot chance on a shared slot other than the exact `+ intended %` of a tier-D share; `treasureProxyName` on any record (R-99); the three amulet / three mask / three Bough item records and every loot table (all still authored where they were); the shared potion / amulet / relic tables themselves (only their WEIGHT on the named rows moves, and only where documented above).

### 10. WHAT IS CLAIMED AND WHAT IS NOT - the DONE-means-DONE line

**CLAIMED, with byte proof + two gates + an independent re-measurement:** the database now routes 31 of the 32 drops through a template-declared slot at the published rate, and no record in the mod carries an undeclared slot variable. **NOT CLAIMED:** that anyone has SEEN one of these items fall (`BL-R260-DEBT-3`, Will's kills), that the rant scroll drops (WITHHELD, `BL-R260-DEBT-1`), that the work-layout cold build with the asset gates has run (`BL-R260-DEBT-5`), or that the independent vet has passed. **Lead with those.** [Integration 2026-09-11: two of the four are now CLAIMED - the work-layout det-2x cold build HAS run (`BL-R260-DEBT-5` discharged, section 8) and the independent vet HAS passed (its three docs fixes landed in the integration docs commit); still NOT CLAIMED: an item seen falling in play (`BL-R260-DEBT-3`) and the rant scroll (`BL-R260-DEBT-1`, withheld).]

### DEBTS (no-new-surface-without-a-gate + debt-register law)
- `BL-R260-DEBT-1` (**P1, WILL / design**): the Devourer's rant scroll + Rite channel (R-13, R-15, R-17) is WITHHELD - it never paid, and it cannot be re-homed without displacing a live hand-designed roll or waiting on `BL-R258-DEBT-1`. Options (a)-(e) costed in section 6; the lane recommends the per-player container for the rant and nothing further for the Rite (the Enslaver and both Hunts now pay it at 100%). Records kept: the master, both rant tables, the item, the three tags.
- `BL-R260-DEBT-2` (**P2, WILL / balance ratification**): five bosses stop dropping one potion per kill - the Enslaver (Misc1), the Hunt and the Endless Hunt (Misc1), the Mnemophage (Misc2), Ephialtes (Misc2) - the R-258 price, paid so the guaranteed item is the slot's only pick. Reverse = restore the muted weights (they are still named on the records) and find each item another home.
- `BL-R260-DEBT-3` (**P1, WILL / in-game**): nobody has seen one of these drops fall on this stack. Cheapest proofs: kill the Enslaver (the Rite formula on the ground every kill) and Ephialtes (the Mask); a Sileni or brute farm for a 7-10% charm.
- `BL-R260-DEBT-4` (**P2, WILL / policy ratification**): tiers B (dormant slot re-armed, inherited rows muted by weight) and D (rate-preserving share) are the lane's extensions to the brief's A/C/X policy, adopted under standing autonomy because the brief's own "re-wire every relic at its documented %" is unsatisfiable without them. Veto = FAIL LOUD on the 25 relic sources + the terminal + the wyrms + four archers, and a slot decision per record.
- `BL-R260-DEBT-5` **DISCHARGED at integration 2026-09-11** by the vet's det-2x work-layout cold build (section 8, THE ARTIFACT OF RECORD: arz `eca50cdf`, 55,631,702 B, A9 / M2 / F2 PASS under `SVC_REQUIRE_GATES=1`, contracts 0 P0 / 0 P1, record-diff vs `7dad9a8a` ADDED 0 / REMOVED 0 / CHANGED 35 - the expected figures exactly). Was (**P1, ship lane**): this lane built to a SCRATCH layout (no `Resources` beside the output), so the A9 render-chain, F2 summons-contract and `enslaver_shroud` M2 gates reported UNPROVEN and were skipped (loudly, not under `SVC_REQUIRE_GATES=1`); everything else in the battery ran. The ship lane owes the work-layout cold build under `SVC_NO_CACHE=1 PYTHONHASHSEED=0 SVC_RELEASE_DROPS=1 SVC_REQUIRE_GATES=1`, det-2x, and the record-diff against `7dad9a8a` re-run on it (expected: the same ADDED 0 / REMOVED 0 / CHANGED 35).
- Process note, not a debt (vet-corrected at integration 2026-09-11): TWO branch checkpoints carried `R-260` stamps and `tools/` changes BEFORE the `## R-260` ledger heading existed - the brief-mandated recovery checkpoint `1ffd0ec` (the first `R-260` stamps in `tools/`) and the review-fix checkpoint `4f86dab` (`charon_rework` Misc3 mute + the measured Bough verify, the frost-wyrm twins and the endless Hunt through the helper, the Devourer `dropItems` assert) - while the completing commit `efa975b` is DOCS-ONLY (the `## R-260` heading, the BACKLOG block, the MISTAKES entries; no `tools/` file). The rulings-ledger law couples ledger + tools in ONE commit, and on `main` that commit is the `--no-ff` merge commit **`b2294ac`** (2026-09-11), which carries the R-260 ledger and every R-260 `tools/` change together; the branch-side checkpoints are the reason the merge was made `--no-ff` rather than fast-forwarded. `gate_ruling_ids --vs main --branches` was run before the heading landed to confirm no other lane claims the number, and `gate_ruling_ids --vs origin/main` PASSED again on `main` after the merge (56 rulings, highest R-260; R-258, R-259 and R-260 each defined exactly once).
- Ship note, not a debt (2026-09-11): SHIPPED as **build103** to DEV (targeted copy, 1 of 62 files, rollback `local\DEV_arz_deployed_prev.arz` = build102 `7dad9a8a`) and Steam (item 3759792705, ManifestID `6416846304768403107`, push-gates PASS, contracts on dist 0 P0 / 0 P1 / 4516 P2), tag `build103-ship`; the shipped arz is section 8's artifact of record `eca50cdf` unchanged; open for Will: `BL-R260-DEBT-1` (the rant+Rite relocation), `BL-R260-DEBT-2` (the potions), `BL-R260-DEBT-3` (see one fall); gate record `docs/BACKLOG.md` BUILD103 GATE RECORD.

---

## R-261 [2026-09-11] IMPLEMENTED (branch `fix/chest-generosity` from `24d3383`/build103-ship; new artifact gate `tools/gate_chest_generosity_sv_parity.py` wired into the post-write battery of `tools/build_svc_database.py` beside the R-260 gates; ZERO record writes; **MERGED to `main` 2026-09-11 as `7801ded` `--no-ff`, independent vet GO; BUILT TO THE WORK LAYOUT under `SVC_REQUIRE_GATES=1` by that vet and the written arz is BYTE-IDENTICAL to build103 `eca50cdf` (record-diff 0/0/0), so NO build104, NO DEV deploy, NO packaging and NO Steam push - a byte-identical arz is not re-shipped**) - THE FIVE CHESTS WILL NAMED ON 2026-08-14 ARE PROVEN BY BYTES AT OR ABOVE THEIR SV 0.98i ORIGINALS, AND THE FLOOR IS NOW READ FROM THE UPSTREAM ARZ ON EVERY BUILD INSTEAD OF AUTHORED BY THE MOD

**ORIGIN:** Will's five reports, verbatim, all still on the live-play queue (`BL-W0814-2/-5/-7/-11/-13`), re-dispatched 2026-09-11 with the orchestrator's hypothesis *"FIVE chests gutted identically = ONE shared upstream cause"*:

> (2026-08-13, R-247 part 7, `BL-R247`) "wtf did you do to all the chests like toxeus the murderer devourer of blood's stash? Revert it back to what it was dropping in the original sv you nerfed the fuck out of it"
>
> (`BL-W0814-2`) "the obsidian hoard chests that we have added in the game in locations such as the obsidian halls are still nerfed too much. if these share the same record as the chest - toxeus the murderer, devourer of blood hidden's chest we need to seperate those records since those should be tuned differently."
>
> (`BL-W0814-5`) "Another terrible chest aphoryteus (spelled wrong) dread hoard is a terrible chest, it got nerfed somewhere along the way and needs to drop more items."
>
> (`BL-W0814-11`) the chest in the Great Hall of Propontis "literally just dropped two items one thing of gold and incarnation of guan-yu's grace. are you kidding me. this is outrageous"
>
> (`BL-W0814-13`) "same problem with the chest guarded by tantalus"
>
> (`BL-W0814-7`) "the gift box in the secret places need to increase the number of items dropped by 3x."

Standing autonomy (Will 08-28 "keep proceeding as you see fit", 09-06 "proceed as you see fit until completion"). No new Will words in this lane.

### 1. THE VERDICT FIRST, BECAUSE IT IS NOT THE ONE THE DISPATCH EXPECTED

**Every one of the six surfaces is ALREADY at or above its SV 0.98i original in the shipped build103 bytes (`eca50cdf`), and has been since build95 (2026-08-15).** The dispatch asked for a restore; the bytes say the restore landed four weeks ago under R-247.7a (the stash, 2026-08-13) and R-251 (the hoard family + the gift box, 2026-08-14), was re-verified live on build97, and survived every build since. **This lane wrote ZERO record changes** - not one `tables` pointer, not one `numSpawn` equation, not one weight. What it adds is the thing the repo never had: a gate whose floor is Will's yardstick (what the original Soulvizier paid) rather than a mod-authored band, so the next nerf that passes every mod-authored gate reds on THIS one. Section 10 is the DONE-means-DONE line; read it before reporting this lane as "the chests are fixed".

### 2. WHAT THE BYTES SAY, PER CHEST (measured this lane on build103 `eca50cdf9d2afbf4c1c08c66a24386ba` / 51,355 records vs `upstream/soulvizier_098i/Database/database.arz` / 51,186 records; SVAERA holds NONE of these records and is not a reference for any of them; reproduce with `py tools/gate_chest_generosity_sv_parity.py <arz> --table`)

Notation: B8 = `(3+(1.8*numberOfPlayers))` (4.8 solo), G8 = `(1+(1.8*numberOfPlayers))` (2.8 solo), B6 = `(3+(1.6*numberOfPlayers))` (4.6 solo); S = mean solo spawn iterations; mass = sum of live group chances / 100; E[items] = S x mass (the house model, `svc_loot_distribution.ChestProfile.expected_drops`).

| chest (Will's name) | records (`records\drxitem\container\`) | build103 NOW | SV 0.98i original | what Will actually opened (build92 `b888f022`) | verdict |
|---|---|---|---|---|---|
| **the Devourer's stash** ("Toxeus the Murderer, Devourer of Blood's Stash") | `hidden_bloodcave_chest_01/02/03` -> `loottable_hidden_bloodcave_01/02/03` | B8`*3.8`/`*4.1` (S **18.96**), chances 40/40/0/21.2/40/40 (mass 1.812), E[items] **34.36**; unique_1h@600, unique torso/head/arms/legs/shield@850, + `svc_unique_weapons_<t>01`@800 and `svc_unique_armor_<t>01`@1700 | B8`*3.8`/`*4.1` (S 18.96) **string-identical**; chances 14/33/0/21.2/31/14 (mass 1.132), E[items] 21.46; unique_1h@200, torso/head/shield@100, arms/legs@200 | build90: `*0.323/*0.3485` etc. = S ~1.6-2.1 (the R-240 sweep, ~11x cut) | **>= SV on every axis.** Volume = SV verbatim (R-247.7a); composition a strict superset (every chance and every unique weight >= SV, relic row 21.2 byte-equal, every SV member row present). The chest record differs from SV only by path-case normalisation of `goldGenerator` / `levelEquationFile` / `openSound`. NOTHING TO RESTORE; a second write of these fields trips `r247_bloodcave_rulings.apply()`'s fail-loud pre-state check. |
| **Obsidian Hoard** (the Obsidian Halls 25% roulette chest) | `svc_obsidianhoard_01/02/03` (via `svc_obsidianhoard_pool_0N`) -> `svc_obsidianhoard_loot_01/02/03` | its OWN table: B8`*2.4`/`*2.8` (S **12.48**), 40/40/**100**/21.2/40/40 (mass 2.812), loot3 = guaranteed `svc_unique_weapons_<t>01`@100 + `<t>_act4_relics`@60 (tier-correct, R-251 H8) | record does not exist (mod-authored, b42 era) | `boss_default_39-41/57-59/63-65` = B6`*1.5/*1.7` (S 7.36), 14/27/**10**/21.2/25/14 (mass 1.112), E[items] 8.18, NO guaranteed row; own tables orphaned at `*0.2188/*0.25` (S 1.125) | own record + own table (H7 forbids any share with the stash or another hoard); 66% of the stash's S by the R-247 crown law; the guaranteed unique+relic row the stash never had |
| **"aphoryteus dread hoard"** = Ephialtes's Dread-Hoard (`tagSVCEphialtesHoard`, the Dread Halls uber) | `svc_ephialteshoard_01/02/03` -> `svc_ephialteshoard_loot_01/02/03` | as the Obsidian Hoard (S 12.48, loot3Chance 100) | absent | `boss_default_57-59/63-65/63-65` (S 7.36, loot3Chance 10) | as above |
| **Great Hall of Propontis uber chest** (Kroisos the Coin-Drowned / Dorus) | `svc_dorushoard_01/02/03` -> `svc_dorushoard_loot_01/02/03` | as the Obsidian Hoard (S 12.48, loot3Chance 100); own family authored by R-251 in the content pass | absent | `boss_default_41-43/57-59/63-65` (S 7.36, loot3Chance 10 = "one thing of gold and incarnation of guan-yu's grace"); before b42 it had NO table of its own and SHARED `svc_obsidianhoard_loot_0N` as a `clone_record` of the Obsidian chest - the sharing Will sensed | as above. Still LABELLED "Obsidian Hoard" (`description = tagSVCObsidianHoard`, `BL-R251-DEBT-2`, carried as `BL-R261-DEBT-4`) |
| **the chest guarded by Tantalus** (Den of Tantalus, `tagSVCTantalusHoard`) | `svc_tantalushoard_01/02/03` -> `svc_tantalushoard_loot_01/02/03` (ONE world chest, R-130 `UBER_CHEST_COUNT=1`) | as the Obsidian Hoard (S 12.48, loot3Chance 100) | absent | `boss_default_51-53/63-65/63-65` (S 7.36, loot3Chance 10) | as above |
| **the Secret Present gift box** (`tagSecretPresentBOX`; 5 world placements: DarkForestEnter x1, SecretForest2 x2, PillagedVillage x2) | `sp_chest_01/02/03` -> `loottable_sp_01/02/03` | G8`*5.4`/`*6.3` (S **16.38**), chances 14/33/0/21.2/31/14 (mass 1.132), E[items] **18.54** | G8`*1.8`/`*2.1` (S 5.46), same chances, E[items] 6.18 | never nerfed: it sat at its SV value when Will asked for 3x | **exactly 3.000x the item count** (5.4/1.8 = 6.3/2.1 = 3.0); composition BYTE-EQUAL to SV on every chance AND every member weight (this lane diffed the members SV vs build: identical, closing the "not byte-diffed" gap the forensic left) |

The whole R-251 family is 12 hoard families x 3 tiers = **36 chests** on build103 (aithon, charon, diadochi, dorus, ephialtes, generalaguard/b/c, mnemophage, obsidian, tantalus, ushkaret), every one wired to its OWN table, 0 orphans, all at B8`*2.4/*2.8` with `loot3Chance` 100 (`gate_uber_hoard_generosity` PASS this lane). The family grew from 30 (R-251) to 36 by name shape alone, exactly as R-251 said it would. Composition is byte-identical across the 12 families per tier (measured: one distinct composition on 01, one on 02, one on 03) - separation is STRUCTURAL (own record per chest), tuning is not per-boss; that is a possible new content ask, not a regression.

### 3. THE SHARED CAUSE - THE ORCHESTRATOR'S HYPOTHESIS WAS RIGHT, AND IT IS ALREADY DIAGNOSED, FIXED AT THE SOURCE, MERGED AND ON STEAM

Two causes, both fixed once, both before this lane:

1. **THE WIRE (the primary cause, `git blame`):** `apply_svc_patches._svc_standardize_boss_chests`, commit **`db2d409` (2026-07-13 20:54, author wtrevena = the b42 round-2 agent lane on Will's account)**, *"b42 CHEST: reduce + region-tune every placed-uber bespoke hoard to Cyclops-grade base loot"* - an implementer scope decision recorded in `docs/reports/b42_fixedboss_dedup.md` S3.3, NEVER a Will ruling (Will's own b42 words were only *"for all the monsters we placed we need to reduce the chests, replace the current chest with three large magestic chests"* - the COUNT, later amended to one by R-130). It ran at finalization and rewrote every placed-uber hoard chest's `tables` to a base-game `boss_default_<lo>-<hi>` (S 7.36, loot3Chance 10, no guaranteed row). Its own report says: *"The old bespoke guaranteed-unique loot tables are left unreferenced."* On build92: 21 of 30 hoard chests on a base-game table; 18 of 27 `svc_*hoard_loot_0N` records with ZERO references; R-180/R-181/R-220 tuned those dead records for three waves while FOUR gates stayed green, because every gate audited the TABLE and none audited the WIRE. **Fixed by R-251** (`_svc_wire_boss_hoard_chests`, the last writer of every hoard chest's `tables`, scope derived from the name shape; `svc_loot_volume._r251_exempt`; the Dorus family authored; H1-H8; merged `ee5252d` 2026-08-14, shipped build95 2026-08-15). The repoint is RETIRED, not deleted (retirement protocol; `BL-R251-DEBT-1` records the accepted loss of region banding).
2. **THE SWEEP (the secondary cause):** R-240's `tools/patches/loot_volume_trim.py` (`53ae6d5`, 2026-08-11; Will-ratified for the polis-vault cage: *"we probably need to trip the loot-volume trim ... from the two chests, you get guaranteed 1 legendary item"*) swept the hoard family (`*2.4/*2.8` -> `*0.2188/*0.25`; invisible on the 18 orphans, REAL on the six general-guard tables still wired) and the Devourer's stash (`*3.8/*4.1` -> `*0.323/*0.3485` per tier). **Fixed by R-247.7a** (the stash, SV equations verbatim, 2026-08-13) and **R-251** (the hoards, out of the trim's scope by the same carve-out mechanism).

**No supersession is minted by R-261.** Both supersessions already exist in this ledger as amendments, not silent overrides: the R-240 entry carries the R-247.7a and R-251 carve-outs; the R-251 entry retires the b42 repoint; R-130 Part 2 amended b42's chest COUNT. Nothing here contradicts a prior ruling; the one genuine unresolved conflict is section 5.

### 4. THE GATE: `tools/gate_chest_generosity_sv_parity.py` (the fifth chest gate, and the first whose floor the mod cannot re-author)

Why a fifth: `gate_chest_loot_breadth` (R-180) audits WHAT a table can pay; `gate_loot_distribution` (R-181) in WHAT PROPORTIONS; `gate_loot_volume` (R-240) HOW MUCH, against the mod's own bands; `gate_uber_hoard_generosity` (R-251) WHETHER THE TUNED TABLE IS THE ONE THAT OPENS, and pins the family at the mod's authored value. The b42 repoint and the R-240 sweep each passed every gate of their day because every band was a mod-authored number the nerf itself could re-author (R-240 literally re-derived its own floors from the trimmed bytes). This gate's floor is READ from `upstream/soulvizier_098i/Database/database.arz` on every run through the one `check_build_inputs` resolver; the only constants in the file are PINS that prove the upstream table is the one the floor was derived on. **Precisely how far that "cannot be re-authored" claim reaches, corrected at integration 2026-09-11:** `check_build_inputs.resolve()` md5-verifies the FALLBACK ladder only - a path handed to it explicitly (`--sv`, or argv into the build) short-circuits the ladder and is used AS-IS, unhashed, by documented design ("A caller-supplied path (argv) is used AS-IS with no hashing"). So the floor is protected by TWO things, and only one of them is a hash: the ladder's md5 pin on auto-resolution, and **P6**, which reds unless the upstream table read for the floor carries the pinned SV equations. An explicitly supplied upstream arz that happens to carry those equations would still be accepted. Tightening that is `BL-R261-DEBT-6`.

Per chest, per tier, solo (`numberOfPlayers = 1`):

| arm | asserts | what it would have caught |
|---|---|---|
| **P1 WIRE** | the chest record the player opens names the table the SV original names (SV-original class) / its OWN `svc_<fam>hoard_loot_<tier>` (hoard class, derived by the R-251 name shape - 36 today, never typed) | the b42 repoint (named in the finding when the destination is `\containers\defaultloot\`), a stash/hoard share |
| **P2 COUNT** | spawn iterations min, max, mean and E[items] per open >= the SV original's; the gift box EXACTLY 3.0x (over as well as under reds); the hoard family >= the R-251 floor imported from `svc_uber_hoards.HOARD_MIN_EQ/MAX_EQ` (this gate is the family's FLOOR; H3 stays its exact PIN, so raising the hoards reds H3 alone, lowering them reds both) | the R-240 sweep on the stash, the hoards and (had it happened) the gift box |
| **P3 CHANCES** | every `lootNChance` >= SV's, so the count floor holds under BOTH engine readings of `FixedItemLoot` (independent groups per iteration, or one group per iteration - `BL-R240-DEBT-5` leaves the engine's reading unproven); hoards keep `loot3Chance` >= 100 | a chance re-nerf; the guaranteed row demoted to boss_default's 10 |
| **P4 QUALITY** | per group, the weight share on `unique` rows >= SV's, and every SV member row still present at weight > 0 (a restored chest is a SUPERSET of its original, never a swap) | a zeroed unique row, a dropped donor row |
| **P5 FORM** | both equations keep `numberOfPlayers` (the build28/29/30 "opens and drops NOTHING" P0: a bare literal can pass P2 by arithmetic while the engine evaluates it to 0) | the literal-equation P0 |
| **P6 PIN** | the upstream table carries the pinned SV equations (`B8*3.8/*4.1`, `G8*1.8/*2.1`) | a wrong or modified upstream file silently lowering the floor |
| **SURFACED, not asserted** | Leinth's chest (section 5), printed every run with `BL-R261-DEBT-1` | nothing - it is an open Will decision, and a gate must not decide it |

**Measured on build103 (this lane): PASS** - "6 SV-original chest tiers pay >= their SV 0.98i original on count, chances and unique share (the Secret Present box exactly 3.0x), 36 hoard chests open their own table at or above the R-251 floor with the guaranteed row intact (leanest svc_aithonhoard_loot_01.dbr at 12.48 iterations)". **ANTI-INERT (`--negtest` on the real build103 arz, in memory): 12 planted re-nerfs, 12/12 RED on the intended arm, 12/12 restorations GREEN** - the b42 repoint of the Propontis chest to `boss_default_41-43` (P1), the stash chest re-wired onto a hoard table (P1), the R-240 trim on the stash `*0.323` (P2), the stash max one tenth under SV (P2), the gift box un-tripled to the SV `*2.1` (P2), the gift box OVER 3x at `*5.5` (P2), R-240's `*0.2188` on a hoard (P2), a hoard's guaranteed row demoted to 10 (P3), a stash group chance under SV (P3), the stash's `unique_1h` row zeroed (P4), a bare literal on a hoard (P5), the gift box's `numberOfPlayers` stripped (P5). **`--control <arz>` demands RED on a pre-fix artifact, and at integration 2026-09-11 it was MEASURED on two real pre-fix artifacts that ARE on disk** (the earlier claim that none was is corrected here): `work\SoulvizierClassic\Database\leinth_vet.arz` (md5 `0d861748df91442ab860995cdea243eb`, 2026-07-28, 55,429,694 B) REDs with **27 problems - P1 x15** (the b42 `_svc_standardize_boss_chests` repoint, live on five hoard families x 3 tiers: charon, dorus, ephialtes, obsidian, tantalus, each naming a base-game `boss_default_<lo>-<hi>` instead of its OWN `svc_<fam>hoard_loot_<tier>`) **+ P2 x12** (the Secret Present box at the un-tripled SV `*1.8/*2.1`: min, max, mean and E[items] x 3 tiers); and `work\SoulvizierClassic\Database\SoulvizierClassic_b37fg.arz` (md5 `4f69e9ab79a4ec9c3322ec133574654d`, 55,221,697 B) REDs with **15 - P1 x3** (the Dorus family sharing `svc_obsidianhoard_loot_0N`, i.e. exactly the pre-b42 chest share Will sensed in `BL-W0814-2`) **+ P2 x12** (the same gift box). So the control arm is proven on artifacts this repo actually shipped, not only on in-memory plants, and the P1 arm is proven against BOTH historical shapes of the wire defect (the repoint and the share). `BL-R261-DEBT-5` is DISCHARGED. Wired into `tools/build_svc_database.py` after the R-260 gates, unconditionally (exit != 0 raises; under `SVC_REQUIRE_GATES=1` an unresolvable upstream arz is a build failure, and the upstream arz is a mandatory build input anyway).

**Re-banding: NONE was needed, and this lane checked all four rather than assuming.** `gate_uber_hoard_generosity` (H1-H8 + crown), `gate_loot_volume` (V1-V7b; the 36 hoard + 3 stash tables are out of its scope by `_r251_exempt` / `R247_STASH_EXEMPT`), `gate_chest_loot_breadth` (B1-B3, volume-neutral) and `gate_chest_artifacts` (composition-only) all PASS on build103 and none encodes a nerf on any of the six surfaces. The bands that WOULD red a change are documented so nobody edits them silently: lifting the hoards to the stash's 3.8/4.1 reds H3 (exact pin) and the crown check; a literal byte revert of the stash composition to SV reds R-180 B1/B2 (SV's donor rows lack spears) and R-181 parity and would pay LESS than now; restoring the cage reds V1/V6 and contradicts Will 08-11.

### 5. RULING CONFLICTS - SURFACED, NOT OVERRIDDEN

- **Will's literal "revert it back to what it was dropping in the original sv" vs how R-247.7a implemented it.** R-247.7a restored the VOLUME to SV verbatim and kept the COMPOSITION as a superset (every chance and unique weight >= SV, plus the mod's own `svc_unique_*` rows, carrying R-180/R-181/R-185). A byte-for-byte composition revert would drop the spear rows R-180 exists for and pay LESS than the shipped chest. R-261 keeps R-247.7a's reading and now GATES it: >= SV on every axis (P2/P3/P4). If Will wants the literal SV composition instead, that is a one-line ruling and P4 flips from ">=" to "==" - surfaced here so it is his call, not an implementer's.
- **THE ONE LIVE CONFLICT - LEINTH'S CHEST (`bosschest_leinth_01_normal/02_epic/03_legendary`, `tagLeinthChest` "Leinth's Essense", `DRX\meshes\leinth_chest.msh`; R-72: "Leinth drops a BESPOKE DRX CHEST").** It is an SV-ORIGINAL boss chest: SV 0.98i wires it to `loottable_leinth_29-31/49-51/63-65` at B6`*2.2/*2.4` (S 10.58, mass 1.75) with a GUARANTEED 100% amulet/relic/ring/formula row. build103 wires it to `svc_uberorb_apex_n01c/e01c/l01c` at B6`*0.2283/*0.2609` (S 1.125, mass 1.912) with that row at 21.2 - one ninth of its SV volume. Lineage: R-72/R-75/R-99 (Will 07-27/28) unified her with the Toxeus apex orb; R-240 trimmed the apex ~10x; R-241 superseded the R-72/R-99 no-nerf floor in Will's own words ("you made the orbs way too good... those dont need to have guaranteed legendary drops"); R-242 (Will 2026-08-12, verbatim part 2) FROZE the apex: *"Note that Leinth and the toxeus variants keep their current higher / better orbs / better drop rates / more loot"*; R-247.7a's both-arz sweep then classed her tables as "uber-ORB tables = the R-242 orb class, untouched here". So by RECORD and MESH she is a chest inside Will's 08-13 "all the chests like ... stash ... revert to original sv"; by RULING lineage she is an orb Will froze on 08-12. **Two Will statements one day apart point opposite ways for one record. NOT acted on.** The gate prints her every run as SURFACED. If Will rules CHEST: `bosschest_leinth_0N.tables` -> `loottable_leinth_*` (still in the db, unmodified) restores SV exactly, and needs an R-240 carve-out (`svc_loot_volume`), an R-241/R-242 amendment (`uber_apex_orb.verify`'s no-nerf proof + `orb_legendary_chance`'s exclusion set) and the roster entry flipped to ASSERTED; if ORB: the freeze stands and the INFO row is retired by a ledger line. `BL-R261-DEBT-1`.
- **The polis-vault cage** stays trimmed by Will's own 08-11 words; R-251 explicitly left it and so does R-261. The R-247.7a text's "flagged for Will below, not reverted" list for mod-authored cage/hoard/vault chests was never itemised in the R-247 section (grep finds only the sentence); the hoards were covered by R-251, the cage remains as asked. Confirm with Will that the cage trim still stands now that the hoards and the stash are back up - a question, not a change.
- **Is 2.4/2.8 the right number for the hoards?** R-251 pinned the mod's own authored value (S 12.48 = 66% of the stash's 18.96) because no SV original exists for any `svc_*hoard` chest; the only SV uber-chest datapoints are the stash (3.8/4.1, no guaranteed row) and Leinth (2.2/2.4 on the 1.6 bracket with a 100% row). If Will, after opening them (`BL-R261-DEBT-2`), wants more: the levers are `svc_uber_hoards.HOARD_MIN_MULT/HOARD_MAX_MULT` + the monolith's authored strings + an R-251 amendment; values >= 3.8/4.1 also repeal the R-247 crown law. Never a silent edit.

### 6. THE QUESTION NO TABLE CAN ANSWER

The `boss_default_41-43` table Will opened in Propontis evaluates to 6.90..7.82 spawn iterations solo at mass 1.11 - roughly eight rolls - yet he saw two items. Either the engine's evaluation or rounding of `numSpawn*Equation` is not what the arithmetic says (`BL-R240-DEBT-5`, still open; R-240 enforces both a continuous and an integer-truncated reading for exactly this reason), or the rolls landed on gold and misc. R-251's 12.48 and the gift box's 16.38 are table-derived expectations, not observed counts. **The single biggest risk on this item is that Will re-reports the same chests after build103, and only an in-game count settles it** (`BL-R261-DEBT-2`, the live-play queue's closing proof, carried from `BL-R251-DEBT-3`).

### 7. PLAYER-SURFACE CHECKLIST (process law 3, enumerated)

No new record, no new Text tag, no map, no quest, no mesh: this lane ships a gate and docs. The player surfaces it protects already exist: the Devourer's stash (blood cave), the Obsidian Hoard roulette (Obsidian Halls), Ephialtes's Dread-Hoard (Dread Halls), the Propontis uber chest (still named "Obsidian Hoard" - `BL-R261-DEBT-4`), the Tantalus hoard (Den of Tantalus), the Secret Present box (5 places). What to open and what to count is in `docs/WILL_TEST_GUIDE.md` (the R-261 note). No colour, silhouette or in-game count is claimed by this lane.

### 8. BUILD EVIDENCE - the implementer's SCRATCH build AND the artifact of record: the independent vet's WORK-LAYOUT cold build under `SVC_REQUIRE_GATES=1` (both 2026-09-11)

**THE ARTIFACT OF RECORD (the vet's, and the reason there is no build104):** the independent vet ran the cold build in the WORK layout with `SVC_REQUIRE_GATES=1`, so the asset gates that a scratch layout cannot run actually ran - **A9 PASS**, **F2 0 P0 / 0 P1 / 111 P2** - and the written arz is **BYTE-IDENTICAL to build103: md5 `eca50cdf`, record-diff ADDED 0 / REMOVED 0 / CHANGED 0**. That is the whole shipping consequence of R-261 stated as a measurement: the lane's gate is now inside the build and cannot be built around, and the bytes the player runs do not move. `BL-R261-DEBT-3` is DISCHARGED by that build. Because the arz is byte-identical, the integration deliberately stops at merge + docs: **no build tag, no DEV deploy, no packaging, no Steam push** (recorded as the integration decision in `docs/BACKLOG.md` and `docs/HANDOFF_LIVE_STATE.md`).

**The implementer's earlier SCRATCH build, kept for the record (not the artifact of record):**

**Cold build with the R-261 gate wired, exit 0, and the written arz is BYTE-IDENTICAL to build103: md5 `eca50cdf9d2afbf4c1c08c66a24386ba`, 55,631,702 B, 51,355 records** (`PYTHONIOENCODING=utf-8 PYTHONHASHSEED=0 SVC_NO_CACHE=1 SVC_RELEASE_DROPS=1 SVC_NO_STAGE_BASELINE=1 py tools/build_svc_database.py <sv098i> <sv09> <sv041> <scratch>/Database/SoulvizierClassic.arz <base-game arz>`; no `SVC_REQUIRE_GATES=1` because the scratch layout has no Resources beside the output, so A9 and F2 reported themselves DID NOT RUN exactly as R-260's scratch build did). So the record-diff against build103 is ADDED 0 / REMOVED 0 / CHANGED 0 by identity - the gate writes nothing, and the proof is the hash. In the post-write battery the gate ran AFTER `gate_no_phantom_loot_slots` (PASS) and `gate_guaranteed_drops_measured` (PASS) and printed the same parity table as the standalone run (stash S 18.96 = SV, mass 1.812 vs 1.132, E[items] 34.36 vs 21.46; gift S 16.38 = 3.0 x 5.46, mass 1.132 = SV; leanest hoard 12.48 = the R-251 floor, 36 chests; Leinth surfaced x3) and `gate_chest_generosity_sv_parity: PASS`. Every registry verify and every earlier artifact gate was green (`Done.`). Static proof this lane: `py -m py_compile` OK on both files; the gate PASS + `--negtest` 12/12 on build103; `--table` and `--control` code paths exercised (a GREEN control correctly fails the control arm); `gate_uber_hoard_generosity` PASS (36/36/0 orphans), `gate_loot_volume` PASS (39 + 18 surfaces), `gate_chest_loot_breadth` PASS, `gate_chest_artifacts` PASS, all on build103; `gate_ruling_ids --vs main --branches` PASS (adds [261], 57 rulings, highest R-261).

### 9. WHAT THIS RULING DOES NOT TOUCH (asserted, never written)

The three stash tables (R-247.7a owns them; `r247_bloodcave_rulings.apply()` fails loud on drift), the 36 hoard tables and their wires (R-251 owns them), the three gift tables (R-251's module owns the x3), the polis-vault cage (R-240, Will 08-11), every orb table incl. the apex Leinth opens (R-242), `bosschest_highpriest_0N` and `typhonorb_chest` (unchanged from SV in both arzs), the 226 base-game `bosschestNN_*` rows (identical to SV bar the three R-247.1 charon rows), any composition anywhere (R-180/R-181/R-220/R-185/R-240-artifacts).

### 10. WHAT IS CLAIMED AND WHAT IS NOT - the DONE-means-DONE line

CLAIMED: the six surfaces are at or above SV in the shipped bytes (proven by field-level diff against the upstream arz, not by reading a ledger); the shared cause is named, blamed and already fixed at the source; a floor gate now reads SV on every build and reds on every re-nerf this repo has actually shipped; the ledger, BACKLOG, MISTAKES and test guide say so. NOT CLAIMED: that Will has seen more items fall (nobody has counted one of these chests in game since build95 - section 6); that Leinth's chest is right (section 5); that 2.4/2.8 is Will's number rather than the mod's (section 5). **Now PROVEN, having been in the NOT-CLAIMED list until integration:** the gate HAS run inside a real work-layout cold build under `SVC_REQUIRE_GATES=1` with A9 and F2 live, and that build's arz is byte-identical to build103 (section 8; `BL-R261-DEBT-3` DISCHARGED). `BL-W0814-2/-5/-7/-11/-13` are marked FIXED BY R-261 on the queue; they CLOSE only on Will's in-game COUNT (`BL-R261-DEBT-2`) - and because the arz never moved, that count is taken on build103 itself, which is already live on DEV and Steam.

### DEBTS (no-new-surface-without-a-gate + debt-register law)

- `BL-R261-DEBT-1` (**P1, WILL / ruling**): Leinth's chest - chest (restore `loottable_leinth_*` = SV, S 10.58, 100% row; needs the R-240 carve-out + R-241/R-242 amendment named in section 5) or orb (keep the R-242 freeze at S 1.125)? Surfaced every run by the gate; not acted on.
- `BL-R261-DEBT-2` (**P1, WILL / in-game, the closing proof; carries `BL-R251-DEBT-3`**): open the Propontis chest, the Tantalus hoard, Ephialtes's Dread-Hoard, an Obsidian Hoard, the Devourer's stash and one Secret Present on build103 or later (full quit TQ + restart Steam first) and COUNT the items; the engine's reading of `numSpawn` is unproven (`BL-R240-DEBT-5`), so the table numbers are expectations. Expected solo: stash ~19 iterations (34 items by the house model), hoards ~12.5 with one guaranteed unique + relic, gift ~16.
- `BL-R261-DEBT-3` (**P2, vet / ship**) - **DISCHARGED 2026-09-11 at integration.** The independent vet's WORK-LAYOUT cold build ran under `SVC_REQUIRE_GATES=1` with the asset gates live: **A9 PASS, F2 0 P0 / 0 P1 / 111 P2**, written arz **byte-identical to build103 `eca50cdf`**, **record-diff 0/0/0**, the R-261 gate PASS in the post-write battery. Nothing is owed; the identity of the arz is also why no build104 exists.
- `BL-R261-DEBT-4` (**P2, WILL / naming; carries `BL-R251-DEBT-2`**): the Propontis chest is still labelled "Obsidian Hoard" (`svc_dorushoard_*.description = tagSVCObsidianHoard`); a minted `tagSVCDorusHoard` in the amgoz1 voice is an arz+Text coupled ship.
- `BL-R261-DEBT-5` (**P3, tooling**) - **DISCHARGED 2026-09-11 at integration.** Two real pre-fix artifacts ARE on disk and both RED under `--control` (the measurement is in section 4): `work\SoulvizierClassic\Database\leinth_vet.arz` (md5 `0d861748`, 2026-07-28) **27 problems - P1 x15** (the b42 repoint on five hoard families x 3 tiers) **+ P2 x12** (the un-tripled gift box), and `work\SoulvizierClassic\Database\SoulvizierClassic_b37fg.arz` (md5 `4f69e9ab`) **15 - P1 x3** (the Dorus -> `svc_obsidianhoard_loot_0N` share) **+ P2 x12**. The in-memory `--negtest` 12/12 stands beside them, not in place of them. build92 `b888f022` is still gone; nothing further is owed.
- `BL-R261-DEBT-6` (**P3, tooling; registered 2026-09-11 at integration**): `gate_chest_generosity_sv_parity.load_sv()` should md5-check an EXPLICITLY supplied upstream path (`--sv`, or argv into the build) against `check_build_inputs.EXPECTED_MD5['sv098i_arz']` (`11773cdc9bffd9a1b9753beab718843c`) before reading a floor from it - `resolve()` hashes the fallback ladder ONLY and short-circuits on a caller-supplied path by documented design, so today an explicitly supplied arz is trusted unhashed and only P6's equation pin stands between it and a lowered floor. Alternative or additional lever: pin the SV chance vector `14/33/0/21.2/31/14` in `SV_PINS` beside the two equation pins, so a doctored upstream that keeps the equations but drops the chances still reds P6. Not fixed in this lane because it is a gate-hardening change to a gate that is currently PASS on the shipped bytes, and it is not a shipping risk (the build resolves the upstream through the hashed ladder).
- `BL-R261-DEBT-7` (**P2, WILL / decision; registered 2026-09-11 at integration; carries `BL-R261-DEBT-1`**): with R-261 on `main`, Leinth's chest (`bosschest_leinth_01/02/03`) is now SURFACED by a gate that runs on **every build** - measured at B6`*0.2283/*0.2609` (S 1.125) against SV's B6`*2.2/*2.4` (S 10.58) = **1/9.4 of its SV volume**. The gate will print those three INFO rows forever until a ruling retires them. Restoration is Will's call and nobody else's (the two-Will-statements conflict in section 5); the implementation cost if he says CHEST is named in section 5 (repoint + an R-240 carve-out + an R-241/R-242 amendment + the roster flip), and if he says ORB the INFO rows are retired by one ledger line.
- Carried, not re-minted: `BL-R251-DEBT-1` (region banding lost with the retired b42 repoint - does Will want it back on top of the bespoke tables?), `BL-R251-DEBT-5` (the diadochi registry module's fallback to the SHARED `_SVC_HOARD_POOL` should fail loud; H7 cannot see a pool-level share), the per-boss hoard flavour question (12 families share one composition per tier by design - a new content ask if Will wants it), and the cage-trim confirmation (section 5).
