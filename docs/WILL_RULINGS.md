...
### 8. BUILD EVIDENCE (this lane; filled in below the DEBTS when the scratch cold build completes - the work-layout build under `SVC_REQUIRE_GATES=1` is the vet's)

The gate writes nothing, so the expected record-diff of any build carrying R-261 against build103 `eca50cdf` is ADDED 0 / REMOVED 0 / CHANGED 0 and the expected arz is byte-identical `eca50cdf9d2afbf4c1c08c66a24386ba`. Static proof this lane: `py -m py_compile` OK on both files; the gate PASS + `--negtest` 12/12 on build103; `gate_uber_hoard_generosity` PASS (36/36/0 orphans); `gate_ruling_ids --vs main --branches` PASS (adds [261]).
...
- `BL-R261-DEBT-3` (**P2, vet / ship**): the gate has not run inside a real cold build in this lane (static gates only; the scratch cold build's result is recorded under section 8 when it lands). The vet's work-layout cold build under `SVC_REQUIRE_GATES=1` is the proof; expected arz byte-identical to build103 `eca50cdf`.
