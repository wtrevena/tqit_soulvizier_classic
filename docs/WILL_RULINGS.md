...
## R-260 [2026-09-10] IMPLEMENTED (branch `fix/misc4-audit`, resumed in the main tree after the first implementer was killed by a session limit with ~16 files uncommitted - see `docs/MISTAKES.md` 2026-09-10; helper `tools/svc_loot_slots.py` + `apply_svc_patches._svc_guarantee_unique`, registry module `tools/patches/phantom_loot_slots.py`, gates `tools/gate_no_phantom_loot_slots.py` / `tools/gate_guaranteed_drops_measured.py`; [integration 2026-09-11: the independent vet PASSED, its det-2x work-layout cold build is THE ARTIFACT OF RECORD (section 8), MERGED to `main` as `b2294ac` (`--no-ff`); NOT deployed, NO tag] was: PENDING the independent vet and the work-layout cold build) - THE `Misc4` AUDIT: 32 DROPS RODE A SLOT THE ENGINE DOES NOT DECLARE; 31 ARE RE-HOMED ONTO DECLARED SLOTS AT MEASURED RATES, THE DEVOURER'S RANT+RITE CHANNEL IS WITHHELD FOR WILL'S CALL
