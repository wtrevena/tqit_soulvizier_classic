...
- R-80 [2026-07-27] IMPLEMENTED b92 (fix/green-diff, tag build53-dev) - THE GREEN GLOW, verbatim: "i am pretty sure it is a skill or ability or something that is causing the green glow. i think this was inherited from the Toxeus the Murderer uber boss base monster that we created these monsters off of. If you compare the visuals to the secret passage toxeus the murderer who doesnt have the green glow, you may be able to find the difference". Will's method WAS the fix: the secret-passage Toxeus `um_toxeus_99` is the CONTROL and the differential against it produced the answer immediately after four failed rounds. ROOT CAUSE, PROVEN FROM ASSET BYTES (not names): the `mesh` field. A TQ `.msh` can carry an embedded CreateEntity block, and `Creatures\Monster\Skeleton\RevenantPoison.msh` ends with `CreateEntity { attach = "Waist"; entity = "Records\Effects\MonsterFX\Buffs\RevenantPoison_FX.dbr" }`. That record is ABSENT from the mod arz and resolves from the BASE game db (Class=EffectEntity, effectFile=`Effects\MonsterFX\Buffs\RevenantPoison.pfx`, boneList=Bone_R_Weapon;Bone_L_Weapon), and the .pfx colour keyframe tracks decode to R 0.534 / G 1.000 / B 0.591 = GREEN. The channel read is self-validating: the identical offsets in the sibling files give frost ice-blue (B 1.000) and storm blue-violet (green LOWEST), and three independent decoders (implementer, vet, round-2) agree to 3 decimals. The Devourer + Enslaver INHERITED that mesh from the green-by-design Greece Toxeus `um_toxeus_21`; the control wears `RevenantStorm.msh`. Roster corroboration: every OTHER RevenantPoison.msh wearer also wears `newskeleton_grean.tex` - vanilla pairs that mesh with the green skin on purpose, and our Toxeus records were the only wearers given a crimson/charcoal skin. ⚠️ THE LESSON, now a standing rule: **a `.msh` can carry its own FX attachment, so "the DB is clean" NEVER proves a visual is clean.** b55 (FX fields), b71 (soul->skill->icon->pet chain), b75 (shroud pak) and b81 (pet identity) were all DATABASE fixes and could not see it; b75 even opened RevenantPoison.msh, read its texture strings and cleared it as "innocent" without reading the trailing entity script. Decisive tell available the whole time: the Devourer carries NO `charFxPakRunningNames` at all, so no shroud theory could ever have explained his green. R-10 (b75's shroud swap) was a REAL fix to a REAL second surface - it was simply not this bug. FIX: `mesh` -> `Creatures\Monster\Skeleton\GoldenSkeleton01.msh` on exactly 15 records - the 2 fought bosses, their 6 soul pets, the 3 End-of-All-Things soul pets cloned from them, and their 4 spawn proxies. GoldenSkeleton01 was chosen on re-derived bytes after round 1's pick (Skeleton01.msh) was rejected by the vet as an unproven rig for live creatures: it carries NO CreateEntity at all, its bone-name set is EXACTLY RevenantPoison's 24 bones (zero missing, zero extra; the earlier "25" was a miscount - bone names must be regexed out of the RAW mesh bytes, since a printable-string-run scan drops non-run-initial `Bone_Neck01` and fabricates a phantom delta), it is the canonical mesh for the `anm_skeleton01` table all 15 records use (307 mod / 299 base records vs ONE for Skeleton01, an unreferenced zzdev dummy), it has 285 Monster + 22 Pet + 121 Proxy live wearers (19 of the Pet-class wearers are soul pets in the same folder as ours, and the 121 Proxy wearers already cover our 4 proxy targets, so all 15 records can share one mesh) where Skeleton01 is a Proxy placeholder (203/205 mod, 278/279 base), and it keeps the same StandardSkinned shader + GoldenSkeleton01BMP bump; its default diffuse never renders because every target overrides `baseTexture`. ⚠️ WILL-DECISION SURFACED, not assumed: this moves the Devourer off the exact Athens mesh recorded as directive #2 ("the GREEN Athens Toxeus, but RED"). Removing the green REQUIRES leaving that mesh; GoldenSkeleton01 keeps the identical rig, silhouette family and crimson skin, so he still reads as the Athens boss in red - but if Will wants the literal Athens mesh back, the only other options are to accept the green or accept a non-green aura (RevenantStorm = blue-violet). That comment block in apply_svc_patches.py is also CORRECTED: revenantstorm.msh is NOT "a DIFFERENT rig" - all four Revenant meshes and GoldenSkeleton01 share one 24-bone set. Owner: registry module `tools/patches/toxeus_mesh_aura.py`, whose `verify()` gates it in BOTH directions (no target on a GREEN or never-byte-audited mesh; `um_toxeus_21`'s intentional green and the `um_toxeus_99` control both preserved) and deliberately does NOT outlaw non-green auras, which is Will's art call and not the gate's. Committed negative test `tools/test_toxeus_mesh_aura.py`, 23/23 legs, including the decisive one: the deployed arz `1c27d5fa` that Will filed the bug against FAILS the gate. NOT TOUCHED, deliberately: the Greece green Toxeus (Will: intentional), the secret-passage control, and every vanilla RevenantPoison wearer. docs/reports/b92_green_differential.md.
  > ⚠️ **ID CORRECTED 2026-07-28 (b92 round 2).** Round 1 filed this ruling as **R-50**, colliding with the 2026-07-16 Process meta-ruling R-50; the vet caught that and round 1 refiled it as **R-19** on the belief that "R-18 was the max" in the Toxeus arc. That was a stale read - the b84 historical backfill (merged to main in the debt-wave integration) had already filled R-14..R-19, and **R-19 is the parchment-room retirement ruling**. So this entry collided a SECOND time. Per this ledger's own law a number in use is NEVER reused or reassigned, so the live R-19 keeps its number and **this ruling moved to R-80**: the Toxeus decade 1-19 is now FULL, so - exactly as the b84 pass did for Souls when 40-49 filled (it allocated the 70-79 overflow for R-70/R-71) - this pass allocates the **Toxeus arc OVERFLOW decade 80-89** (recorded in the header). Nothing about the ruling itself changed. Round 1 also edited the existing R-10 entry in this append-only file; that edit is REVERTED and its cross-reference lives inside this entry instead.
...
## **2026-07-28 (b92): the Toxeus arc decade 1-19 is now FULL too (the b84 backfill took
## R-14..R-19) - its OVERFLOW decade is 80-89. b92's green-glow ruling is R-80.**
