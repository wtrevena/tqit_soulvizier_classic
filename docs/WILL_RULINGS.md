...
Why a fifth: `gate_chest_loot_breadth` (R-180) audits WHAT a table can pay; `gate_loot_distribution` (R-181) in WHAT PROPORTIONS; `gate_loot_volume` (R-240) HOW MUCH, against the mod's own bands; `gate_uber_hoard_generosity` (R-251) WHETHER THE TUNED TABLE IS THE ONE THAT OPENS, and pins the family at the mod's authored value. The b42 repoint and the R-240 sweep each passed every gate of their day because every band was a mod-authored number the nerf itself could re-author (R-240 literally re-derived its own floors from the trimmed bytes). This gate's floor is READ from `upstream/soulvizier_098i/Database/database.arz` on every run through the hash-verified `check_build_inputs` ladder; the only constants in the file are PINS that prove the upstream table is the one the floor was derived on.
...
### 8. BUILD EVIDENCE (this lane, 2026-09-11; a SCRATCH layout, NOT the artifact of record - the work-layout build under `SVC_REQUIRE_GATES=1` is the vet's)
...
- `BL-R261-DEBT-3` (**P2, vet / ship**): the gate HAS run inside a cold build in this lane (section 8: scratch layout, exit 0, arz byte-identical to build103 `eca50cdf`, the gate PASS in the post-write battery), but A9 / F2 did not run there (no Resources beside a scratch output). The vet's WORK-LAYOUT cold build under `SVC_REQUIRE_GATES=1` is the artifact of record; expected arz byte-identical `eca50cdf`, record-diff 0/0/0.
...
- `BL-R261-DEBT-5` (**P3, tooling**): no pre-R-251 artifact is on disk for `--control` (build92 `b888f022` is gone); the in-memory `--negtest` on build103 is the anti-inert control of record. If a build92 arz resurfaces, run `--control` against it once and record the RED here.
