...
# Doctor cannot detect a NAS path, so the operator-set lines are PRESERVED from an existing
# config.env; a re-run never silently drops them (deploy refuses to run without the root).
PRESERVED_KEYS=""
if [[ -f "$CONFIG_FILE" ]]; then
    PRESERVED_KEYS="$(grep -E '^[[:space:]]*(WIN_BACKUP_ROOT|BACKUP_KEEP)[[:space:]]*=[[:space:]]*[^[:space:]]' "$CONFIG_FILE" || true)"
fi
if [[ -n "$PRESERVED_KEYS" ]]; then
    pass "Preserved operator keys: $(echo "$PRESERVED_KEYS" | tr -d '\r' | tr '\n' ' ')"
else
...
if [[ -n "$PRESERVED_KEYS" ]]; then
    {
        echo "# Operator-set (R-259): preserved by doctor.sh, never auto-detected"
        echo "$PRESERVED_KEYS"
    } >> "$CONFIG_FILE"
fi
