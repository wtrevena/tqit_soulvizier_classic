...
# R-259: deploy backups live on the network drive (WIN_BACKUP_ROOT, rotated by BACKUP_KEEP).
# Doctor cannot detect a NAS path, so it PRESERVES the operator-set lines from an existing
# config.env; a re-run never silently drops them (deploy refuses to run without the root).
$preservedKeys = @()
if (Test-Path $ConfigFile) {
    $preservedKeys = @(Get-Content $ConfigFile | Where-Object { $_ -match '^\s*(WIN_BACKUP_ROOT|BACKUP_KEEP)\s*=\s*\S' })
}
if ($preservedKeys.Count -gt 0) {
    foreach ($k in $preservedKeys) { Pass "Preserved operator key: $k" }
} else {
    Skip 'WIN_BACKUP_ROOT not set: deploy_to_custommaps.ps1 refuses to run until you add it to local/config.env (README, R-259)'
}
...
if ($preservedKeys.Count -gt 0) {
    ...
    $configContent += $nl + '# Operator-set (R-259): preserved by doctor.ps1, never auto-detected' + $nl + ($preservedKeys -join $nl)
}
